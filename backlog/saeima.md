...
### [FIX] Dzīvās sēdes darba kārtība: apakšpunktu (`getTechDKP`) balsojumi helperim neredzami — 34 argumentu forma salabota 2026-09-23

> **Daļēji slēgts 2026-09-23 (`a1a50da8`).** Rīcības (1) un (3) izpildītas: 34 argumentu formā helperis GUID lasa no `[5]` ar GUID validāciju un `[28]` `voteType` vārtiem, 21 argumenta forma nemainīta; fikstūra — verbatim izsaukumi no 2026-09-17 `actual=1` lapas, un `nr=` formas izsaukums (tur `[5]` ir 32-hex punkta UNID) apzināti nedod URL, lai nerastos dublikāti ar `addVotesLink`. Kas to izpildīja: 16 testi `tests/test_saeima_live_agenda_pattern4.py` + vienreizējs palaidiens pret īsto 09-17 lapu — `actual=1` → **32** URL (agrāk 0), `nr=` → 37 caur `addVotesLink`, 0 dublikātu. Dzīvā sēdē vēl NAV izpildīts. **Atvērts paliek:** (2) apakšpunktu balsojumi (5 no 37) — vajag atsevišķu `getTechDKP` fetch katram `hasTechChilds=1` punktam izsaucēja pusē, ne tīrajā parserī (atbilde nes tos pašus 34 argumentu `drawDKP_*`, tāpēc salabotais helperis to ķermeni jau parsē); un blakus atradumi (a)–(c) zemāk.
