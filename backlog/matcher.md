...
### [DAĻĒJI SLĒGTS 2026-09-22] Uzvārds kā apakšvirkne garākā vārdā — 7 rindas dzēstas, klases atlikums neizmeklēts

Saistītājs pirms vārda robežas labojuma ķēra uzvārdu kā apakšvirkni un pielika politiķim rakstu par pavisam citu tēmu. **Pašreizējais saistītājs tās vairs neradītu** (pārbaudīts, palaižot `match_politicians` pār visiem 7 dokumentiem), bet vecās rindas palika DB un bija redzamas publiskajos profilos.

**Dzēsts (operatora atļauja 2026-09-22, rollback `data/rollback/20260922-114435-document_politicians-substring-false-links.sql`):**

| Politiķis | Ko raksts tiešām satur | Dok | Bija profilā |
|---|---|---|---|
| Jāzeps Baško (30) | „Baškortostānas", „Baškīriem", „Bašņeftj" | 29954, 20954 | jā, abi |
| Gundars Daudze (83) | „Daudzeva" (Aizkraukles nov. ciems) | 2867, 2888, 3041 | 2 no 3 |
| Igors Rajevs (121) | „Rajevska" (cits cilvēks) | 3093, 3519 | nē |

Vārtu tur `tests/test_matcher.py::test_surname_substring_in_longer_word_is_not_a_match` (4 parametri ar īsto dokumentu tekstu).

**Atlikums — `scripts/audit_stale_politician_links.py`** (tikai lasa; pārlaiž saistītāju pār 9 724 ne-X dokumentiem ar piesaistēm, 41 s): **608 rindas, ko saistītājs vairs neradītu**, no tām **242 „teksts nepamato"**. 2026-09-22 abas sākotnējās neskaidrības izmeklētas un skripts tagad katrai rindai nosauc iemeslu:

| Iemesls | n | Vai defekts |
|---|---|---|
| `institucija` — organizācija/žurnālists, piesaiste no avota, ne teksta | 64 | nē |
| `varda_nav_tekstā` — glabātais teksts ir **nogriezta ekstrakcija** | 51 | nē (sk. zemāk) |
| `apakšvirknes_slazds` — uzvārds tikai garāka vārda iekšienē | 50 | **jā** |
| `teksts_nomainits` — dokuments pārskrāpēts PĒC piesaistes | 35 | nē |
| `piesaiste_velak` — piesaiste no vēlākas kārtas | 23 | jāskata |
| `url_celš` — uzvārds URL-ā (autora sleja) | 12 | nē |

**(a) „Teksts mainījies pēc piesaistes" apstiprinājās, un tam ir divi dažādi mehānismi.** Pirmais: `src/db.py:372` — tas pats URL ar citu saturu **pārraksta `content`**, bet vecās junction rindas neizdzēš (35 rindas; atpazīstamas pēc tā, ka `scraped_at` ir vēlāks par `created_at` vairāk nekā par 3 h — tik daudz, cik ir starp `now_lv()` UTC+3 un SQLite `CURRENT_TIMESTAMP` UTC). Otrais, biežākais: **glabātais teksts ir īsāks par īsto rakstu**. Pierādījums: doc 42838 (`nra.lv/.../522071`) glabā 313 vārdus, kuros nosaukta tikai Mieriņa, bet 16 ministru piesaistes tapušas 31 s pirms satura ieraksta tajā pašā ingest solī — un dzīvajā rakstā „Abu Meri", „Kozlovskis" u.c. **ir** (pārbaudīts ar `curl` 2026-09-22). Tajā pašā minūtē ingestētajiem 17 rakstiem par to pašu notikumu 15 no 17 teksts pamato visas piesaistes, tātad tā nav partijas kontaminācija, bet konkrētu rakstu ekstrakcijas robs → `avoti.md`.

**(b) Klasifikatora kļūdas apstiprinājās un ir izlabotas skriptā.** Sākotnējais lasījums deva 1 565 „defektus"; pēc `x_mention` izslēgšanas (tur subjekts ir tvīta autors, kura vārds tekstā neparādās) palika 242. Institūcijas (`relationship_type` organization/journalist/neutral) un URL ceļa autora slejas tagad atpazīst atsevišķi. Vienīgā palikusī nepareizā rinda 50 defektu sarakstā ir **Šnore (doc 71307)**: tekstā ir „Šnorem" — īsts vīriešu dzimtes datīvs, ko `_latvian_surname_inflections` negenerē (`-e` uzvārdiem taisa tikai sieviešu formas). Tas ir atsauces robs, ne viltus piesaiste. Pretēji: „Kalniņam", „Siliņam", „Daudzeses", „Viļumsons" ir **citu cilvēku** vārdi, tāpēc tās rindas paliek defektu sarakstā.

**IZPILDĪTS 2026-09-22: 49 rindas dzēstas** (rollback `data/rollback/20260922-143025-document_politicians-substring-batch2.sql`; pēc tam audits rāda apakšvirknes klasē 1 rindu — palikušo Šnori). Dzēstās bija 40 `subject` + 9 `mentioned` uz 46 dokumentiem; `check_output` iekšējās atsauces 56 870 → 56 837 (−33, nevis −49, jo politiķa lapas rāda `LIMIT 50` logu — vecāku rakstu izņemšana logā neparādās). Saraksts bija: (50 mīnus Šnore) — Kols 22 („Kolumbija", „skolotājs"), Daudze 6 („daudzi", „daudzums"), Lāce 5 („lācis"), Vilks 5 („vilki"), Lūsis/Lūse 4 („Lūsija"), Liepiņš 2 („Liepiņas"), Kalniņa/Grasbergs/Viļums/Krasta/Siliņa pa 1. **Visas izveidotas 2026-04…07, nevienas pēc vārda robežas labojuma (`d9eb4f9f`, 2026-07-27)** — labojums tur, atlikums bija vēsturiskas druskas.

**[IZPILDĪTS 2026-09-22 — 1. punkts]** `-e` uzvārdu vīriešu datīvs `-em` tagad tiek ģenerēts (`src/matcher.py`, SWE-2 max aģents; commit zemāk). Izvēle: over-generation visiem `-e` uzvārdiem, jo dzimums DB nav glabāts un zars jau strādāja asimetriski otrādi (vīrieša uzvārds dabūja sieviešu `-ei`). **Neatkarīgi pārmērīts orķestratorā, jo aģents ziņoja tikai par 1 politiķi:** 27 jaunas formas, trāpījumi **54 dokumentos pāri 5 vīriešiem** — Krauze 39, Šnore 6, Pūce 5, Zīle 3, Daudze 1; pārējām 22 (sieviešu fantomformas) **nulle** trāpījumu. Viens viltus kandidāts korpusā tomēr ir — doc 64701 „pieminekļi Jānim Cimzem un **Indriķim Zīlem**" (cits cilvēks, ne Roberts Zīle) — bet `match_politicians` to **nepiesaista**, jo priekšvārds nesakrīt; t.i. veto ķer, un aģenta apgalvojums „nulle viltus kandidātu" bija nepareizs, lai gan secinājums sakrita. Audita defektu klase 1 → **0**. Testi: `tests/test_matcher.py` 58 (4 jauni).

**[IZPILDĪTS 2026-09-22 — 1. punkts]** Saistītājs pārlaists pār **51 dokumentu** (nevis 54 — tie bija dokuments×forma pāri), kuros parādās `-em` forma. Tikai **5** dokumentos rindas trūka; pārējos 46 tā jau bija pa citu ceļu. Pievienotas **17 rindas** (6 `-em` politiķiem: Šnore ×2, Krauze ×3, Pūce ×1; 11 citiem, ko šodienas saistītājs redz, bet ingest brīdī neredzēja), noņemtas 0. Rollback `data/rollback/20260922-204021-document_politicians-em-relink.sql`. **Viltus kandidāti noraidīti:** doc 64701 „Indriķim Zīlem" (cits cilvēks), 93457 un 95612 (uzvārds bez priekšvārda) — nevienā Roberts Zīle netika piesaistīts. Blakusefekts → `vietne-ui.md` (8 lomu pāri; render vaicājums bez `DISTINCT`).

**[IZPILDĪTS 2026-09-22 — 4. punkts, bet NE tā, kā bija iecerēts]** `src/db.py` same-URL UPDATE zars tagad izsauc `_reconcile_junction_suspects`: rinda, ko jaunais teksts nepamato, dabū **`suspect_at` karogu, nevis tiek dzēsta**. Iecerētā dzēšana izrādījās nepierādāmi droša — jaunā teksta pilnvērtīgumu nevar pierādīt (doc 44422: 100 glabāti vārdi, bet dzīvajā jauns.lv rakstā visi 7 „novecojušie" vārdi IR; teksts var būt garāks un joprojām nogriezts), un junction rindām nav provenances kolonnas. Attaisnojuma klases sakrīt ar audita skriptu. Testi `tests/test_junction_suspect_flags.py` (14). **Vēsturiskās 35 `teksts_nomainits` rindas netiek karogotas atpakaļejoši** — karogs strādā tikai turpmākajām pārrakstīšanām; vienreizēja apzīmēšana gaida operatora vārdu.

**Paliek atvērts pēc šīs kārtas:** (1) `varda_nav_tekstā` 51 rinda gaida `avoti.md` truncated-backfill kampaņu, ne dzēšanu; (2) `piesaiste_velak` 23 rindas — jānoskaidro, kurš ceļš tās raksta.
...
- **Bērziņi.** pid=146 Andris Bērziņš (ZZS) pret dziedātāju doc 64681 — vārds tur ir tikai solistu uzskaitījumā bez profesijas vārda blakus (§ Bērziņš false-link). **Otrā instance 2026-09-23:** doc 110049 (LSM 17.09.) — runā SM valsts sekretāra vietnieks **Kaspars** Bērziņš; `negative_patterns` «Kaspars Bērziņ»/«sekretāra vietnieks Bērziņ» veto noķer tikai pirmo pieminējumu, bet 4× kails «Bērziņš» tālāk tekstā saiti izveido no jauna («Andr» tekstā 0×). Junction dzēsts ar operatora lēmumu (`scripts/fix_operator_verdicts_2026-09-23.py`, rollback `data/rollback_operator_verdicts_2026-09-23.sql`); pozīcija netika glabāta (T1 vārti ekstrakcijā strādāja). Mehānisma papildkandidāts: **`negative_patterns` trāpījums dokā bez neviena pilnvārda trāpījuma = šajā tekstā kailais uzvārds pieder citam** → kailās formas šim dokam nesaista.
...
### [OPERATOR] 2026-09-07 seedēšanas blakuskarogs — Lūša ≤4 zīmju formas (T1); (b) Pujāts un (c) Meļņa četras rindas slēgti 2026-09-15
...
- **(b) SLĒGTS 2026-09-15:** pid=246 `negative_patterns` `["kardināl"]` (`data/fix_negative_patterns_kols_pujats_2026-09-15.sql`).
- **(c) SLĒGTS 2026-09-15:** doki 6344 (Agra Melne), 31977 (drošības eksperts Mārtiņš Melnis), 38789/40173 («Melnie» — īpašības vārds) izlasīti, visas četras rindas dzēstas (`data/fix_junction_melnis_vilks_kols_2026-09-15.sql`, 0 claims uz pāriem).
...
### [OPERATOR] pid=192 Kārlis Seržants — identitāte un partija neatrisināta, birka bloķē publicēšanu

**Trigeris:** 2026-09-09 rutīnā `@quality-reviewer` bloķēja dienas pārskatu, jo konteksta kastīte un pozīciju tabula par vienu un to pašu cilvēku pauda pretrunīgu informāciju. Cēlonis nav pārskats, bet pati DB rinda.

**Kas ir zināms, ar pierādījumu:**

- `tracked_politicians` id=192: `party='Apvienotais saraksts'`, `role='Žurnālists un politiķis'`, `notes` pats brīdina: *«vārds dalīts ar bij. ZZS politiķi — pārbaudīt matcher attribution»*.
- Handle `@karlis_serzants` ir reģistrēts tieši id=192, tāpēc **autorība ir droša** — claims nāk no šī konta.
- Bet claim **#709214** (2026-09-08) ir pirmās personas izteikums, kas atsaucas uz **«zelta vīzu» ieviešanu «viņa Saeimas darba laikā»**. Tas ir deputāta biogrāfijas fakts, kas nesaskan ar lomu «Žurnālists un politiķis» un ar birku «Apvienotais saraksts».
- Frakcijas krustpārbaude (T6 lētais tests) **nedod neko**: `saeima_individual_votes` id=192 slotā ir tukšs, tāpēc balsojumu vēsture identitāti neapstiprina.

**Kāpēc tas nav kosmētika.** Partijas birka nonāk publiskā pozīciju tabulā un blokos («Koalīcija vs Opozīcija»), tāpēc nepareiza birka ir publicējama fakta kļūda, ne iekšēja neprecizitāte. 2026-09-09 pārskatā birka šai rindai tāpēc tika noņemta un partija netiek nosaukta vispār — tas ir apiešana, ne risinājums, un tā atkārtosies katru dienu, kad šis konts runā.

**Rīcība:** izšķirt, vai id=192 ir žurnālists, vai bijušais ZZS deputāts, kas pārgājis uz Apvienoto sarakstu, un vai `@karlis_serzants` pieder tam pašam cilvēkam. Ja rinda apraksta divus cilvēkus, tā jāsadala ar pāra rollback un jāpārskata visi 8 esošie claims. Līdz tam pārskatos partiju šai rindai nenosaukt.

**Identitāte IZŠĶIRTA 2026-09-12 (P-rindas #709214 pārbaude, `docs/audits/needs-review-triaza-2026-09-11.md` § Izpilde):** tā ir **viena persona**. CVK SV2026 kandidāta lapa (`dati.cvk.lv/SV2026/kandidati/87840-karlis-serzants/`): Kārlis Seržants, dz. 1959, Latvijas Zaļā partija, «Apvienotais saraksts» Nr. 7, pašlaik Saeimas deputāta palīgs; lv.wikipedia: žurnālists («Diena», «Rīgas Balss», LTV Ziņu dienests, LNT «Degpunktā») un 10.–12. Saeimas deputāts no ZZS (2010-11-02 — 2018-11-06). Tātad «viņa Saeimas darba laiks» claim #709214 tekstā ir viņa paša biogrāfija, `party='Apvienotais saraksts'` un `role='Žurnālists un politiķis'` ir pareizi, un pārskatos partiju drīkst nosaukt. **Paliek operatoram:** `tracked_politicians.notes` brīdinājums «vārds dalīts ar bij. ZZS politiķi — pārbaudīt matcher attribution» ir nepatiess un maldina katru nākamo sesiju — nomainīt uz «žurnālists UN bij. ZZS deputāts (10.–12. Saeima), tagad LZP/AS — viena persona» (ar rollback). Rindas sadalīšana NAV vajadzīga.

**Īpašnieks:** operators (partijas un `name_forms` labojumi ir operatora-izskatīšanas izmaiņas pēc standing noteikuma).
...
### [OPERATOR] 2026-09-13 rutīnas matcher atradumi — biogrāfiska atsauce kā `subject`, `negative_patterns` divi patērētāji; «Vilks» slēgts 2026-09-15

**Konteksts:** Uzulnieka diena (19 raksti par vienu personu, kura pati nerunā) izgaismoja četras klases; Kulberga «Valsts policija» veto un Vilka «Dzelzs Vilks» jau IZLABOTI (CHANGELOG 2026-09-13 (2), rollback `data/rollback_kulbergs_negative_patterns_2026-09-13.sql`, `data/rollback_vilks_negative_patterns_2026-09-13.sql`). Paliek:

- **(a) Biogrāfiska atsauce dod `subject`.** Siliņa (pid 2) ir `subject` uz 107250/107262 tikai ar «Evikas Siliņas vadītās valdības laikā»; Augulis (pid 70) `subject` uz 107253, kur viņš tikai nosaukts kā pienākumu izpildītājs. Tā pati klase kā § «`subject` lomai vajag runātāja pierādījumu» — divas jaunas instances, nevis jauns mehānisms; skaitīt pie tā ieraksta, kad tam ir lēmums.
- **(b) `tracked_politicians.negative_patterns` apkalpo divus semantiski atšķirīgus filtrus.** `src/vad/declarations.py::_row_passes_disambig` (haystack = institūcija + amats — tam paterns bija domāts) un `src/matcher.py::match_politicians` (`any(p in text …)` uz VISU dokumentu → viss doks veto). Kulberga «Valsts policija» (seedēts 2026-05-03 VAD homonīmam — inspektors Andris Kulbergs, Zemgale) 60 dienās veto **66/66** ziņu dokus, visi par premjeru, 0 par inspektoru. **Rīcība:** vai nu atsevišķa kolonna VAD haystack paterniem, vai `match_politicians` veto tikai teikuma/loga robežās; līdz tam katrs VAD-motivēts paterns ir ziņu matcher risks. **Blakus:** `keywords.vad_disambig` pid 10 hinti ir tikai «Saeima» formas — premjera deklarācijas («Valsts kanceleja»/«Ministru prezidents») nākamajā VAD ingestā tiks noraidītas; papildināt hintus, ne tukšot.
- **(c) SLĒGTS 2026-09-15:** «Vilks», «Vilka», «Vilkam», «Vilku» pievienoti `src/matcher.py::_COMMON_WORD_FORMS` (matčo tikai ar personas kontekstu); eval harness `scripts/eval_matcher_collisions.py` pēc izmaiņas: B2D2H `fp_links=1` (≤3), `gold=1938/1975` (≥1260), fidelity 0 nesakritības 932 dokos; vecās «Dzelzs Vilks» rindas 25117/70734 dzēstas (`data/fix_junction_melnis_vilks_kols_2026-09-15.sql`). Bija: 180 d 51 junction doks, ~7 par politiķi.
