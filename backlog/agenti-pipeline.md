...
### [OPERATOR] Sešu publicētu pārskatu virsraksts mainīsies nākamajā pilnajā renderā — `visual_brief_json` vairs nenosaka virsrakstu (2026-09-16)

**Konteksts.** Ieraksts «Pārskata virsraksta labojums ir ČETRU vietu labojums» SLĒGTS 2026-09-16 ar variantu (a): `src/render/blog.py` virsrakstu ņem no PAŠREIZĒJĀ `content` bloka «## Vizuālais brief», glabātais JSON ir tikai fallback, pēc tā — virsraksta regex (CHANGELOG 2026-09-16 (4); vārti `tests/test_blog_title_from_content.py`, četras virsmas). Vēsture un #505 pēda: `data/{fix,rollback}_brief505_kvc_termini_2026-08-27.sql`.

**Lēmums operatoram.** Saucējs pārmērīts 2026-09-16: **184** pārskati ar aizpildītu kolonnu, 13 bloks neparsējas (renderējas kā līdz šim), **6** diverģē un nākamajā pilnajā renderā + deploy mainīs `og:title`/`twitter:title`/H1/`alt`: #212 (05-16) «nominē Kulbergu» → «uztic Kulbergam»; #218 (05-18) un #221 (05-21) no vispārīgā «Dienas analīze» uz īsto tēzi; #252 (06-04) «70 balsojumi» → «pieņem 3 likumus»; #314 (07-04) «nav pieņemama» → «21 mrd eiro»; #508 (08-27) rīkojuma formulējums. Neviens nav faktu kļūda; tas ir satura bloka teksts, ko autors pats uzrakstīja. Ja kāds no sešiem NAV vēlams, labo `content` bloku (ar pāra rollback), ne kolonnu — kolonna renderā vairs neko nenosaka.

**Palieka.** `visual_brief_json` tagad ir render-miris, bet joprojām tiek rakstīts (`src/tools.py`) un `@graphics-designer` to lasa attēla promptam — tātad attēls un virsraksts var atšķirties pēc `content` labojuma. Izvēle: (i) `@graphics-designer` arī pāriet uz `parse_visual_brief(content)`; (ii) kolonnu izmest. Zema prioritāte.
...
**Atkārtojās 2026-09-23 (backfill 1. partija):** orķestrators Kulberga 16 dokus sadalīja 8+8 diviem aģentiem — noteikums nebija ieraksts brīfu veidnē, tikai šeit. Dublikāts neradās, jo abu aģentu jaunās pozīcijas krita tālu laikā (25.–28.05 pret 14.06; pārbaudīts ar ±5 d vaicājumu pēc darba), ne tāpēc, ka vārti strādāja. Nākamās partijas brīfā: viens politiķis = viens aģents; ja politiķim >12 doku, otrā daļa iet nākamajā kārtā.
...
**Pārmērīts 2026-09-23** (`SELECT … FROM context_notes WHERE note_type='context' AND created_at>='2026-07-29'` + regex `#\d{3,}`): **5 no 182** piezīmēm nes kailu `#N` — #384–386 (07-29), #442 (08-10), #481 (08-22, `#477` = piezīmes ID); kopš 08-22 **0**. Pārskatos (`daily_brief`, kopš 08-01, regex `#\d{5,}`): **1 no 53** — #446 (08-10), kura konteksta blokā ir labojuma piezīme «claim #14391 dzēsts kā #11253 dublikāts», un tā ir LIVE lapā `blog/2026-08-10.html`. Paterns ir apsīcis bez koda vārta; atvērts paliek (a) vai labot #446 publicēto tekstu (UPSERT + deploy — operatora lēmums, ārēja virsma) un (b) vai likt lētu vārtu (`check_output.py` vai `store_context_note` brīdinājums), kas noķertu nākamo gadījumu.
