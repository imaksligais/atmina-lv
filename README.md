...
- **Pozīcijas** — politiķu izteikumi 33 tēmās; katram ir citāts, datums un saite uz avotu
...
| Partiju programmu solījumi | 230+ (14 partijas) |
...
Šīs divas komandas palaiž CI, un tās ir vārti, kas jāiztur katram *pull request*. Kailais `python` te ir apzināts: bloks apraksta svaigu klonu ar tikko aktivētu `.venv`, un tieši tā izsauc arī `.github/workflows/ci.yml`. Uzturētāja mašīnā, kur `.venv` nav aktivēta, spēkā ir pretējais noteikums — vienmēr `.venv/Scripts/python.exe` (sk. [`wiki/operations/commands.md`](wiki/operations/commands.md)). `bash scripts/check.sh` uz svaiga klona **nestrādās** — tā trešais solis renderē vietni no `data/atmina.db`, operatora datubāzes, kas repozitorijā netiek izplatīta. Tas ir uzturētāja pēdējais solis pirms publicēšanas, nevis prasība līdzstrādniekam.

Piekļuves atslēgas glabājas operētājsistēmas atslēgu glabātavā (*keyring*; `.venv/Scripts/python.exe -m src.credentials set …`), nevis repozitorijā. Pilna uzstādīšanas instrukcija: [`wiki/operations/dev-setup.md`](wiki/operations/dev-setup.md); komandu saraksts: [`wiki/operations/commands.md`](wiki/operations/commands.md); datu kontrakti un invarianti: [`CLAUDE.md`](CLAUDE.md); shēmas lēmumu vēsture: [`wiki/CHANGELOG.md`](wiki/CHANGELOG.md).
