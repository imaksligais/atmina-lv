...
**Nav automātisks.** Nav webhook. Nav atsevišķa draftu rindas paneļa (ops dashboarda footerā ir tikai plānoto darbu mini-saraksts ar "social drafts" rindu — sk. [atmina-ops.md](atmina-ops.md)). Ir flow eksperiments — vēlāk taps separāta aģentu pārvaldības sistēma.
...
Kategorijas (no `src.render._common.CATEGORY_LV` analoga): `pozīcijas maiņa` (position+position), `vārdi vs. darbi` (position+saeima_vote), `balsojuma maiņa` (vote+vote). Paraphrases neliek pēdiņās; verbātīmas citātu rindas — ar. Latvieši — "premjere" (nevis "premjēre"). Stats grafiki izslēdz žurnālistus / influencerus / neitrālus kontus (`relationship_type`).
...
| `__main__.py` | `.venv/Scripts/python.exe -m src.social_agent ...` entry point |
...
**3. Analīžu highlights** — no `political_tensions` (agrākais `oppo_briefs.strongest_attacks` zars noņemts 2026-07-29 kopā ar politracker mantojuma tabulu). Dedup pret `posted`. Vizuāls: nanobanana ilustrācija vai quote card.
...
```
1. .venv/Scripts/python.exe -m src.social_agent brainstorm
...
3. Operators atbild Telegrāmā:
     ok 42              → .venv/Scripts/python.exe -m src.social_agent approve 42
     skip 42            → .venv/Scripts/python.exe -m src.social_agent skip 42
     42 pārraksti īsāk  → .venv/Scripts/python.exe -m src.social_agent revise 42 pārraksti īsāk
     │
     ▼
...
```bash
.venv/Scripts/python.exe -m src.credentials set telegram_bot_token         # Bot API tokens
.venv/Scripts/python.exe -m src.credentials set telegram_operator_chat_id  # Piem. 619646282
.venv/Scripts/python.exe -m src.credentials set x_atmina_cookies_path      # ceļš uz atminaLV cookies
...
.venv/Scripts/python.exe -c "from src.db import init_db; init_db()"
```

## Komandas

```bash
.venv/Scripts/python.exe -m src.social_agent brainstorm                        # atlasa + sūta top 3 draftus
.venv/Scripts/python.exe -m src.social_agent approve <id>                      # publicē uz X
.venv/Scripts/python.exe -m src.social_agent skip <id>                         # noraidīt draftu
.venv/Scripts/python.exe -m src.social_agent revise <id> <instrukcija...>      # pārģenerē ar LLM instrukciju
.venv/Scripts/python.exe -m src.social_agent resend <id>                       # atkārtoti sūta uz Telegrāmu
```

## Smoke test

Atsevišķa soli-pa-solim manuālā smoke apraksta vairs nav (agrākais `scripts/social_agent_smoke.md` repo neeksistē). Pirmo palaišanu joprojām dari pret burner X kontu, pirms pārslēgties uz `@atminaLV`: `brainstorm` → Telegram drafts → `approve` uz burner → pārbaudi `social_drafts` statusu un `tweet_id`.
...
- **Attēli:** katram reply 1 sepia "aged archival editorial illustration" (muted sepia + slate-blue, cross-hatching, aged paper, 16:9, **bez teksta/cipariem**), metaforu vadīts. Nanobanana caur `.venv/Scripts/python.exe -m src.graphics.cli thread` (kanoniskā SEPIA_STYLE).
...
- Nav draftu rindas paneļa operatora dashboardā — draftu stāvoklis redzams DB (dashboarda footeris rāda tikai plānoto darbu mini-sarakstu)
