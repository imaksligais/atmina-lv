...
5. **Resursdatoram, kas raksta DB, jāstrādā Europe/Riga laikā.** `claims`
   `review_status_at` trigeris izmanto SQLite `'localtime'`, kas seko resursdatora
   laika joslai; citā joslā zīmogs nobīdās (vārts: `tests/test_lv_time.py`).
