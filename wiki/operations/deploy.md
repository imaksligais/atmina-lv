...
# 4. Dzīvā pārbaude — 7 pārbaudes, katra ar denominatoru n=; exit 1, ja kāda krīt
.venv/Scripts/python.exe scripts/verify_host.py --base https://atmina.lv
#   (pagaidu adrese līdz 4. fāzei: https://atmina.atmina.workers.dev)
```
...
**Vēsture — rsync uz Namecheap līdz 2026-09-16 (`scripts/deploy.sh` git logā, komits pirms c4b5289d).** Sadaļa paliek, kamēr vecais hostings ir rezervē; pēc Task 9 → arhīvs. Statiskā vietne (`output/atmina/`) tika publicēta uz Namecheap shared hosting caur `rsync` over SSH. Tikai diff tiek nosūtīts, tāpēc ikdienas deploy ir sekundes. Koka izmērs aug ar katru pārskatu un attēlu variantu — 2026-08-09 mērījums: **166 MB, 1948 faili** (`du -sh output/atmina`). Skaitli pārmēri, nevis tici šai rindai; agrāk šeit stāvēja „~37 MB", kas bija novecojis 4,5×.
...
nenostrādājuša deploy. `verify_host.py` 9. pārbaude (`served_by_cloudflare`) to
