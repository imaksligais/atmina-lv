...
| Saeimas sēdes IELĀDE (jauni likumprojekti) | `bills,balsojumi,likumi,static` | ~1 min |
...
> **Saeimas sēdes ielāde paplašina rendera tvērumu — `dashboard,blog,static` NEPIETIEK (2026-09-10).** `process_vote_snapshot()` upserto arī `saeima_bills`, un `static` domēns ieliek sitemapā `likumprojekti/<slug>.html` rindas KATRAM likumprojektam — bet pašas lapas renderē `bills` domēns. Ja tajā pašā dienā ir bijusi sēdes ielāde un renderē tikai pārskata recepti, sitemaps norāda uz lapām, kuru kokā nav: 2026-09-10 tas bija **40 salauztas atsauces** (52 jauni likumprojekti), un `deploy.sh` preflight `check_output.py` deploy pareizi atteica ar `ERROR: built tree has broken references`. Tas ir vārts, kas nostrādāja, ne kļūda — bet tā ir arī vienīgā vieta, kur to pamana, tāpēc pēc sēdes ielādes renderē `bills,balsojumi,likumi,static` PIRMS pārskata recepti (vai vienkārši abus kopā). Pēc labojuma: 1001 lapa, 53 289 atsauces, sitemaps 1000/1000 atrisinās.
...
## Publicēšana uz Cloudflare (kopš 2026-09-16)

```bash
bash scripts/deploy.sh --dry-run --no-delete   # Sausais reiss: preflight vārti + failu skaits, wrangler netiek saukts
bash scripts/deploy.sh --no-delete             # Faktiskais deploy — VIENMĒR pilns output/atmina/ koks (--no-delete ir no-op, runbooki to raksta)
.venv/Scripts/python.exe scripts/verify_host.py --base https://atmina.lv   # 7 dzīvās pārbaudes ar denominatoru
```

Pilns runbook: [deploy.md](deploy.md) § Cloudflare (token, wrangler versija, domēni panelī).
...
```bash
# Brief attēls (dienas noklusējums kopš 2026-09-13: sepia bez teksta, kā pavedienam; nedēļas → "weekly" rāmis ar virsrakstu;
# `--style editorial` vai `--with-text` atgriež Economist plakātu ar virsrakstu; build_prompt + audits + budget + approval gate approved=0):
.venv/Scripts/python.exe -m src.graphics.cli brief --note-id N [--metaphor "..."] [--mood "..."] [--accent "..."] [--style sepia|editorial|weekly] [--no-text|--with-text]
...
## Partiju tests (`content/partiju-tests/`)

```bash
.venv/Scripts/python.exe scripts/partiju_tests_draft.py            # T1: kandidāt-apgalvojumu melnraksts no DB (tikai lasa)
.venv/Scripts/python.exe scripts/partiju_tests_validate.py         # T3: kodejums.json ↔ jautajumi.yaml ↔ DB; exit 1 pie kļūdas, drukā saucēju
.venv/Scripts/python.exe scripts/render_partiju_tests.py           # T5: melnraksts content/partiju-tests/partiju-tests.draft.html
.venv/Scripts/python.exe scripts/render_partiju_tests.py --publish # uz curated/atmina/analizes/ — TIKAI ar operatora atļauju; vienlaikus noņem `draft: true` kartītē
```

Ieejas punkts turpinājumam: `docs/plans/2026-09-17-partiju-tests-handoff.md` (stāvoklis, bloķētājs, divi ceļi). Plāns un vēsture: `docs/plans/2026-09-10-skirotava-atmina.md`. Kartīte `content/analizes/partiju-tests.md` ar `draft: true` neparādās indeksā (`src/render/analyses.py`), tāpēc rutīnas deploy melnrakstu neaizved.
