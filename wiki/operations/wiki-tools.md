...
### link_politician_to_document(document_id, politician_id, role="subject") → None
**Avots:** `src/db.py`
Manuālai junction pievienošanai (piem., matcher nepamanīta runātāja piesaiste jau ievāktam dokumentam — 78573 klase). Idempotents (`INSERT OR IGNORE`) — lieto ŠO, ne kailu SQL INSERT, lai atkārtots izsaukums nerada dublikātu. Repo kodā izsaucēju nav ar nolūku: tas ir interaktīvo sesiju rīks (lēmums 2026-08-04, sk. CHANGELOG).
```python
from src.db import link_politician_to_document
link_politician_to_document(document_id=78573, politician_id=243, role="subject")
```
...
### read_ingest_log(log_path="wiki/log-ingest", last_n=20) → list[str]
**Avots:** `src/ingest_log.py`
Nolasa pēdējos N ingest ierakstus (jaunākie pirmie). `log_path` tagad ir MAPE ar mēneša rotāciju (`wiki/log-ingest/<YYYY-MM>.md`), ne viens fails — `DEFAULT_LOG_PATH="wiki/log-ingest"`.
```python
