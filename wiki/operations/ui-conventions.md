...
- **Tēmas-adaptīvi attēli = divi `<img>` + CSS kaskāde** (`img.theme-img-dark`/`img.theme-img-light`, style.css pie `.analysis-feat-media`): noklusējums bez `data-theme` rāda TUMŠO (JS-disabled atkāpe), `html[data-theme="light"]` pārslēdz — momentāni, bez JS. Analīžu kartītēm to ieslēdz `image_light:` frontmatter (`_load_analyses`). Bāzes slēpšanas noteikums ir `html img.theme-img-light` (0,1,2) — ar nolūku PĀRSIT jebkuru `.konteiners img` (0,1,1) neatkarīgi no vietas failā; kaila `.klase` (0,1,0) un pat `img.klase` pirms vēlāka `.brief-hero img` abas reizes izgāzās dzīvajā pārbaudē 2026-07-29. Sedz kartītes (analizes/sintēzes/landing) un rakstu hero (`blog-post.html.j2`, sintēzēm `image_light_src`). Teksta kaveriem (Siliņas, Sprūda sintēzes) dvīņu NAV ar nolūku — LV teksts ģenerētā attēlā ir halucināciju risks.
...
- **Kurētās statistika/finanses lapas deploy izmaiņu sarakstā (wrangler `+`/`~` rindas; agrāk rsync) ir GAIDĪTAS** — `_sync_curated_chrome` katrā renderā iešuj dzīvo nav/footer, tāpēc `output/` kopija apzināti atšķiras no `curated/atmina/`. Tas nav drift.
- **Kurēta analīze `analizes/` apakšmapē = `standalone: true` paterns:** lapa dzīvo `curated/atmina/analizes/<slug>.html` (chrome sync par brīvu; `active_page` no pirmā ceļa segmenta), un indeksa kartīti dod `content/analizes/<slug>.md` ar `standalone: true` frontmatter — `render_analyses()` tad lapu NEģenerē. Slug jāsakrīt abās vietās. Pirmais lietojums + detaļas: [CHANGELOG § 2026-07-29](../CHANGELOG.md#2026-07-29--kurētās-analīzes-standalone-true-paterns--nvo-dotāciju-lapa).
...
## Personas lapa un profilu kartītes

- **"alfabēts" sort = UZVĀRDS.** Kartītei jānes `data-sort-name` no `_persona_sort_name(name, category)` (personas flip uz "Uzvārds Vārds"; Mediji/Iestādes/Citi paliek pilni) — `pnv1.js` kārto un tie-breako pēc `sortName`, nekad pēc `data-name` (tas ir "Vārds Uzvārds", meklēšanas haystackam). Jaunai kartītei/kategorijai pārbaudi, kurā pusē `_PERSON_CATEGORIES` tai jābūt.
- **Nulles stati = `<div class="pnv1-card-stat is-zero">`, ne saite un ne izlaists šūnas.** Grid `grid-auto-columns: 1fr` tur kolonnas stabilas tikai tad, ja šūna paliek; `.is-zero` (opacity .38) norāda "nav klikšķināms". Izņēmums: `bals.` stat ne-deputātiem tiek IZLAISTS pavisam (maldinošs "0" — sk. komentāru personas.html.j2).
- **Aktivitātes datums kartītēs = `last_activity.display`** no `_get_last_activity()` (gada-apzinīgs: "07-25" šogad, "2025-11-03" vecākiem gadiem). Jaunās veidnēs nekad `date[5:]` — bez gada vecs ieraksts izlasās kā nesens.
- **Profila cilņu joslas `profile-stat-value` tikai REĀLIEM skaitļiem** (pozīcijas, balsojumi, pretrunas, saites). "Pārskats" cilnei value-span NAV — `parskats_data|length` ir bloku skaits (1–3), ne saturs, un lasījās kā viltus metrika. Bez-skaitļa cilnes augstumu tur `.profile-stat { justify-content:center }`.

## Prozas tabulas (pārskati, sintēzes) — `.post-content table`

**Tās nav datu tabulas, tās ir rindkopas režģī**, un abos galos tas sāp citādi. Mērījums 2026-09-08, 7. septembra pārskats, 12 tabulas:

| Ekrāns | Platākā tabula | Konteiners | Pāri malai |
|---|---|---|---|
| 1440 px | 6 705 px | 900 px | 11 no 12 |
| 390 px | 3 980 px | 358 px | 12 no 12 |

Uz datora bija SLIKTĀK: `.table-scroll > table { min-width: max-content }` nozīmē «neviens teikums nelūzt», un platā ekrānā pārlūks kolonnai atdod visu dabisko platumu. Lasītāja sūdzība (X, 2026-09-08) nāca no telefona, bet cēlonis bija kopīgs.

**Kas tagad ir spēkā:**

- **`min-width: max-content` NEATTIECAS uz `.post-content`.** Skaitliskām tabulām (balsojumu matrica, `.tbl` kurētajās analīzēs) tas paliek pareizs — tur kolonna nedrīkst lūzt. Prozas tabulās to atceļ, un `auto` izkārtojums sašaurina platāko kolonnu pats.
- **Trim šaurajām kolonnām ir platuma grīda** (1., 2. un pēdējā; kopā ~23rem). Bez tās `auto` izkārtojums prozas kolonnai atdod visu un pārējās nomērdē — pirmajā mēģinājumā «Avots» sanāca `ls / m.l / v`, viens burts rindā.
- **`overflow-wrap: break-word`, NEKAD `anywhere`.** `anywhere` lauž vārdus pa burtiem arī tad, kad tas nav vajadzīgs; `break-word` lauž tikai vārdu, kas pats ir garāks par rindu.
- **≤768 px tabula vairs nav tabula.** Rindas kļūst par kartītēm: 1. šūna = virsraksts, `Partija` = apakšvirsraksts bez etiķetes, `Pozīcija`/`Apraksts` bez etiķetes (šūna JAU ir kartītes teksts), pārējām etiķete virs satura. `Avots` paliek ar etiķeti — operatora lēmums 2026-09-08, kartītē tas nav traucēklis.

**Etiķetes nāk no servera, ne no JS.** `src/render/_common/filters.py::_label_table_cells()` (izcelta no `blog.py` 2026-09-23, lai sedz arī sintēzes) pieliek katrai `<td>` tās kolonnas nosaukumu kā `data-label`; CSS to lasa ar `attr()`. Pārskatos to sauc `_brief_markdown_to_html`, sintēzēs `_enhance_synthesis_html` — tikai uz AUTORU markdown tabulām, pirms vidžetu injekcijas: `syn-*` vidžetu tabulas etiķetes neiegūst (tās jau ir kartītes ar savu struktūru). Stingrā CSP inline skriptus nogalina, un ārējs skripts nozīmētu salauztu lapu, ja tas neielādējas. **Fail-safe:** ja rindas šūnu skaits nesakrīt ar galvenes kolonnu skaitu, tā tabula paliek pilnīgi neaiztikta — nepareizi piešķirta etiķete ir sliktāka par neviena, un klusa šūnas pazušana vēl sliktāka.

**Palaga sadalīšana.** `_segment_prose_cell()` garu šūnu dala pēc semikoliem, kas nav pēdiņās vai iekavās, un katru gabalu ietin `<span class="cell-seg">`. Uz datora tie ir `inline` (tabula izskatās tāpat kā līdz šim), telefonā — bloki. Konservatīvi ar nolūku: dala tikai ≥170 zīmju šūnas, tikai ja sanāk ≥2 gabali pa ≥40 zīmēm, un **nekad šūnu ar bērnu tagiem** — šūna ar saiti vai treknrakstu paliek neaiztikta. Mērījums 2026-09-08: 41 no 83 garajām šūnām sadalījās, 0 sadūrās ar saitēm.

**Ko šis NEsedz.** Kurētās analīžu lapas (`curated/atmina/analizes/`) lieto savu `.tbl` ar `min-width: 560px` inline lapā — tā ir salīdzināma skaitliska tabula, kur rindu stiepšana vienu zem otras atņemtu jēgu. 348 px ekrānā tā ritinās ~1,6 ekrānus; tas ir apzināti, ne aizmirsts.

**Pārbaude pēc jebkuras izmaiņas šajās kārtulās** — nosauc SAUCĒJU, ne tikai «izskatās labi»: cik tabulu, cik pāri malai, platākā pret konteineru, un **cik šūnu pret cik etiķetēm** (kartīšu izkārtojumā klusi var pazust vesela kolonna).
...
- **`.brief-hero` 100vw full-bleed** ietver ritjoslas platumu → ~5px h-scroll. ATRISINĀTS 2026-07-29: `html { scroll-behavior: smooth; overflow-x: clip; }` (`assets/style.css`) nogriež pārkari, nesalaužot sticky pozicionēšanu. Jaunam 100vw blokam pārkare vairs neizlaužas — bet `overflow-x: clip` uz `html` nenoņem, citādi tā atgriežas.
