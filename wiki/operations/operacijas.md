...
| ------------------------------------- | ------------ | ---------------------------------------------------------------------------- |
| [Dienas rutīna](daily-routine.md)     | Katru dienu  | 11 soļi: ingest → analīze → pretrunas → brief → featured image → publikācija → deploy |
...
| `/audit-integrity` | `.claude/commands/audit-integrity.md` | Read-only DB integritātes pārbaude — **18 čeki + 9b apakšsaucējs** (matcher kolīzijas, x_handle diverģence, novecojušas review rindas — vecums no `review_status_at`, stale party, ministri bez balsojumu seguma, junction lomu inversija, `claim_vectors` novecošana, nekanoniskas brief un `position` tēmas u. c.; 18. pārbaude un 9b kopš 2026-09-07). Pilns uzskaitījums dzīvo prasmes failā un **šeit netiek dublēts**: līdz 2026-08-09 šī sūna sasala uz „10. čeks", kamēr failā jau bija 17. |
...
| [Deploy](deploy.md) | `output/atmina/` publicēšana uz Cloudflare Workers (`wrangler`, pilns koks; kopš 2026-09-16) + vecā hostinga sadaļa rezervē + politiķa deaktivācijas checklist |
| [Cloudflare drošība](cloudflare-security.md) | Security Insights triāža (15 ieraksti, 2026-09-16) — SPF/DMARC paneļa soļi, `security.txt`, Bot Fight Mode izmēģinājuma procedūra, „ko nekad" saraksts |
...
| [Devin CLI (SWE-2)](devin.md) | Ārējs apakšaģents koda uzdevumiem — `--prompt-file` (ne `-p "$(cat)"`), tiesību režīmu tabula, DB momentuzņēmums pirms `dangerous`, un kāpēc aģenta atskaites skaitļi jāpārmēra pašam |
...
> **Statuss:** ceļš pārbaudīts, bet **produkcijā nekad nav izmantots** — `SELECT COUNT(*) FROM documents WHERE platform='video'` = **0**. Ko tas ir izgājis: vienu E2E testu 2026-07-22 (2 minūšu klips ar karstu pārrunāšanos; pyannote 4.x community-1 + AiLab LV ASR, bez Docker), kurā `@video-extractor` atribūcijas stop-gate korekti neļāva neko saglabāt; testa dokuments pēc tam dzēsts. Tātad vārti ir pierādīti, laimīgais ceļš nav. Zināmā robeža, ko tests atklāja: diarizācija uz karstas pārrunāšanās jauc runātāju robežas — pirmajam īstajam ingestam izvēlies mierīgu interviju, nevis debašu klipu (sk. CLAUDE.md Data Contract #13 un BACKLOG § Video ingest).
...
Četri zināmi paterni, kur `@saeima-tracker` var **klusi** noklust (darbs neizdodas bez kļūdas signāla). Pilnais runbooks: [.claude/agents/saeima-tracker.md](../../.claude/agents/saeima-tracker.md).
...
4. **Novecojis sēžu manifests.** Audits redz tikai manifesta ģenerēšanas brīdī zināmās sēdes — pārbaudi `generated_at` pirms palaišanas ([commands.md](commands.md)).
