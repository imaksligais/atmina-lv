...
| **X cookie pool** | live probe 4 endpoints × 5 slots | 60s cache; force-refresh poga (`R`) |
...
**Darbības (HTMX action endpoints — piegādāti, `src/dashboard/server.py`):**
...
| `POST /api/slots/refresh` | `↻ Pārbaudīt` poga vai `R` | Live probe 5 slots × 4 endpoints; toast ar healthy count |
| `GET /api/deploy/confirm` | `🚀 Deploy` poga vai `D` | Atver modal ar pēdējā deploy laiku |
| `POST /api/deploy` | Modal "Apstiprināt" | `bash scripts/deploy.sh --no-delete` 300 s timeout; toast success/failure/timeout |

**Poga vienmēr deploy aditīvi.** Karogs ir padots skaidri, un kopš 2026-08-01 tas ir arī paša skripta noklusējums — dzēšošs deploy prasa `--delete` ar roku no komandrindas. Līdz tam datumam poga palaida skriptu bez karoga, tātad `--delete` režīmā: nospiešana pēc šaurā rendera būtu novākusi visu, ko tas renders neizdeva.
...
- brief vēl nav uzrakstīts un pulkstenis jau pēc 15:00 LV (pirms 15:00 → `'waiting'` stāvoklis, nav alarm — sk. CLAUDE.md § Standing Decisions, punkts *Timing*),
- search_tweet slot health zem 4/5 (pūlā ir 5 cookie sloti — 6. izņemts 2026-06-14 kā 2. slota dublikāts) → guardrail nostrādās un kritīs uz timeline stratēģiju.
...
|---|---|---|
| `X_MENTIONS_STRATEGY` | `search` (kopš 2026-06-12 flip; `timeline` = guardrail fallback) | A/B strategy panelī parādīts cipot. Lai uzspiestu `timeline`, palaid `setx X_MENTIONS_STRATEGY timeline` un atver jaunu PowerShell sesiju (user-scope mainīgais inheritējas), tad `.venv/Scripts/python.exe serve.py`. |
...
**Slot probe paneli lādē lēnāk pār 10s:** pirmajā page load pēc servera startup cache ir auksts → live probe pār twikit (~1-2s × 5 sloti = ~8s). Nākamie page loadi 60s laikā saņem cache. **Force-refresh** poga ar `R` keyboard shortcut ir pieejama (`POST /api/slots/refresh`).

**Backlog rāda `0` šodien, bet ekstrakcijas vēl nav palaistas:** tā ir gaidāmā uzvedība pirms 15:00 LV — operatora plūsma ir "ingest visu dienu, ekstraktē + brief pēcpusdienā" (CLAUDE.md § Standing Decisions, punkts *Timing*).
...
- **M2 (piegādāts):** HTMX action buttons — image approve/reject, slot probe force-refresh, deploy ar confirm modal, keyboard shortcuts. Plāna Phase 2.
- **M3 (vēlāk):** tooltips, empty-state polish, settings page, first-visit tour, optional SSE. Plāna Phase 3.

## Saistītie

- [implementation plan](../../docs/superpowers/plans/archive/2026-05-16-operator-dashboard.md)
