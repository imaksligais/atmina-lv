...
- [ ] `confidence` pēc [rubrics.md](rubrics.md): ≤0.6 → `reasoning` PREFIKSĀ `NEEDS_REVIEW: ` (skaidrs atstāsts bez citāta = 0.65 bez marķiera, kopš 2026-09-23 — `claim-extractor.md` Critical Rules § 8); <0.5 → tas pats + iemesls tajā pašā rindā. Karogu joprojām RAKSTA kā tekstu `reasoning` laukā — `needs_review` parametra nav, un liekā atslēga tiek klusi atmesta (pārbaudīts 2026-08-02). **Bet LASA to no `claims.review_status` kolonnas** (kopš 2026-08-03, atvasināta ar trigeriem); nekad ar `LIKE` pār prozu, jo marķiera forma un novietojums dreifē
...
## Sintēze (`wiki/synthesis/*.md`) un analīzes (`content/analizes/*.md`)
...
- [ ] Katrai nosauktai personai amata etiķete ir PAŠREIZĒJAIS stāvoklis ar vienu svaigu avotu (iestādes lapa vai ziņa ≤3 mēn.), bijušajiem «b.» ar gadiem — sweep vai deklarācijas rinda nav statuss (2026-09-20: 12 no 18 VAD lapas etiķetēm bija novecojušas, CHANGELOG 2026-09-20 (2))
- [ ] VAD/UR datiem — homonīma pārbaude pa personai: visas citētās deklarācijas nes vienu ģimenes parakstu un amata līniju; «tikai X deklarācijās, ne Y» = divi cilvēki, kamēr nav pierādīts pretējais (Baltiņš, 2026-09-20)
- [ ] Katrs lapas skaitlis, ko var pārrēķināt no DB/probe datiem, pirms publicēšanas PĀRRĒĶINĀTS un logs nosaukts («~232 000 €» nebija atkārtojams ne no viena gadu loga — Jaunzeme, 2026-09-20)
- [ ] Lapai, kuras skaitļi nāk no DB, ir ģenerators + audits pret profiliem, un pēc katras datu kopas maiņas (ielāde, purge, reparse) skaitļi pārrēķināti — `vad-2026` četrus mēnešus rādīja tādvārža rindu bez viena sarkana vārta (2026-09-20; paraugs `scripts/vad_analysis_numbers.py` + `audit_vad_profile_match.py`)
- [ ] `content/analizes/*.md` neiet cauri LV kalku lintam (`src/lv_style.py` sedz tikai pārskatus) — pirms rendera grep pēc IT-žargona un aizdomīgiem darinājumiem (`dedup|parsē|fetč|neviendār`); «dedupēts», «neviendārtā» nostāvēja live dienu (2026-09-20)
