...
5. Palaiž site renderēšanu — šaurais renders (~15–30 s), nevis pilnais:
   ```bash
   PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m src.render --only=balsojumi
   # vai: PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe scripts/render_balsojumi_only.py
   ```
   Pilnais `generate_public_site()` (`from src.render import ...`, ~3 min) — tikai release/baseline gadījumiem.
6. Pārbauda:
...
```bash
.venv/Scripts/python.exe scripts/backfill_saeima_bills.py        # restore bills + stages from votes
.venv/Scripts/python.exe scripts/backfill_base_law_slug.py       # restore base_law_slug matches
```
