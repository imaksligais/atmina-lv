...
4. Vērtēšana: **`.venv/Scripts/python.exe scripts/eval_claim_extractor_score.py docs/eval/_run-<m>-<datums>.txt`**
   (kopš 2026-09-22 — agrāk šis solis bija ar roku). Skripts pats izvelk JSON no raw
   (arī ar narratīva apvalku vai `session_id` astes rindu), pārbauda katru citātu kā
   nepārtrauktu apakšvirkni, atdala pieturzīmju klasi (◐), un piemēro abus 2026-09-22
   prompta vārtus: `quote=null` → conf ≤ 0.6; nogriezts avots → `NEEDS_REVIEW:`.
   **Ar roku paliek tas, ko skripts nevērtē: tēmas pareizība un claim skaits** (skripts
   saka «lēmums OK», kaut tēma novirzījusies — sk. v3 dokumenta § «Ko šis nepierāda»).
   Rubrika: `claim-extractor-golden-cases-2026-09-16.md`; ◐ = 0.5.
