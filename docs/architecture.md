...
> **VĒSTURISKS MOMENTUZŅĒMUMS (2026-04-14) — netiek uzturēts.** Šis fails ir auto-ģenerēts vienreiz un kopš tā laika nav atjaunots; moduļu saraksts, skaitļi un ceļi atspoguļo aprīļa vidus stāvokli (piem., vēl bez `src/saeima/` pakotnes split). Aktuālais arhitektūras apraksts: [`ARCHITECTURE.md`](../ARCHITECTURE.md) repo saknē. Fails paturēts kā dokumentēta arhitektūras vēsture.
...
| `src/preflight.py` | Pre-operation safety checks (DB, sources, model, credentials) | `preflight_check`, `ensure_analysis_env` (cheap `find_spec` gate for hot paths), `ensure_embeddings_live` (one real embed; startup gate for bulk-write scripts) |
