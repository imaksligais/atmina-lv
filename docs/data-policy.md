...
atmina.lv skaita apmeklējumus ar **Umami** (apstrādātājs: Umami Cloud; konta dati glabājas Eiropas Savienībā) — lai zinātu, cik cilvēku lasa un kuras lapas ir noderīgas. **Sīkdatņu nav.** IP adrese netiek saglabāta: no tās servera pusē tiek atvasināta katru dienu mainīga jaucējvērtība, pēc kuras apmeklētāju nevar atpazīt nākamajā dienā. Netiek veidoti lasītāju profili un netiek izsekota pārlūkošana citās vietnēs. Skaitīšanu var izslēgt ar jebkuru reklāmu bloķētāju vai pārlūka izsekošanas aizsardzību; vietne strādā tāpat.

Umami ir **vienīgais** trešās puses hosts, ko publiskā lapa ielādē (`cloud.umami.is` skriptam, `gateway.umami.is` bīkonam — abi CSP allowlistā `assets/htaccess.template`). Fonti, JS bibliotēkas un attēli ir pašmitināti tieši tāpēc, lai neviens cits neredzētu lasītāja IP adresi.

atmina neizmanto:
- reklāmas tīklus;
- lietotāju kontu sistēmas;
- starpvietņu identifikatorus vai lasītāju profilēšanu;
- sīkdatnes (izņemot serveru tehniskās sīkdatnes, ja tādas būtu).
