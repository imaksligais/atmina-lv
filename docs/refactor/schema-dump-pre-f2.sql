...
CREATE INDEX idx_dp_extracted ON document_politicians(politician_id, extracted_at);
CREATE INDEX idx_dp_politician ON document_politicians(politician_id, role);
CREATE INDEX idx_dp_suspect ON document_politicians(suspect_at);
CREATE INDEX idx_external_profiles_opp ON external_profiles(opponent_id);
CREATE INDEX idx_image_audit_generated
    ON image_audit(generated_at);
CREATE UNIQUE INDEX idx_image_audit_source
...
CREATE TABLE document_politicians (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, extracted_at TIMESTAMP, suspect_at TIMESTAMP,
...
CREATE TABLE tension_judged (
            document_id INTEGER PRIMARY KEY REFERENCES documents(id),
            judged_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
CREATE TABLE tension_proposals (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TRIGGER claims_review_status_ai AFTER INSERT ON claims
        BEGIN UPDATE claims SET review_status_at = CASE WHEN review_status IS NOT (
        CASE
            ...
            WHEN reasoning GLOB '*Izvērtēts*' OR reasoning GLOB '*REVIEWED*'
                ...
                THEN 'reviewed'
            ELSE NULL
        END) THEN datetime('now', 'localtime') ELSE review_status_at END, review_status = 
        CASE
            WHEN reasoning GLOB '*NEEDS_REVIEW*' THEN 'needs_review'
            WHEN reasoning GLOB '*Izvērtēts*' OR reasoning GLOB '*REVIEWED*'
                OR reasoning GLOB '*IZSKATĪTS*'
