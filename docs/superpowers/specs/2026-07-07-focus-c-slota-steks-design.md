...
**Papildinājums 2026-07-24:** `.focus-grid` `align-items: start` → `stretch` + `.focus-slot` flex-kolonna ar `margin-top: auto` uz citāta kickera — B/C kolonnu apakšas tagad vienmēr līdzinās, dienas citāts noenkurots C apakšā ar `border-top` atdalītāju (start-izkārtojumā C kaste beidzās dabiskajā augstumā un citāts "karājās"); citāta meta rindā pievienota tēmas saite (`topic_slug` no `_quote_of_day`, tas pats `_slugify`, ko lieto temas lapas). Playwright 1440/375 abas tēmas atkārtoti verificēts.
