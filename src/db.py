...
# The production DB. `DB_PATH` is the path code actually opens and the ONE
# place that decides it: modules must not keep their own copy (a private
# `_DB_PATH` escapes a test's monkeypatch and silently reads the live DB —
# 2026-09-24, src/confidence_drift.py). tests/conftest.py points DB_PATH at a
# nonexistent path for the whole session, so a test that forgets its own DB
# fails locally exactly as it would in DB-less public CI.
PRODUCTION_DB_PATH = "data/atmina.db"
DB_PATH = PRODUCTION_DB_PATH

# Latvia: EEST (UTC+3) from the last Sunday of March, EET (UTC+2) from the last
# Sunday of October. Until 2026-09-24 this was a fixed `timedelta(hours=3)`,
# which would have shifted every LV timestamp, the 05:00 routine-day cut and
# review_status_at by one hour from 2026-10-25. Needs the `tzdata` package on
# Windows (installed; tests/test_lv_time.py fails loudly without it).
#
# THE single definition of Latvia time. Until 2026-08-15 the offset was re-typed
# in five other modules; import `LV_TZ` / `utc_to_lv()` / `now_lv()` instead of
# writing a number again.
LV_TZ = ZoneInfo("Europe/Riga")


def utc_to_lv(dt: datetime) -> datetime:
    """UTC datetime (naive = UTC, or aware) → naive Latvia local time."""
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)
    return dt.astimezone(LV_TZ).replace(tzinfo=None)
...
# izsaukt Python, tāpēc pārrēķins ir SQL pusē. `datetime()` formāts ir tieši tas
# pats "YYYY-MM-DD HH:MM:SS", ko raksta now_lv(), tātad kolonnas ir salīdzināmas
# bez konversijas. `localtime` = mašīnas laika josla, kas ir Rīga — tātad
# vasaras/ziemas laiks ir pareizs (līdz 2026-09-24 te bija fiksēts '+3 hours').
# Tāpēc resursdatoram, kas raksta DB, JĀSTRĀDĀ Europe/Riga laikā (CI: `TZ:
# Europe/Riga` .github/workflows/ci.yml) — to pin tests/test_lv_time.py::
# test_sqlite_localtime_matches_riga. Python pusē (routine.py u.c.) lieto
# utc_to_lv(), kas no mašīnas joslas nav atkarīgs.
_REVIEW_STATUS_AT_EXPR = "datetime('now', 'localtime')"
...
def now_lv() -> str:
    ...
    return utc_to_lv(datetime.now(timezone.utc)).strftime("%Y-%m-%d %H:%M:%S")


def now_lv_dt() -> datetime:
    """Return current datetime as naive datetime in Latvia time (EEST/EET)."""
    return utc_to_lv(datetime.now(timezone.utc))
...
def insert_document(
    ...
    preserve_scraped_at: bool = False,
    ...
    * **Same URL, different content, ``platform='web'``** → the existing row is
      ...
      returned. Junction rows the NEW content no longer justifies are flagged
      ``suspect_at`` for operator review — never deleted (see
      ``_reconcile_junction_suspects``; stored texts are routinely truncated
      extractions, so "name absent from stored text" cannot prove the link
      is wrong — docs 42838/44422, audit 2026-09-22).
    ...
    * ``reviewed_at`` is reset to ``NULL``. **This is deliberate and its point is
    ...
    ``preserve_scraped_at`` (2026-09-23, truncated-doc backfill): the UPDATE
    branch normally restamps ``scraped_at`` to now, which is right for a
    routine re-scrape — but a backfill re-fetch must NOT move a historic
    document into today's day bucket (every ``scraped_at``-scoped reader
    would misattribute it to the campaign day). True keeps the stored value;
    ...
    if existing:
        ...
        if politician_links and source_url and existing["source_url"] == source_url:
            for pid, role in politician_links:
                ...
                clear_suspect_link_flag(db, existing["id"], pid)
            db.commit()
        db.close()
        return None
    ...
    if source_url and platform == "web":
        existing_url = db.execute(
            "SELECT id, word_count, scraped_at FROM documents WHERE source_url = ? AND platform = 'web'",
            (source_url,),
        ).fetchone()
        if existing_url:
            ...
            db.execute(
                """UPDATE documents
                   SET content=?, content_hash=?, simhash=?, word_count=?,
                       ...
                       reviewed_at=NULL
                   WHERE id=?""",
                (content, content_hash, sim, word_count,
                 existing_url["scraped_at"] if preserve_scraped_at else now_lv(),
            ...
            # Junction rows the NEW content no longer justifies are FLAGGED
            # (suspect_at), never deleted — see _reconcile_junction_suspects
            # for why deletion here is unprovably unsafe. Runs after the
            # merge so caller-asserted links are seen as justification.
            _reconcile_junction_suspects(
                db, existing_url["id"], content, source_url,
                politician_links, existing_url["word_count"],
                db_path=db_path,
            )
            db.commit()
            db.close()
...
# Transliteration map for the URL-justification check — the same mapping as
# scripts/audit_stale_politician_links.py's `url_celš` class: a surname that
# survives in the article URL slug (NRA viedokli author columns etc.)
# justifies the junction row without any name occurrence in the body text.
_URL_SLUG_TR = str.maketrans("šķļņāēīūčžģ", "sklnaeiuczg")

# relationship_type values whose junction rows are justified by the SOURCE
# attribution rather than by a name in the text — mirrors the audit's
# `institucija` class.
_SOURCE_JUSTIFIED_RELS = ("organization", "journalist", "neutral")


def _dp_has_suspect_col(db: sqlite3.Connection) -> bool:
    """True when document_politicians carries the suspect_at column.

    Defensive — fork/dev DBs built from schema.sql alone (without
    apply_migrations) lack the column; callers must no-op there rather
    than break a write path on an optional flag.
    """
    cols = {r[1] for r in db.execute("PRAGMA table_info(document_politicians)")}
    return "suspect_at" in cols


def clear_suspect_link_flag(
    db: sqlite3.Connection, document_id: int, politician_id: int
) -> None:
    """Clear ``suspect_at`` when a junction row is explicitly re-asserted.

    Every junction write asserts that this (doc, pid) link is justified —
    the caller's fresh links, a manual ``link_politician_to_document`` call,
    or a matcher rescan that just re-derived it from the current text. Any
    of those outweighs the flag's evidence ("the last stored text did not
    contain the name"), so the flag must clear or it becomes permanent
    noise accusing rows that are already fine. Clears ALL roles of the pid
    on the doc — the flag tests name presence, so a re-assertion justifies
    the person, not one role row. No-op on DBs without the column.
    """
    if not _dp_has_suspect_col(db):
        return
    db.execute(
        """UPDATE document_politicians SET suspect_at = NULL
           WHERE document_id = ? AND politician_id = ?
             AND suspect_at IS NOT NULL""",
        (document_id, politician_id),
    )


def _reconcile_junction_suspects(
    db: sqlite3.Connection,
    document_id: int,
    content: str,
    source_url: Optional[str],
    politician_links: Optional[list[tuple[int, str]]],
    old_word_count: Optional[int],
    db_path: Optional[str] = None,
) -> None:
    """Flag junction rows the NEW content does not justify; clear the rest.

    Runs inside ``insert_document``'s same-URL UPDATE branch, after the new
    content and the caller's junction merges are staged but before commit —
    flags, clears, the content update and the merges commit atomically.

    THE FLAG IS NEVER A DELETE, by design (2026-09-22). Deletion would need
    two proofs this layer cannot produce:

    * **That the new text is complete.** ``len(new) >= len(old)`` only rules
      out a strictly shorter extraction — the 2026-09-22 audit shows stored
      texts are routinely truncated extractions (doc 42838: 313 words while
      the live NRA article names 16 ministers; doc 44422: 100 words while
      the live jauns.lv article contains every "stale" name — verified by
      live fetch). A re-fetch can be longer AND still truncated, ending on
      a clean sentence, so no text-local signal proves completeness.
    * **That the row was text-justified.** document_politicians records no
      provenance: rows from ``source_opponent_id``, URL-slug author match,
      later matcher scans and manual ``link_politician_to_document`` calls
      are indistinguishable.

    So the row is marked ``suspect_at`` for operator review (verify against
    the live article — the only authority), and the scan writes one `logs`
    row with the examined denominator per CLAUDE.md "a gate that cannot fail
    is not evidence".

    Justified (flag cleared / never set) means ANY of:
      * the pid is in the caller's fresh ``politician_links`` (covers
        source_opponent_id and URL-slug links, which fetch_source folds in);
      * some name form of the pid occurs in the new content at word
        boundaries — the audit's `substring_only` test, deliberately
        stricter than match_politicians so veto-class rows are kept;
      * the pid's relationship_type is organization/journalist/neutral —
        the link came from the source, not the text (`institucija` class);
      * the pid's surname (>4 chars) survives in the URL slug (`url_celš`).
    """
    if not _dp_has_suspect_col(db):
        return
    rows = db.execute(
    ...
    # Lazy import: src.matcher imports src.db at module top, so a top-level
    # import here would be a cycle. Same pattern as init_db→db_migrations.
    # db_path is forwarded so the forms come from the SAME database the
    # document lives in — the default-None path uses the cached loader and
    # resolves DB_PATH (production), an explicit path stays hermetic.
    from src.matcher import _load_politician_forms, _occurrences

    # Same form list the audit uses for its "teksts nepamato" test: the
    # matcher's derived forms plus negative_patterns (4th tuple element).
    forms_by_pid: dict[int, list[str]] = {}
    for pid, forms, _first_name, neg_patterns in _load_politician_forms(db_path):
        forms_by_pid[pid] = list(forms) + list(neg_patterns or [])
    meta = {
        r["id"]: (r["name"], r["relationship_type"])
        for r in db.execute(
            "SELECT id, name, relationship_type FROM tracked_politicians"
        )
    }
    new_pids = {pid for pid, _ in (politician_links or [])}
    url_folded = (source_url or "").lower().translate(_URL_SLUG_TR)

    flagged: list[int] = []
    cleared: list[int] = []
    for pid, role in rows:
        justified = pid in new_pids or any(
            _occurrences(content, form) for form in forms_by_pid.get(pid, [])
        )
        if not justified:
            name, rel = meta.get(pid, ("", ""))
            if rel in _SOURCE_JUSTIFIED_RELS:
                justified = True
            else:
                surname = name.split()[-1] if name else ""
                justified = (
                    len(surname) > 4
                    and surname.lower().translate(_URL_SLUG_TR) in url_folded
                )
        if justified:
            db.execute(
                """UPDATE document_politicians SET suspect_at = NULL
                   WHERE document_id = ? AND politician_id = ? AND role = ?
                     AND suspect_at IS NOT NULL""",
                (document_id, pid, role),
            )
            if db.execute("SELECT changes()").fetchone()[0]:
                cleared.append(pid)
        else:
            # COALESCE: the first flag time wins — re-overwrites with the
            # name still absent must not churn the stamp.
            db.execute(
                """UPDATE document_politicians
                   SET suspect_at = COALESCE(suspect_at, ?)
                   WHERE document_id = ? AND politician_id = ? AND role = ?""",
                (now_lv(), document_id, pid, role),
            )
            flagged.append(pid)

    if flagged:
        db.execute(
            """INSERT INTO logs (timestamp, action, source_id, opponent_id,
               status, details) VALUES (?, ?, ?, ?, ?, ?)""",
            (
                now_lv(), "suspect_link_flag", None, None, "success",
                json.dumps({
                    "document_id": document_id,
                    "junction_rows_examined": len(rows),
                    "flagged_pids": sorted(set(flagged)),
                    "cleared_pids": sorted(set(cleared)),
...
def link_politician_to_document(document_id: int, politician_id: int, role: str = "subject") -> None:
    ...
    clear_suspect_link_flag(db, document_id, politician_id)
    db.commit()
    db.close()
