...
def apply_migrations(db: sqlite3.Connection) -> None:
    ...
    # 2026-09-22 — suspect_at: atzīme "pašreizējais glabātais teksts šo
    # piesaisti vairs nepamato". Uzstāda insert_document() same-URL UPDATE
    # zars (_reconcile_junction_suspects), kad junction rindai nav nevienas
    # attaisnojošas vārda formas JAUNAJĀ content un nav cita pamata
    # (avota opponent_id, URL-slug, institūcija). KAROGS, ne dzēšana —
    # nogrieztas ekstrakcijas klase (audits 2026-09-22: doc 44422 — 100
    # vārdu teksts, bet dzīvajā rakstā visi 7 "novecojušie" vārdi ir
    # klāt; doc 42838 — 313 vārdi, dzīvajā 16 ministri) pierāda, ka
    # "vārda nav glabātajā tekstā" nav drošs dzēšanas pierādījums.
    # NULL = nav aizdomas. Notīrīšana: jebkurš skaidrs piesaistes
    # rakstītājs (insert_document merge, link_politician_to_document,
    # matcher rescan) un nākamā pārrakstīšana, kurā vārds atgriežas.
    # Laika zona: LV (now_lv()) — tāpat kā documents.scraped_at.
    if "suspect_at" not in _dp_cols:
        db.execute(
            "ALTER TABLE document_politicians ADD COLUMN suspect_at TIMESTAMP"
        )
    db.execute(
        "CREATE INDEX IF NOT EXISTS idx_dp_suspect "
        "ON document_politicians(suspect_at)"
    )
    ...
    # 2026-09: TypeSafe Saites proposals. Candidates for political_tensions
    # written by scripts/saites_proposals.py; a human accepts them via
    # scripts/saites_accept.py (which calls store_tension). Never rendered.
    # tension_judged remembers documents already sent to the model so the
    # overlapping daily window does not pay for them twice.
    # Laika zona: created_at / decided_at / judged_at = UTC (CURRENT_TIMESTAMP),
    # tāpat kā political_tensions — lasot lieto datetime(col, 'localtime');
    # decided_at raksta SQL CURRENT_TIMESTAMP, ne now_lv().
    db.execute("""
        CREATE TABLE IF NOT EXISTS tension_proposals (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            document_id INTEGER NOT NULL REFERENCES documents(id),
            source_pid INTEGER NOT NULL REFERENCES tracked_politicians(id),
            target_pid INTEGER NOT NULL REFERENCES tracked_politicians(id),
            relation TEXT NOT NULL CHECK (relation IN ('atbalsts', 'uzbrukums', 'spriedze')),
            p_link REAL NOT NULL,
            p_relation REAL NOT NULL,
            p_speaks REAL NOT NULL,
            snippet TEXT,
            status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'accepted', 'rejected')),
            tension_id INTEGER REFERENCES political_tensions(id),
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            decided_at TIMESTAMP,
            UNIQUE (document_id, source_pid, target_pid)
        )
    """)
    db.execute("""
        CREATE TABLE IF NOT EXISTS tension_judged (
