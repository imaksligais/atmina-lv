...
from src.db import get_db, lv_cutoff, clear_suspect_link_flag
from src.roles import enforce_subject_guards
from src.matcher_veto import VETO_THRESHOLD, judge_candidate, record

# TypeSafe name-collision veto (2026-09), applied inside match_politicians()
# to candidates matched by a bare surname only (no first name, no @handle,
# not an institutional entity). off = current behaviour; shadow = judge and
# record what WOULD be vetoed, keep everything; enforce = drop vetoed
# candidates. Flip the default only after a shadow week with zero gold
# losses (scripts/typesafe_veto_report.py). tests/conftest.py pins "off".
TYPESAFE_VETO_MODE = os.environ.get("ATMINA_TYPESAFE_VETO", "off")

# The project's own X account(s), lowercase. A tweet AUTHORED by one of these
# is atmina's output, not a politician's speech act: it must never become a
# platform='twitter' document with politician junctions, or the daily thread
# (which tags the people it summarises) re-enters the corpus and the
# extraction queue as a source about them — a self-citation loop. Eight such
# rows accumulated 2026-05-14…09-18 via other politicians' feeds (reply /
# quote / tagged-thread context); doc 110938 reached the junction-recovery
# candidates on 2026-09-18 (backlog/dati-db.md § 2026-09-18 (f)). Both write
# paths — `_store_tweets` and `link_politicians_to_documents` — consult this
# set, so a lowered guard in one is caught by the other's test.
PROJECT_X_HANDLES: frozenset[str] = frozenset({"atminalv"})


def is_project_account_url(source_url: str | None) -> bool:
    """True when the tweet URL's author is the project's own X account."""
    handle = extract_twitter_author_handle(source_url)
    return handle is not None and handle in PROJECT_X_HANDLES
...
_COMMON_WORD_FORMS: set[str] = {
    ...
    # vilks = wolf; also "Vilku drupas", other Vilks surnames (P. Vilks, VK;
    # A. Vilks, ex-minister), "Dzelzs Vilks". 180 d before 2026-09-13: 51
    # junction docs for Andrejs Vilks (pid 205), ~7 about him. Operator
    # decision 2026-09-15; the slot is `inactive`, so today's cost is low.
    "Vilks", "Vilka", "Vilkam", "Vilku",
}
...
def _latvian_surname_inflections(surname: str) -> list[str]:
    ...
    - -e surnames decline by GENDER (fem. 5th decl. vs masc.): nom,
      gen -es (same for both), dat -ei fem. (Mūrniece) OR -em masc.
      (Šnore→Šnorem), acc -i (same for both)

    Gender is not passed in — the function only sees the surname — so the
    -e branch emits BOTH datives for every -e surname. This deliberate
    over-generation is the same principle _first_token_decls documents and
    the same asymmetry the branch already had in reverse: every male -e
    surname used to get the female dative -ei (Šnore→"Šnorei"), a form
    that can never refer to him. Why it is safe here: a phantom "-em" on
    a female -e surname ("Mūrniecem") is not a Latvian word — word-final
    -em occurs essentially ONLY as the masculine -e-surname dative — so
    it can only match text referring to a same-surname MAN. That case is
    already governed by existing machinery: the B2 foreign-first-name
    veto fires on his first name, and the shared-surname disambiguation
    covers tracked m/f pairs — exactly the same protection that already
    handles the genitive "-es", which is identical for both genders and
    has always collided this way. Measured on the corpus before landing:
    ~20 phantom female "-em" forms produced zero false-link candidates;
    the real gain was doc 71307 ("E. Šnorem" → Edvīns Šnore, pid 7).
    ...
    # -e surnames: fem. 5th declension (Mūrniece→Mūrniecei) and masc.
    # (Šnore→Šnorem, Zīle→Zīlem) share gen -es and acc -i but split on the
    # dative — emit both (see docstring: deliberate over-generation).
    elif surname.endswith("e"):
        stem = surname[:-1]
        forms.extend([stem + "es", stem + "ei", stem + "em", stem + "i"])
...
def _load_politician_forms(db_path: str | None = None) -> list[tuple[int, list[str], str, list[str]]]:
    ...
    ``db_path`` (2026-09-22): an explicit path bypasses the module caches
    ENTIRELY — forms are computed from that DB and no global state is read or
    written. Used by ``src.db._reconcile_junction_suspects`` so junction
    flagging reads forms from the SAME database the document lives in;
    without it the loader would resolve ``DB_PATH`` and a test's fixture
    insert would silently consult (and cache) the live DB's forms.
    """
    global _politician_forms_cache, _politician_aux_cache, _shared_surname_set
    if db_path is None and _politician_forms_cache is not None:
        return _politician_forms_cache
    # get_db() without args on the default path — tests monkeypatch get_db
    # with zero-argument stubs, and get_db(None) would break that contract.
    db = get_db() if db_path is None else get_db(db_path)
    ...
    shared_set = {s for s, c in surnames.items() if c > 1}
    ...
    for shared in list(shared_set):
        for inflection in _latvian_surname_inflections(shared):
            shared_set.add(inflection)
    result = []
    aux: dict[int, dict] = {}
    for r in rows:
    ...
    if db_path is None:
        _shared_surname_set = shared_set
...
def match_politicians(
    ...
    risky: dict[int, bool] = {}  # pid -> matched by bare surname only (veto class)
    ...
    for pid, forms, pol_first_name, neg_patterns in forms_list:
        ...
        # Institutional voices are exempt like in the B2 veto above: the
        # TypeSafe question is person-shaped (A0: 13/13 organization gold
        # links answered "not a person").
        risky[pid] = not handle_conf and not first_in_any and not pol_aux.get("institutional")
    ...
    if all_candidates and TYPESAFE_VETO_MODE in ("shadow", "enforce"):
        kept: list[tuple[int, int]] = []
        for pid, count in all_candidates:
            if not risky.get(pid):
                kept.append((pid, count))
                continue
            p = judge_candidate(text, pid)
            vetoed = p is not None and p < VETO_THRESHOLD
            record({"mode": TYPESAFE_VETO_MODE, "pid": pid, "p_same": p, "vetoed": vetoed,
                    "snippet": text[:160]})
            if vetoed and TYPESAFE_VETO_MODE == "enforce":
                continue
            kept.append((pid, count))
        # Dropping the first candidate promotes the next one to 'subject' —
        # the role is derived from ranking, not chosen.
        all_candidates = kept
...
def link_politicians_to_documents(
    ...
    adds what is new and disturbs nothing that is already there — a pid that
    already holds a junction row keeps its stored role; the scan never adds a
    second, conflicting role row beside it (one role per (doc, pid)).
    """
    db = get_db()
    ...
    # (A `relay_handles` set was built here until 2026-09-07 and never read —
    # dead since the 2026-04-25 commentator demotion removed the relay
    # exemption below. The relay question now lives in src/roles.py, keyed on
    # the POLITICIAN, not on the author handle.)
    ...
    for r in rows:
        ...
        if matches:
            ...
            # The project's own tweets are never a source about anyone
            # (PROJECT_X_HANDLES above); `_store_tweets` already refuses to
            # store them, this covers rows that pre-date that guard and any
            # other ingest path that lands an @AtminaLV URL here.
            if platform == "twitter" and is_project_account_url(source_url):
                continue
            ...
            # A junction row already on the document wins over a fresh
            # text-derived role: the PK is (document_id, politician_id, role),
            # so INSERT OR IGNORE alone would NOT stop a second, conflicting
            # role for the same pid — a caller-asserted 'subject' (e.g.
            # ingest_url --politician-id via insert_document's
            # politician_links) would end up with a matcher-derived
            # 'mentioned' beside it (doc-111520 class), and a manual role
            # correction would be re-inverted by the next rescan. Same
            # `already` rule as social._link_first_party_mentions. The default
            # branch selects only docs with NO junctions, so the lookup is
            # needed only when rescanning (rescan_all / doc_ids).
            already: set[int] = set()
            if rescan_all or doc_ids is not None:
                already = {
                    row["politician_id"]
                    for row in db.execute(
                        "SELECT politician_id FROM document_politicians "
                        "WHERE document_id = ?",
                        (r["id"],),
                    ).fetchall()
                }
            # Speaker guards (2026-09-07, operatora verdikti 36 + 38): a relay
            # media slot and the retweeter of an office voice never keep
            # 'subject'. This writer INSERTs directly rather than through
            # insert_document, so it applies the same rule itself — see
            # src/roles.py for both classes and their measured cost.
            for pid, role in enforce_subject_guards(db, resolved, r["content"]):
                if pid in already:
                    # Row stays as stored — but a fresh text match still
                    # justifies it: clear suspect_at same as a write does.
                    clear_suspect_link_flag(db, r["id"], pid)
                    continue
                db.execute(
                ...
                # A successful re-derivation from the CURRENT stored text is
                # fresh justification — clear any suspect_at the same-URL
                # overwrite branch set earlier (src/db.py::_reconcile_junction_suspects).
                clear_suspect_link_flag(db, r["id"], pid)
