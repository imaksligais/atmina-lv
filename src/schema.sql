...
-- Laika zona: created_at = UTC (shēmas noklusējums; neviens rakstītājs to nepadod).
...
-- Laika zona: scraped_at = LV (raksta now_lv() — src/db.py::store_document un
-- src/video_ingest/db.py). DATE-filtram NEpievieno 'localtime'.
...
-- Laika zona: created_at = UTC (shēmas noklusējums; junction INSERT to nepadod).
...
-- Laika zona: created_at = UTC (shēmas noklusējums).
...
-- Laika zona: created_at = UTC (shēmas noklusējums — src/tools.py INSERT kolonnu
-- neuzskaita). DATE-filtram vajag DATE(created_at, 'localtime').
...
-- Laika zona: created_at = LV (raksta now_lv() — src/db.py::store_claim).
-- stated_at ir notikuma datums no avota, ne ieraksta laiks — cita semantika.
CREATE TABLE IF NOT EXISTS claims (
    ...
    review_status TEXT,
    -- review_status_at = WHEN review_status last changed, in LV time (the same
    -- "YYYY-MM-DD HH:MM:SS" shape now_lv() writes; the triggers use
    -- datetime('now','localtime') because a trigger cannot call Python).
    -- DERIVED by the same two triggers as review_status — never hand-written.
    -- NULL for every row written before 2026-09-07: nobody recorded when those
    -- markers were applied, so readers use COALESCE(review_status_at,
    -- created_at) (src.db.REVIEW_AGE_EXPR). It exists because the 14-day
    -- review gate measured claim age and therefore could not tell a row
    -- rotting in the queue from one retro-marked yesterday. Added to the live
    -- DB via the ALTER migration in src/db_migrations.py (idempotent,
    -- PRAGMA-guarded); declared here so fresh DBs match prod.
    review_status_at TIMESTAMP,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Laika zona: detected_at = LV (raksta now_lv() — src/db.py::store_contradiction).
...
-- Laika zona: created_at = LV (raksta now_lv() — src/social_agent/storage.py).
...
-- Laika zona: created_at = LV (raksta now_lv() — src/tools.py, konteksta piezīmju
-- INSERT). note_type='context' rindām dienas robeža nav pusnakts, bet 05:00 LV —
-- sk. src.briefs.routine_day_window() (CLAUDE.md § ROUTINE day).
...
-- image_audit: VIENS saucējs attēlu ģenerēšanas mēneša budžetam (5,00 USD),
-- rakstīts `src/graphics/nanobanana.py::generate_image()` līmenī, tāpēc katrs
-- ceļš — `cli brief`, `cli thread`, tiešs izsaukums vienreizējā skriptā — dod
-- tieši vienu rindu (verdikts 47b, 2026-09-07). Līdz tam rindu rakstīja tikai
-- brief ceļš (`brief_images`), tāpēc pavediena un sintēžu attēli budžetā
-- neparādījās; 2026-09-06 divi sintēzes attēli tā palika nereģistrēti.
--
-- Kāpēc jauna tabula, ne `brief_images` ar `kind`: `brief_images.note_id` ir
-- NOT NULL REFERENCES context_notes(id), un pavediena attēlam notes nav —
-- NOT NULL noņemšana SQLite-ā prasa tabulas pārbūvi.
--
-- `brief_images` PALIEK — tā ir apstiprināšanas darbplūsma (approved domēns,
-- image_path, varianti); `image_audit` ir tikai izmaksu un promptu reģistrs.
-- Vēsturiskās `brief_images` rindas ir iekopētas te ar
-- `source_table='brief_images'` + `source_id`, lai mēneša summa nepazustu;
-- unikālais indekss padara backfill idempotentu.
--
-- Laika zona: generated_at = LV (now_lv()), tāpat kā brief_images.
-- kind domēns: 'brief' | 'weekly' | 'synthesis' | 'thread' | 'direct'.
-- status domēns: 'ok' (attēls saņemts) | 'error' (API atteica; cost_usd = 0.0).
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_image_audit_source
    ON image_audit(source_table, source_id)
    WHERE source_table IS NOT NULL;

-- Laika zona: timestamp = LV (raksta now_lv() — src/db.py::log_action).
...
-- Laika zona: updated_at = UTC (shēmas noklusējums).
...
-- Laika zona: created_at = UTC (shēmas noklusējums — _upsert_donor to nepadod).
...
-- KNAB: Donations
-- Laika zona: scraped_at = LV (raksta now_lv() — src/knab/ingest.py::_insert_donation).
...
-- Laika zona: scraped_at = LV (raksta now_lv() — src/knab/ingest.py, deklarāciju INSERT).
...
-- Laika zona: created_at = LV (raksta now_lv() — src/knab/analyze.py, 4 INSERT vietas).
