...
def _compute_backlog(date: str, db_path: str | None) -> dict[str, Any]:
    ...
    try:
        # `documents.scraped_at` ir LV kolonna (rakstīta ar now_lv()), tāpēc te
        # NAV 'localtime' — modifikators to pārbīdītu vēlreiz. Sk. CLAUDE.md
        # § Schema invariants; vārti: tests/test_timestamp_timezone_gate.py.
        platform_rows = db.execute(
            """SELECT platform,
                      SUM(CASE WHEN DATE(scraped_at) = ? THEN 1 ELSE 0 END) AS today,
                      COUNT(*) AS total
                 FROM documents
        ...
        today_unrev = db.execute(
            ...
            "WHERE reviewed_at IS NULL AND DATE(scraped_at) = ?",  # LV kolonna — bez 'localtime'
            (date,),
        ).fetchone()["cnt"]
