...
Two endpoints are wired by the route layer:
  ...
  - `POST /api/deploy`        → runs `bash scripts/deploy.sh --no-delete`, logs result
...
def run_deploy_script(timeout: int = DEPLOY_TIMEOUT_SECONDS) -> dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            # `--no-delete` is passed explicitly even though it is now the
            # script's default. The button has no flag UI, so this caller can
            # never opt back out of additive mode by accident — and if the
            # default is ever flipped again, the button does not follow it.
            # shutil.which: uz Windows CreateProcess meklē System32 PIRMS PATH,
            # tāpēc kails "bash" ir WSL bash (bez npx un bez Windows env).
            [shutil.which("bash") or "bash", "scripts/deploy.sh", "--no-delete"],
            capture_output=True,
            text=True,
            timeout=timeout,
            cwd=_REPO_ROOT,
