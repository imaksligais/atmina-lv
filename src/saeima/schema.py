...
def init_saeima_tables(db_path: str | None = None) -> None:
    ...
    db.executescript("""
        ...
        -- Laika zona: created_at = LV (raksta now_lv() — src/saeima/votes.py::store_vote).
        -- vote_date/vote_time ir sēdes laiks no titania, ne ieraksta laiks.
        CREATE TABLE IF NOT EXISTS saeima_votes (
            ...
            result TEXT,
            -- Provenance of `result` (2026-08-18). Domēns:
            --   'agenda_label' = BURTISKA avota etiķete — darba kārtības punkta
            --                    `drawDKP_*(...)` 9. arguments (`<b>…</b>`), vai
            --                    renderētās balsojuma lapas etiķete. Citāts, ne
            --                    mūsu secinājums.
            --   'computed'     = aprēķināts no skaitļiem (klātesošo vairākums;
            --                    `total_nebalso` NAV saucējā).
            --   NULL           = izcelsme nezināma vai apgalvojuma nav vispār.
            -- Kāpēc: līdz šim `result` bija jauktas izcelsmes lauks bez kolonnas,
            -- kas to šķirtu, tāpēc neviena vērtība nebija citējama kā avota vārds
            -- (BACKLOG § Saeima). Vēsturiskās rindas paliek NULL — izcelsmi
            -- neuzminam.
            result_source TEXT
                CHECK (result_source IN ('agenda_label', 'computed')),
            url TEXT UNIQUE,
            summary TEXT,
            document_nr TEXT,
            document_url TEXT,
    ...
    # Migrācija 2026-08-18 — result_source esošām DB (CREATE TABLE IF NOT EXISTS
    # augstāk neskar jau izveidotu tabulu). Tas pats PRAGMA-nosacītais paterns kā
    # `bill_id` zemāk / `negative_patterns` src/db.py. CHECK ierobežojums brauc
    # līdzi ADD COLUMN, tāpēc domēns ir vienāds gan svaigā, gan migrētā DB.
    cols = [row[1] for row in db.execute("PRAGMA table_info(saeima_votes)").fetchall()]
    if "result_source" not in cols:
        db.execute(
            "ALTER TABLE saeima_votes ADD COLUMN result_source TEXT "
            "CHECK (result_source IN ('agenda_label', 'computed'))"
        )
...
def init_saeima_bills(db_path: str | None = None) -> None:
    ...
    db.executescript("""
        -- Laika zona: created_at = UTC (shēmas noklusējums), BET last_updated_at = LV —
        -- to raksta append_bill_stage() ar now_lv() (src/saeima/bills.py).
        -- Divas kolonnas, divas zonas, viena tabula.
        ...
        -- Laika zona: created_at = UTC (shēmas noklusējums) — append_bill_stage() to
        -- NEpadod, kaut dažas rindas zemāk raksta LV saeima_bills.last_updated_at.
