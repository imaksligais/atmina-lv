...
def parse_vote_snapshot(snapshot_text: str) -> VoteResult:
    ...
    faction_values = {'LPV', 'ZZS', 'JV', 'NA', 'AS', 'PRO', 'AP', 'LA', 'K', 'NP',
                      'ST', 'ST!'}
    # Store ONE canonical label per faction, whatever titania's cell says that
    # season — `parties.short_name` is the form coalition logic and
    # /audit-integrity check 9 key on. Verbatim storage of the drifted `ST!`
    # split one faction's 50k-ballot history across two labels (normalized in
    # data 2026-08-04, rollback data/rollback_faction_st_2026-08-04.sql).
    ...
    while i < len(cells):
        ...
        if i + 1 < len(cells):
            ...
            if next_cell in faction_values:
                faction = faction_normalize.get(next_cell, next_cell)
