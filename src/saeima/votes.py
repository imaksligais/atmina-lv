...
# ---------------------------------------------------------------------------
# Faction label canonicalization (shared by EVERY ingest path)
# ---------------------------------------------------------------------------
# ST/ST! = Stabilitātei! (Rosļikova frakcija, atdalījās no LPV 2024-01).
# Titania's cell text drifts by season: 2025-12 sēdēs `ST!`, 2026-03–04-01 `ST`,
# no 2026-04-16 cell ir tukša. Abas formas jāATPAZĪST (citādi parseris pazaudē
# deputātu un saglabā "ST!" kā pseudo-deputy), bet GLABĀT drīkst tikai vienu
# kanonisko formu — `parties.short_name`, jo tieši uz to atslēdzas
# `src.coalition.get_coalition_map()`, `FACTION_COLORS` un /audit-integrity
# 9. pārbaude. Verbatim `ST!` sadala vienas frakcijas vēsturi divās etiķetēs UN
# izkrīt no koalīcijas kartes ('other', ne 'opposition').
#
# ŠIS DZĪVO MODUĻA LĪMENĪ AR NOLŪKU (2026-09-04). Līdz šim karte bija lokāla
# `parse_vote_snapshot()` iekšienē, tāpēc otrs ingest ceļš —
# `scripts/p3_backfill_year_urllib.py::_decode_entry`, kas parsē JS
# `voteFullListByNames`, nevis HTML tabulu — to nekad neredzēja un 2026-08-04
# datu normalizācijas ATPAKAĻ ielika 7 448 `ST!` rindas. Jauns ingest ceļš
# importē `normalize_faction()`; savu karti NETAISA.
FACTION_NORMALIZE = {"ST!": "ST"}

# Šūnu vērtības, kas snapshot tabulā ir frakcijas etiķete (nevis vārds/balss).
# Satur ARĪ ne-kanoniskās formas — atpazīšana un glabāšana ir divi soļi.
FACTION_CELL_VALUES = {
    "LPV", "ZZS", "JV", "NA", "AS", "PRO", "AP", "LA", "K", "NP", "ST", "ST!",
}


def normalize_faction(raw: Optional[str]) -> Optional[str]:
    """Titania frakcijas šūna → kanoniskā `parties.short_name` forma.

    Tukša/None → None (Saeimas Prezidija locekļi balso bez frakcijas, un no
    2026-04-16 šūna ir tukša visiem). Nezināma etiķete iet cauri NEMAINĪTA —
    klusi to nomest nozīmētu pazaudēt jaunu frakciju; vārti
    (`tests/test_faction_label_canonical.py`) to noķer kā nesakritību ar
    `parties.short_name`.
    """
    if raw is None:
        return None
    ...
    if not f:
        return None
    return FACTION_NORMALIZE.get(f, f)
...
class VoteResult:
    ...
    total_atturas: int = 0
    total_nebalso: int = 0
    # Stored VERBATIM as the source labels it. The parser below only ever
    # writes `Pieņemts`/`Noraidīts`, but the corpus dictionary is wider: the
    # agenda page sometimes labels the ACTION a carried motion produced
    # (`Nod. kom.` = nodots komisijām, 4 rows, operator verdict 2026-08-18).
    # Readers must NOT assume two values — treat anything that is not
    # `Pieņemts` as "not the generic accept label" and render it literally with
    # a neutral badge. `None` = the source asserts no outcome.
    result: Optional[str] = None              # Pieņemts / Noraidīts / Nod. kom. / None
    # Where `result` came from — 'agenda_label' (burtiska avota etiķete),
    # 'computed' (aprēķināts no klātesošo vairākuma) vai None (nav apgalvojuma).
    # Bez šī lauka `result` ir jauktas izcelsmes un neviena vērtība nav citējama
    # kā avota vārds (BACKLOG § Saeima, 2026-08-18).
...
def parse_vote_snapshot(snapshot_text: str) -> VoteResult:
    ...
    # decisions pass with absolute majority of klātesošo deputātu. The 51-of-100
    # absolute majority only applies to special cases (constitutional changes,
    # no-confidence motions, etc.) and is NOT the right default for legislative
    # + Lm votes.
    #
    # Present = par + pret + atturas. `total_nebalso` is NOT in the denominator
    # (corrected 2026-08-17; the evidence is in
    # scripts/audit_saeima_vote_results.py::compute_expected_result).
    #
    # A row where NOBODY cast a ballot is not a motion — attendance
    # registration, quorum check, secret ballot for an office. Leave `result`
    # empty there: the old code fell through to `0 > 0 == False` and wrote
    # "Noraidīts", which put a fabricated rejection on 540 procedural rows.
    #
    ...
    #
    # Provenance is recorded alongside the value (`result_source`, 2026-08-18):
    # a label lifted from the page is a source quote ('agenda_label'), the
    # majority computation is ours ('computed'), and no result means no
    # provenance either. The authoritative label is the AGENDA item's — pass it
    # to `store_vote(agenda_result=...)`, which overrides both branches below.
    if "Noraidīts" in snapshot_text:
        vote.result = "Noraidīts"
        vote.result_source = "agenda_label"
    elif "Pieņemts" in snapshot_text or "pieņemts" in snapshot_text:
        vote.result = "Pieņemts"
        vote.result_source = "agenda_label"
    else:
        present = vote.total_par + vote.total_pret + vote.total_atturas
        if present:
            threshold = present // 2  # majority = strictly more than half
            if vote.total_par > threshold:
                vote.result = "Pieņemts"
            else:
                vote.result = "Noraidīts"
            vote.result_source = "computed"
    ...
    faction_values = FACTION_CELL_VALUES
    ...
    while i < len(cells):
        ...
        if i + 1 < len(cells):
            ...
            if next_cell in faction_values:
                faction = normalize_faction(next_cell)
...
def store_vote(
    ...
    *,
    ...
    agenda_result: Optional[str] = None,
) -> int:
    """Store a vote result with all individual ballots. Returns vote DB id.

    `agenda_result` is the VERBATIM outcome label of the agenda item this vote
    belongs to (`extract_agenda_result_labels()` on the DK page, or read off the
    rendered agenda). When given it wins over anything the parser produced and
    is recorded as `result_source='agenda_label'` — the source's own word.
    Without it the parser's provenance carries through ('computed' for the
    majority calculation, NULL when no outcome is asserted). Never synthesize a
    label to fill this: absent provenance is NULL, not a guess.
    ...
    result = vote.result
    result_source = vote.result_source
    if agenda_result and agenda_result.strip():
        result = agenda_result.strip()
        result_source = "agenda_label"
    if not result:
        # No outcome asserted → no provenance to record either.
        result_source = None
    # Keep the in-memory object consistent with what is persisted (same reason
    # `vote.url` is normalized in place above) — callers report from it.
    vote.result, vote.result_source = result, result_source

    db.execute(
        """INSERT INTO saeima_votes
           (agenda_item_id, motif, vote_date, vote_time,
            total_par, total_pret, total_atturas, total_nebalso,
            result, result_source, url, topic, summary, document_url,
            document_nr, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (agenda_item_id, vote.motif, vote.date, vote.time,
         vote.total_par, vote.total_pret, vote.total_atturas, vote.total_nebalso,
         result, result_source, vote.url, topic, summary, document_url,
         document_nr, now_lv()),
    )
...
# biļetenu vērtības ir Reģistrējies/Nereģistrējies, kas nav Par/Pret/Atturas.
# Prefikss, NE '%reģistrācij%' — pēdējais noķertu īstus likumus (Civilstāvokļa
# aktu reģistrācijas likums); tā pati izvēle kā render/votes.py:173.
_REGISTRATION_MOTIF_PREFIX = "Deputātu klātbūtnes reģistrācija"

# Vienīgās biļetena vērtības, kas ir nostāja. Reģistrējies/Nereģistrējies ir
# klātbūtnes stāvokļi — tie parādās gan reģistrācijas notikumos, gan atsevišķi
# īstu balsojumu vidū ("Kvoruma pārbaude"), un `vote_lv` kartē to nav, tāpēc
# stance sanāca no atkāpšanās zara ar neapstrādātu vērtību. Filtrs pēc vērtības,
# ne pēc motīva, noķer abas vietas un jebkuru nākotnes procedūru ar to pašu
# biļetena semantiku. Nebalsoja PALIEK — deputāts bija klāt un nenobalsoja, kas
# ir jēgpilna atzīme. (2026-07-25)
_BALLOT_VALUES_THAT_ARE_POSITIONS = frozenset({"Par", "Pret", "Atturas", "Nebalsoja"})
...
def generate_claims_from_votes(
    ...
    docs/superpowers/plans/archive/2026-04-25-saeima-vote-doc-cleanup.md.
    ...
    Māsas balsojumu kopsavilkuma konvencija (operatora lēmums 2026-08-17,
    atkārtoti apstiprināts 2026-08-18; variants b): `stance` top no `summary`,
    bet `summary` pieder LIKUMPROJEKTAM, ne konkrētajam balsojumam — viena
    `document_nr` balsojumi (lasījumi, priekšlikumi Nr.N, steidzamība) mēdz
    dalīt vienu tekstu. Ja tajā ir cita balsojuma iznākuma teikums («pirmajā
    lasījumā vienbalsīgi pieņemts (84 par)»), tas ieaužas arī māsas balsojuma
    stancē: 6916 (priekšlikums Nr.1, 2. lasījums, Noraidīts) nes 6103
    (1. lasījums) iznākumu visos 84 claims. Mērogs 2026-08-05 — 246 priekšlikumu
    balsojumi, ~20k claims. Operators to pieņēma kā likumprojekta KONTEKSTU:
    vēsturiskās rindas NETIEK pārrakstītas, un iznākuma frāze stancē lasāma kā
    ziņa par likumprojekta gaitu, nevis par šo balsojumu — paša balsojuma
    iznākums vienmēr ir `saeima_votes.result`, bet totāļi — `reasoning` laukā.
    Jauniem ne-lasījumu balsojumiem noteikums ir stingrāks: kopsavilkums saka
    tikai priekšlikuma būtību, bez cita balsojuma iznākuma teikuma
    (`.claude/agents/saeima-tracker.md` 3.B, vēsturiskais nosaukums Step 3.5).
    """
    claim_ids = []

    # Divi vārti pret to pašu defektu, aizsardzība dziļumā: notikuma tips un
    # biļetena vērtība. Biļetena filtrs viens pats pietiktu šodienas datiem, bet
    # motīva vārti tur arī gadījumu, kur reģistrācijas notikums kaut kā atnāktu
    # ar Par/Pret biļeteniem. Kopā tie 2026-07-25 aizvēra 30 476 claims (5,5% no
    # visiem) — sk. abu konstanšu komentārus augstāk.
    if (vote.motif or "").startswith(_REGISTRATION_MOTIF_PREFIX):
        return claim_ids

    tracked_votes = [
        iv for iv in vote.individual_votes
        if iv.politician_id and iv.vote in _BALLOT_VALUES_THAT_ARE_POSITIONS
    ]
    ...
    # The summary is loop-invariant — it was already read into `_summary_row`
    # above, for this same `vote_db_id`. Until 2026-08-15 the loop re-opened a
    # connection and re-ran that identical query once PER DEPUTY (~85 per vote)
    # and closed none of them: `_db.close()` appears nowhere in this file.
    summary = _summary_row["summary"] if _has_summary else None
    ...
    # 2026-08-21 operatora verdikts: VIENA kopsavilkuma rinda uz ielādi, nevis
    # claim-pa-claim — 642 764 no 646k logs rindām bija šī darbība (~246 MB).
    # Deputātu līmeņa informācija jau pilnībā dzīvo claims +
    # saeima_individual_votes; žurnāls tur auditu ielādes līmenī. Darbības
    # vārds saglabāts, lai dashboard aktivitātes filtrs (kas šo action klasi
    # apzināti izlaiž) un vēsturiskie vaicājumi paliek saderīgi.
    if claim_ids:
        log_action(
            ...
            details={
                ...
                "vote_db_id": vote_db_id,
                "claims_generated": len(claim_ids),
...
def process_vote_snapshot(
    ...
    *,
    ...
    Args:
        ...
        agenda_result: verbatim outcome label of the agenda item (from
                 `extract_agenda_result_labels()` on the DK page). Stored as
                 `result_source='agenda_label'`; omit when the agenda states
                 none — the row then keeps the computed value or stays NULL.
