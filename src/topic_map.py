...
Maps ~166 granular topic strings into 33 canonical groups for the position matrix.
...
TOPIC_GROUPS: dict[str, list[str]] = {
    ...
    # 2026-08-06 — 33. kanoniskā tēma (operatora lēmums; slieksnis >50 no
    # 2026-07-31 atlikšanas nostrādāja pie 85 ekrāna kandidātiem). NVO
    # finansējuma/pārvaldības debate kopš 07-22 šķēlās pa 8+ tēmām un tāpēc
    # nekad neparādījās tēmu tabulās. Kurēts backfill + re-embed:
    # data/fix_nvo_topic_migration_2026-08-06.sql.
...
# Latvian diacritics stripping for fuzzy matching. Vēsturiskais privātais
# nosaukums; vienīgā definīcija `src/lv_text.py` (2026-09-05, plāna 4.3).
# Te bija tikai mazo burtu tabula (13 zīmes) — bez uzvedības atšķirības, jo
# abi izsaukumi zemāk ievadu jau nolaiž ar `.lower()` (vārti:
...
_SAEIMA_KEYWORD_MAP: list[tuple[list[str], str]] = [
    ...
    # 2026-07-28 — bare "elektr" narrowed to energy-specific stems: it was a
    # substring of "Elektronisko plašsaziņas līdzekļu likums" (media law),
    # "elektroniskās naudas", "elektronisko sakaru" — same class as the bare
    # "sport" warning below. Media keyword "plašsaziņ" added to Sabiedriskie
    # mediji at the end of this list.
    (["enerģētik", "elektroenerģij", "elektroapgād", "elektrotīkl",
      "elektrostacij", "elektrolīnij", "elektrības", "degviel"],
     "Degviela un enerģētika"),
    ...
    (["sabiedrisk", "medij", "plašsaziņ"], "Sabiedriskie mediji"),
]
...
def normalize_topic(raw_topic: str) -> str:
    ...
    # Word-START anchored, not bare substring (2026-09-15): the bare `in` let
    # the 3-letter "ets" (EU emissions trading) fire inside "pretstatā", so a
    # health-care motion (136/P14, 2026-09-10) derived `Klimats` and 83 vote
    # claims inherited it. Same semantics as `src/saeima/claims.py::_stem`.
