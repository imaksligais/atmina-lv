...
# Vienīgā slug definīcija (`src/lv_text.py`, 2026-09-05). Līdz tam šis modulis
# importēja `_slugify` no `src.wiki` FUNKCIJAS IEKŠĀ, jo augšas importa gadījumā
# aizvērtos cikls `src.wiki:21 → src.wiki_lint` ↔ `src.wiki_lint → src.wiki` —
# repo vienīgais importu cikls. Ar leaf moduli atliktais imports vairs nav
# vajadzīgs; NEatgriez to atpakaļ uz `src.wiki`.
...
def lint_wiki(
    ...
    # Denominator (CLAUDE.md § "A gate that cannot fail is not evidence"): a
    # green "0 issues" must be distinguishable from "the walk found no files".
    # Reported per top-level folder so a vanished subtree shows up as a 0.
    scanned = [p for p in wiki.rglob("*.md") if p.is_file()]
    pages_by_dir: dict[str, int] = {}
    for p in scanned:
        rel = p.relative_to(wiki)
        key = rel.parts[0] if len(rel.parts) > 1 else "."
        pages_by_dir[key] = pages_by_dir.get(key, 0) + 1
    stats = {
        "pages_scanned": len(scanned),
        "pages_by_dir": dict(sorted(pages_by_dir.items())),
