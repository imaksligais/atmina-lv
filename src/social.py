...
def _store_tweets(tweets: list[dict], opponent_id: int) -> list[dict]:
    ...
    """
    from src.matcher import extract_twitter_author_handle, is_project_account_url
    ...
    for tweet in tweets:
        ...
        # 2026-09-19: the project's own account is not a politician. Its
        # tweets surface here through the feeds of the people they tag
        # (reply / quote / thread context) and would otherwise be stored with
        # 'mentioned' junctions — see PROJECT_X_HANDLES in src/matcher.py.
        if is_project_account_url(source_url):
            continue
