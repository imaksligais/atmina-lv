...
def fetch_pretrunas_candidates(db_path: str | None = None) -> list[dict]:
    ...
    for r in rows:
        d = dict(r)
        # Chronological swap — mirrors src.render._common._enrich_contradiction so that
...
def fetch_highlights_candidates(db_path: str | None = None, lookback_days: int = 7) -> list[dict]:
    """Return list of highlight candidates from recent political_tensions.

    Each row is a dict with `kind` = 'tension' + pillar-specific fields.
    Skips rows already represented in approved/posted social_drafts.
    (The former 'attack' branch read oppo_briefs — a politracker-era table
    that was always empty since the 2026-04-06 pivot; removed 2026-07-29.)
    """
