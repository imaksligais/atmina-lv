...
# Mirrors CATEGORY_LV in src.render._common — duplicated here to avoid a circular import.
_CATEGORY_SUBTITLE = {
...
def draft_highlight(row: dict) -> str:
    """Dispatch to the tension renderer (the only highlight kind since 2026-07-29)."""
    kind = row.get("kind")
    if kind == "tension":
