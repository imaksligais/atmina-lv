...
KNOWN_KEYS = [
    ...
    "dashboard_password",
    "session_secret",
    # `youtube_api_key` + `facebook_page_token` removed 2026-08-15 with their
    # fetchers (src/social.py). They were never set, so `verify_all()` reported
    # them missing on EVERY preflight run — two permanent WARNINGs for
    # platforms Data Contract #11 says we do not ingest.
    "anthropic_api_key",
    "typesafe_api_key",  # TypeSafe (Jev) System One — matcher veto + Saites proposals (2026-09)
    "watchdog_smtp_host",
    "watchdog_smtp_user",
    "watchdog_smtp_pass",
    "watchdog_alert_to",
