...
# Latvijas reģ.nr.: 11 cipari. 4 = UR juridisko personu sērija, 9 = vecākās
# struktūras; 5 = VID izsniegtā sērija (biedrības/institūti arī ar 50008…,
# 50103… u.c.) — paplašināts 2026-08-21 pēc mērījuma: 600 neizgūtu numuru
# visi ar 5-prefiksu, glabātajā korpusā tikai 4/9, 3-sērijas (personu kodi)
# nav novēroti nevienā raw_html.
_REG_NUMBER_RE = re.compile(r"\b[459]\d{10}\b")
...
_FREEFORM_MONEY_RE = re.compile(
    r"(\d[\d\s.,]*\d|\d)\s*"
    r"(EUR|USD|LVL|LTL|EEK|RUB|GBP|JPY|CHF|SEK|NOK|DKK|PLN|CZK|CAD|AUD)\b",
    re.IGNORECASE,
)


def _parse_freeform_money(text: str) -> list[tuple[float, str]]:
    """Vecā VAD formāta (2002–~2010) vienšūnas naudas lauks:
    '520 000 USD' vai '5 928 853 USD, 450 000 EUR, 7813 LVL'."""
    out = []
    for m in _FREEFORM_MONEY_RE.finditer(text or ""):
        amt = _norm_amount(m.group(1))
        cur = _norm_currency(m.group(2))
...
def _parse_debts(siblings) -> list[VadDebtRow]:
    ...
    for cells in _table_rows(siblings):
        if len(cells) >= 4:
            creditor = cells[0]
            ra = cells[1]
            amt = _norm_amount(cells[2])
            cur = _norm_currency(cells[3])
            ...
            if amt is None or cur is None:
                continue
            out.append(VadDebtRow(
                creditor_name="", amount=amt, currency=cur,
                amount_in_words=cells[2] or None,
            ))
        elif len(cells) == 1:
            for amt, cur in _parse_freeform_money(cells[0]):
                out.append(VadDebtRow(
                    creditor_name="", amount=amt, currency=cur,
                    amount_in_words=cells[0] or None,
                ))
    return out
...
def _parse_loans_given(siblings) -> list[VadLoanGivenRow]:
    ...
    for cells in _table_rows(siblings):
        if len(cells) >= 3:
            ...
            for amt, cur in _parse_freeform_money(cells[0]):
                out.append(VadLoanGivenRow(
                    amount=amt, currency=cur, amount_in_words=cells[0] or None,
                ))
    return out
...
def _parse_other_info(siblings):
    """13. sadaļa — brīvteksts. VID renderē saturu ne tikai <p>/<div>, arī
    <table> rindās (205 no 1276 §13 blokiem, 2026-08-21 mērījums) — abas formas
    jāsavāc, citādi other_info paliek NULL, kaut raw_html saturu ir."""
    if not siblings:
        return None
    blocks = []
    for sib in siblings:
