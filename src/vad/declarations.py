...
from src.vad.denylist import VadDenylist, deny_hit, load_denylist
...
def fetch_for_politician(
    ...
    *,
    ...
    dry_run: bool = False,
    denylist: VadDenylist | None = None,
) -> StoreResult:
    ...
    denylist (None → ielādē data/vad_denylist.json): homonīmu aizsardzība pirms
    disambig un dedup soļiem — bloķētās rindas netiek arī vad_uuid refresh.
    """
    pol = db.execute(
    ...
    for r in rows:
        ...
        # Homonīmu deny-list PIRMS disambig un dedup — bloķētā rinda nedrīkst
        # ne iet cauri filtriem, ne atsvaidzināt esošās rindas vad_uuid.
        hit_reason = deny_hit(opponent_id, r.vad_uuid, kind, year, denylist)
        if hit_reason:
            log.warning(
                "vad-denylist-skip: pid=%d name=%r uuid=%r kind=%r year=%s reason=%s",
                opponent_id, name, r.vad_uuid, kind, year, hit_reason,
            )
            result.rows_skipped_denylist += 1
            continue
