...
# CVK (Centrālā vēlēšanu komisija) datu portāli — kandidātu saraksti,
# vēlēšanu rezultātu tabulas. Tie NAV ziņu raksti: matcher tos sasaista
# ar desmitiem politiķu vienlaikus un plūsmā tie parādās kā viena
# bezsaturīga "SV2026" kartīte. Izslēgts renderī (read-side), NEVIS
# dzēšot dokumentus — claims/provenance paliek neskarti.
# Operatora verdikts 2026-08-17 (BACKLOG § Operatora verdikti, Matcher).
_CVK_DOMAINS = ("cvk.lv", "cvk.gov.lv")


def _is_cvk_domain(domain: str | None) -> bool:
    """True, ja domēns ir CVK datu portāls (pats domēns vai apakšdomēns).

    Domēna kritērijs, nevis virsraksta teksts: CVK lapu ``<title>``
    mainās katru vēlēšanu ciklu (``SV2026``, ``EPV2024``), domēns ne.
    """
    d = (domain or "").strip().lower().rstrip(".")
    if d.startswith("www."):
        d = d[4:]
    return any(d == base or d.endswith("." + base) for base in _CVK_DOMAINS)
...
def _fetch_news(db: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    ja tiem ir claims (un renderējas bez personu tagiem). CVK datu
    portālu dokumenti (``_is_cvk_domain``) plūsmā neietilpst vispār.
    """
    rows = db.execute("""
    ...
    for r in rows:
        d = dict(r)
        if _is_cvk_domain(d.get("source_domain")):
            continue  # CVK kandidātu saraksti / rezultātu tabulas — ne ziņa
        doc = docs.get(d["id"])
        if doc is None:
