...
shows the hero with stats + sparklines + trends; the
...
from src.render.focus import (
    _Shown,
    _hero_contradictions,
    _hot_topic,
    _quote_of_day,
    _slot_b_contradiction,
    assemble_focus,
    hero_feed,
)
...
def _fetch_stats(db: sqlite3.Connection) -> dict[str, int]:
    ...
    There is deliberately no ``saeima_vote`` claim count here. One existed
    until 2026-08-02 as ``saeima_claims``: computed on every dashboard
    render, passed into the template context, and read by no template —
    a ``COUNT(*)`` over 568k rows whose result never reached a page. The
    vote-side numbers that ARE published live elsewhere: ``wiki/index.md``
    via ``src/wiki.py`` and the per-politician node counts in
    ``src/render/links.py``. ``votes`` below counts the ledger
    (``saeima_votes``), which is the right source for a headline anyway.
    """
    cutoff_7d = (today_lv() - timedelta(days=7)).strftime("%Y-%m-%d")
    return {
        # Real politicians only. The template renders this as "N politiķi"
        # (about.html.j2), and the old `NOT IN ('inactive','commentator')` form
        # counted LETA, LTV Panorāma, De Facto, Latvijas Banka and 23 other
        # audience slots among them — 194 where the truthful figure is 167
        # (operator decision 2026-08-15). A POSITIVE predicate is deliberate:
        # an exclusion list has to be extended every time the taxonomy grows,
        # and `'commentator'` in the old list was already dead (0 rows, inv #11
        # deprecated it). Audience accounts are not lost — they keep their
        # profiles, positions and topic rows; they are simply not "politiķi".
        ...
        # Published pretrunas only — `confirmed=0` rows are @devils-advocate
        # survivors / withdrawn rows awaiting the operator (escalation rule 3).
        # Same gate as src/render/contradictions.py; locked by
        # tests/test_confirmed_gate_all_surfaces.py (2026-09-20).
        "contradictions": db.execute(
            "SELECT COUNT(*) FROM contradictions WHERE COALESCE(confirmed, 1) = 1"
        ).fetchone()[0],
...
def _fetch_hero_v2_data(db: sqlite3.Connection) -> dict[str, Any]:
    """28-day sparklines + 7-day contradiction count for the hero v2 layout."""
    ...
    # `COALESCE(confirmed, 1) = 1`: an unconfirmed candidate must not show up
    # as "1 jauna pretruna" in the Šonedēļ strip (it did, 2026-09-20, #49).
    contradictions = series(
        "SELECT date(detected_at) d, COUNT(*) n FROM contradictions "
        "WHERE date(detected_at) >= ? AND COALESCE(confirmed, 1) = 1 GROUP BY d",
        (start,),
    )
    active = series(
    ...
    return {
        ...
        # `hero_ticker` removed 2026-08-15: it queried the 6 newest claims and
        # formatted relative times on every homepage render, and no template,
        # stylesheet or script ever read it — a repo-wide grep for
        # hero_ticker / heroTicker / hero-ticker returned exactly one hit, the
        # producer itself. Work computed and thrown away.
    }
...
_ANALYSIS_NEW_DAYS = 7     # „Jauns" marķieris joslā „Jaunākais" un „Vairāk analīžu"

def analysis_items(
    analyses: list[dict[str, Any]],
    syntheses: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Analyses ∪ syntheses kā viena kopa, kārtota PĒC DATUMA dilstoši
    (2026-08-12: ne pa vienam no katra tipa — mēneša veca sintēze stāvēja
    blakus svaigai analīzei). Pirmais elements iet joslā „Jaunākais",
    nākamie divi — sadaļā „Vairāk analīžu" (spec 2026-09-20)."""
    items: list[dict[str, Any]] = []
    for a in analyses[:3]:
        items.append({
            "kind": "analysis", "kicker": "Tematiskā analīze",
            "kicker_href": "analizes.html#tematic",
            "url": a["url"], "img_dir": "images/analizes",
            "title": a["title"], "description": a.get("description", ""),
            "date": str(a.get("date", ""))[:10],
            "image_filename": a.get("image_filename"),
            "image_light_filename": a.get("image_light_filename"),
            "cta": "Lasīt analīzi",
        })
    for s in syntheses[:3]:
        items.append({
            "kind": "synthesis", "kicker": "Sintēze",
            "kicker_href": "analizes.html#sintezes",
            "url": f"sintezes/{s['slug']}.html", "img_dir": "images/synthesis",
            "title": s["title"], "description": s.get("description", ""),
            "date": str(s.get("created", ""))[:10],
            "image_filename": s.get("image_filename"),
            "image_light_filename": s.get("image_light_filename"),
            "cta": "Lasīt sintēzi",
        })
...
def _age_days(date_str: str, today_iso: str) -> int | None:
    try:
...
def latest_strip(
    ...
    """Josla „Jaunākais" zem meklētāja (spec 2026-09-20): jaunākais pārskats
    (`blog_posts[0]`, jau kārtoti pēc subjekta datuma) + jaunākā analīze/
    sintēze (`items[0]`). `is_new` = vecums ≤ _ANALYSIS_NEW_DAYS. Bez
    pārskata — tikai analīze; bez nekā — None (josla nerenderējas).
    Ievades netiek mutētas — `blog_posts` koplieto arī blog un analizes lapas."""
    brief = dict(blog_posts[0]) if blog_posts else None
    analysis = dict(items[0]) if items else None
    if brief is None and analysis is None:
        return None
    for it in (brief, analysis):
        if it is not None:
            age = _age_days(it.get("date", ""), today_iso)
            it["is_new"] = age is not None and 0 <= age <= _ANALYSIS_NEW_DAYS
    if brief is not None and not brief.get("headline"):
        brief["headline"] = brief.get("title")
...
def render_dashboard(
    ...
    # Lapas līmeņa dedup (2026-09-23): viena un tā pati pretruna/citāts
    # nedrīkst atkārtoties hero karuselī, «Svaigajā pretrunā», «Karstās tēmas»
    # citātos un «Dienas citātā». Prioritāte augšup:
    #   1) karuselis (pirmais ekrāns) patur savas kartītes;
    #   2) «Svaigā pretruna» (B slots) = pirmā svaigā pretruna ĀRPUS karuseļa;
    #      ja tādas nav — assemble_focus esošais rezerves ceļš
    #      (spriedze → citāts → tukšs slots);
    #   3) karstās tēmas citāti, dienas citāts un karuseļa pozīcijas izlaiž
    #      visu, kas jau redzams — sakritība pēc claim id / source_url /
    #      normalizēta teksta (_Shown).
    hero_cons = _hero_contradictions(hero_cards)
    hero_con_ids = {c.get("id") for c in hero_cons}
    shown = _Shown()
    for c in hero_cons:
        shown.add_contradiction(c)
    # B slota pretrunu var paredzēt pirms focus salikšanas — tās citāti arī
    # ir "jau redzami", tāpēc karstās tēmas/dienas citāta atlase filtrē pret tiem.
    slot_b_con = _slot_b_contradiction(contradictions, hero_con_ids)
    if slot_b_con is not None:
        shown.add_contradiction(slot_b_con)
    ...
    focus = assemble_focus(
        _hot_topic(db, exclude=shown), contradictions,
        tensions if tensions is not None else _fetch_tensions(db),
        _quote_of_day(db, exclude=shown),
        exclude_con_ids=hero_con_ids,
    )
    ...
    # kompozīta citātiem UN lapā jau redzamajām pretrunām (shown).
    hero_items = hero_feed(db, hero_cards, votes, focus, shown=shown)

    items = analysis_items(analyses, syntheses)
    latest = latest_strip(blog_posts, items, today_lv().isoformat())
    ...
    _render_page(env, "index.html.j2", atmina_dir / "index.html", {
        ...
        "latest": latest,
        "recent_briefs": blog_posts[1:4],   # [0] jau joslā „Jaunākais"
        "more_analyses": items[1:3],
