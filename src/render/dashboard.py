...
_ANALYSIS_NEW_DAYS = 7     # "JAUNS" marķieris duo kartītē
_ANALYSIS_FRESH_DAYS = 14  # svaiguma josla zem hero

def _analysis_feed_items(
    analyses: list[dict[str, Any]],
    syntheses: list[dict[str, Any]],
    today_iso: str,
) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    """Apvienotā "Jaunākās analīzes" kopa sākumlapai (2026-08-12).

    Atgriež ``(duo, fresh)``: ``duo`` — divi jaunākie ieraksti no
    analyses ∪ syntheses PĒC DATUMA (ne pa vienam no katra tipa — līdz
    2026-08-12 mēneša veca sintēze stāvēja blakus svaigai analīzei un
    grāva sadaļas virsrakstu), katrs ar ``is_new`` (≤7 d) marķieri;
    ``fresh`` — jaunākais ieraksts svaiguma joslai zem hero, ja tas ir
    ≤14 d vecs, citādi None (josla pazūd pati, bez roku darba).
    """
    ...
    def _age(it: dict[str, Any]) -> int | None:
        try:
    ...
    if duo:
        age = _age(duo[0])
        if age is not None and 0 <= age <= _ANALYSIS_FRESH_DAYS:
            fresh = duo[0]
    return duo, fresh
...
def render_dashboard(
    ...
    analysis_duo, fresh_highlight = _analysis_feed_items(
        analyses, syntheses, today_lv().isoformat()
    )
    ...
    _render_page(env, "index.html.j2", atmina_dir / "index.html", {
        ...
        "analysis_duo": analysis_duo,
        "fresh_highlight": fresh_highlight,
