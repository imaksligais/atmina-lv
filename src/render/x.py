...
def _fetch_x_data(db: sqlite3.Connection) -> dict[str, Any]:
    ...
    # dp.role='subject' is load-bearing, not a filter for tidiness: the template
    # prints politician_name as the post's AUTHOR, so a 'mentioned' row renders
    # someone else's tweet under this politician's name. Found 2026-08-08 —
    # Dombrava's tweet carrying an unverified allegation about a named person
    # also rendered as Marats Kasems', because he is a legitimate 'mentioned'
    # link (Data Contract #11). 635 of 1500 feed rows were non-subject. Same
    # discipline as the profile X subtab (`politicians.py` role filter).
