...
def _initials(name: str) -> str:
    """Avatar fallback: first letters of the first and last name parts.

    Single source for the expression that used to be inlined 4x in
    ``index.html.j2`` (``name.split()[0][0] + name.split()[-1][0]`` —
    backlog/vietne-ui.md § gala revīzijas paliekas)."""
    parts = name.split()
    if not parts:
        return ""
...
def _biggest_reversals(
    ...
    One row per person (2026-09-23): the list is sorted desc, so the first
    occurrence of a person already carries their largest gap; later
    contradictions of the same person are skipped instead of consuming
    another row (two rows of one name read as a duplicate bug)."""
    ...
    for c in eligible:
        ...
        # A row without a resolvable person keeps its own identity so
        # unrelated nameless contradictions don't collapse into one row.
        key = slug or f"__row_{c.get('id')}"
        if key in seen:
            continue
        seen.add(key)
        name = c.get("politician_name") or ""
        out.append({
            "id": c.get("id"),
            "name": c.get("politician_name"),
            "slug": slug,
...
def _vote_alignment_outliers(
    ...
    Neutral metric (UI: "Zemākā sakritība ar Saeimas vairākumu") — a factual
