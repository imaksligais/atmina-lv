...
Sub-page boundary: imports only from ``src.render._common``, the leaf
constant modules (``src.image_variants``, ``src.topic_map``) and stdlib.
"""
...
# Reverse map slug → canonical topic-group name. Frontmatter ``topics``
# historically carried slugs (``koalicija-un-partijas``), but the topic
# destination pages match on the canonical NAME
# (``src/render/topics.py::_fetch_topic_detail``: ``name in s["topics"]``),
# so 8 of 9 syntheses attached nowhere. Normalize once at load time.
_TOPIC_SLUG_TO_NAME: dict[str, str] = {_slugify(n): n for n in TOPIC_GROUPS}
...
def _enhance_synthesis_html(
    ...
    a. **Empty-cell muting (authored markdown).** ``<td>—</td>`` (with an
       optional ``align`` attr) gains ``class="cell-empty"``. Runs FIRST:
       once (b) has added ``data-label`` to a ``<td>``, the bare
       ``<td>—</td>`` pattern no longer matches it.
    b. **Mobile column labels.** ``_label_table_cells`` adds each ``<td>``'s
       column header as ``data-label`` (the ≤768 px card layout reads it via
       ``td[data-label]::before``) and segments long prose cells. Runs BEFORE
       widget injection on purpose: ``syn-*`` widget tables are already cards
       with their own structure and must NOT get labels (backlog vietne-ui).
    c. **Widget injection.** A paragraph whose content is exactly
    ...
    d. **Table scroll wrap.** Every ``<table>…</table>`` not already inside a
    ...
    e. **Empty-cell muting (widget tables).** Same pass as (a), repeated
       because (a) predates injection — a widget ``<td>—</td>`` mutes too.
    f. **h2 anchors + TOC.** Each ``<h2>`` gets a unique ``id`` (via
    ...
    # (a) Empty-cell muting — before labeling, whose data-label attr would
    #     break the bare `<td>—</td>` match.
    html = _EMPTY_TD_RE.sub(_mute_cell, content_html)

    # (b) td[data-label] — authored markdown tables only; widget tables are
    #     not in the HTML yet (injected at (c)), which is exactly the intent.
    html = _label_table_cells(html)

    # (c) Widget injection — before the table wrap so injected tables also wrap.
    ...
    html = _WIDGET_MARKER_RE.sub(_inject, html)

    # (d) Table scroll wrap — skip a table already wrapped by a widget author.
    html = _wrap_tables(html)

    # (e) Empty-cell muting, second pass — covers widget tables injected at (c).
...
def _normalize_synthesis_topics(raw: Any, source: str) -> list[str]:
    """Map frontmatter ``topics`` values onto canonical ``TOPIC_GROUPS`` names.

    Accepts the canonical name verbatim (``airBaltic``) or its slug
    (``valsts-kapitalsabiedribas``). An unrecognized value is NOT dropped —
    it stays in the returned list (it simply matches no topic page) and is
    reported on stderr, one line per value, so a frontmatter typo surfaces
    instead of silently detaching a page (CLAUDE.md § "Silent success is a
    defect class").

    Order is preserved; duplicates that collapse onto the same canonical
    name (slug + name for the same group) are kept once.
    """
    out: list[str] = []
    for value in raw or []:
        ...
        if not text:
            continue
        if text in TOPIC_GROUPS:
            resolved = text
        else:
            mapped = _TOPIC_SLUG_TO_NAME.get(_slugify(text))
            if mapped is not None:
                resolved = mapped
            else:
                print(
                    f"[syntheses] {source}: neatpazīta tēmas vērtība "
                    f"'{text}' — tā neatbilst nevienai no {len(TOPIC_GROUPS)} "
                    "kanoniskajām tēmu grupām un netiks piesaistīta "
...
def _load_syntheses(atmina_dir: Path = Path("output/atmina")) -> list[dict[str, Any]]:
    ...
    for md_file in sorted(syn_dir.glob("*.md"), reverse=True):
        ...
        # Neobligāts gaišās tēmas dvīnis <slug>-light.png (tas pats
        # eksistences kritērijs kā has_image; paterns kā analyses image_light).
        img_light_name = f"{md_file.stem}-light.png"
        has_light = (syn_img_dir / img_light_name).exists()
        syntheses.append({
            "slug": md_file.stem,
            "title": fm.get("title", ""),
            ...
            "topics": _normalize_synthesis_topics(
                fm.get("topics", []) or [], md_file.name
            ),
            ...
            "image_light_filename": img_light_name if has_light else None,
...
def render_syntheses(
    ...
    for syn in syntheses:
        ...
        if syn.get("image_filename"):
            ...
            if syn.get("image_light_filename"):
                # Gaišās tēmas hero dvīnis — blog-post.html.j2 renderē abus
                # <img> ar theme-img-* klasēm (rāda CSS pēc data-theme).
                post_ctx["image_light_src"] = (
                    f"../images/synthesis/"
                    f"{variant_filename(syn['image_light_filename'], 'hero')}"
                )
