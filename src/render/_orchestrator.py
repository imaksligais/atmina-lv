...
Sub-page private helpers (``_fetch_x_data``, …) are imported from
their own module (``src.render.x``, …) — the historical
``src/generate.py`` shim was retired 2026-09-05 (plāna 5.10).
...
# Asset/chrome plumbing — carved out to src/render/_assets.py (phase 5.7).
# Re-imported here so the historical import path
# `from src.render._orchestrator import _copy_brief_images` and the
# `monkeypatch.setattr(src.render._orchestrator, ...)` patch points in
# tests/test_orchestrator_gating.py, tests/test_brief_variants_selfheal.py and
# tests/test_synthesis_image_variants.py keep resolving. `generate_public_site`
# below calls these through THIS module's globals, so patching them on
# `src.render._orchestrator` still takes effect.
from src.render._assets import (  # noqa: F401  re-exported: historical import path
    DEPLOYABLE_BRIEF_SUFFIXES,
    _base_chrome_blocks,
    _copy_brief_images,
    _copy_curated,
    _copy_images,
    _ensure_image_variants,
    _heavy_fetch_plan,
    _is_variant_of,
    _rejected_brief_stems,
    _rendered_chrome,
    _sync_curated_chrome,
)
...
def _copy_host_config(atmina_dir: Path) -> list[str]:
    ...
    _redirects — Cloudflare pārrakstīšanas kārtulas (sakne → index.html).
    .assetsignore — ko wrangler NEaugšupielādē (šie paši konfigurācijas faili,
                 kas citādi būtu publiski lasāmi kā /.htaccess u.c.).
    """
    pairs = (
    ...
    for src_name, dest_name in pairs:
        ...
        if src.exists():
            # newline LF: Windows write_text citādi dod CRLF; Cloudflare
            # `_headers` parseris un LiteSpeed .htaccess gaida LF.
...
# RFC 9116 neļauj bezgalīgu termiņu un iesaka ne ilgāku par gadu; 180 dienas
# nozīmē, ka fails paliek derīgs arī tad, ja renderis kādu laiku nav skriets.
SECURITY_TXT_VALID_DAYS = 180


def security_txt(now: datetime) -> str:
    """`/.well-known/security.txt` saturs (RFC 9116).

    `Expires` ir renderēšanas brīdis plus `SECURITY_TXT_VALID_DAYS`, RFC 3339
    formātā UTC laikā ar `Z` beigās — tāpēc šeit NAV `now_lv()`: RFC prasa
    nepārprotamu nobīdi, un `Z` ir vienīgā forma, ko atpazīst visi lasītāji.
    Sekundes noapaļotas uz 00, lai divi viena renderēšanas skrējiena izsaukumi
    nedotu atšķirīgu tekstu.

    Naivs `datetime` tiek lasīts kā UTC — tests to fiksē, lai nejaušs naivs
    laiks neradītu nobīdi klusējot.
    """
    if now.tzinfo is None:
        now = now.replace(tzinfo=timezone.utc)
    expires = (now.astimezone(timezone.utc) + timedelta(days=SECURITY_TXT_VALID_DAYS)).replace(
    ...
    return (
        ...
        f"Policy: {BASE_URL}/kontakti.html\n"
    )


def _write_security_txt(atmina_dir: Path, now: datetime | None = None) -> Path:
    """Uzraksta `security.txt` koka `.well-known/` apakškatalogā (LF, UTF-8)."""
    dest = atmina_dir / ".well-known" / "security.txt"
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_text(
        security_txt(now or datetime.now(timezone.utc)), encoding="utf-8", newline="\n"
    )
    return dest
...
def generate_public_site(
    ...
    # Self-healing FIRST, then copy: `_copy_brief_images` deliberately skips raw
    # PNG masters, so without this the deploy tree gets nothing at all for a
    # brief whose variants were never generated — and the page still references
    # <slug>-hero.webp / <slug>-og.jpg, which then 404 on the live site. That
    # exact failure shipped twice (2026-08-02 audit) and recurred 2026-08-08.
    # `analizes` and `synthesis` already self-heal below; briefs did not.
    #
    # Noraidīto kopu lieto ABOS soļos. Tikai ģenerēšanas izlaišana ir kosmētiska:
    # rinda, kas kļūst noraidīta pēc variantu uzģenerēšanas, paliek avota mapē un
    # tiktu nokopēta un aizsūtīta (2026-08-16 mērījums — sk. _rejected_brief_stems).
    _rejected_stems = _rejected_brief_stems(db_path)
    _ensure_image_variants(briefs_src, skip_stems=_rejected_stems)
    copied = _copy_brief_images(briefs_src, briefs_dest, skip_stems=_rejected_stems)
    logger.info(
        "Copied %d brief images to %s (%d noraidīti stem izlaisti)",
        copied,
        briefs_dest,
        len(_rejected_stems),
    )
    ...
    #
    # Bāzes PNG patiesības avots kopš 2026-09-10 ir wiki/synthesis/images/ —
    # VERSIONĒTS koks, ne output/. Līdz tam masteri dzīvoja tikai output/, kas ir
    # pilnībā gitignored, tāpēc svaigs klons renderēja sintēzes BEZ hero attēliem
    # un `git clean -xdf` tos iznīcināja neatgriezeniski (ģenerēti par naudu,
    # nanobanana to pašu attēlu neatdod). Mērīts tajā dienā: 4 no 20 masteriem
    # bija tikai deploy kokā, jo attēlu skripti rakstīja tieši tur, apejot avotu.
    # Kopējam ar `overwrite=False`: ja avota mapē jau ir tā paša vārda fails,
    # versionētais NEPĀRRAKSTA to — vietējs pārģenerējums paliek spēkā līdz
    # brīdim, kad to apzināti iekopē wiki/synthesis/images/.
    _synth_src = output / "images" / "synthesis"
    _synth_src.mkdir(parents=True, exist_ok=True)
    _synth_masters = ASSETS_DIR.parent / "wiki" / "synthesis" / "images"
    if _synth_masters.exists():
        restored = 0
        for master in sorted(_synth_masters.glob("*.png")):
            target = _synth_src / master.name
            if not target.exists():
                shutil.copy2(master, target)
                restored += 1
        if restored:
            logger.info(
                "Restored %d synthesis image masters from %s", restored, _synth_masters
            )
    ...
    # Download D3 if not present (saites.html force graph)
    d3_js = atmina_dir / "assets" / "d3.v7.min.js"
    if not d3_js.exists():
        _download_d3(d3_js)
    ...
    if _want("dashboard"):
        ...
        render_dashboard(
            ...
            days_until, rankings, tensions=tensions,
        )
    ...
    if _want("static"):
        ...
        # 14b. /.well-known/security.txt (RFC 9116)
        logger.info("security.txt written: %s", _write_security_txt(atmina_dir))

        # 15. sitemap.xml
        topic_slugs = [t["slug"] for t in _fetch_topics(db)]
        # Same source of truth as src/render/laws.py::_fetch_law_pages —
        # wiki/laws/*.md stems, minus the likumi.md index file.
        law_slugs = sorted(
            p.stem for p in Path("wiki/laws").glob("*.md") if p.name != "likumi.md"
        )
        _generate_sitemap(atmina_dir, politicians, parties, blog_posts, analyses, contradictions, syntheses, bills, topic_slugs, law_slugs)
...
def _generate_sitemap(
    ...
    ``law_slugs`` came late (2026-08-01): the ``likumi`` vertical shipped without
    ever being added here, so ``likumi.html`` + 33 per-law pages were emitted and
    then left out of the sitemap entirely — an entire content vertical invisible
    to crawlers except through internal links. Found by ``scripts/check_output.py``,
    which diffs the emitted ``.html`` set against the ``<loc>`` set precisely so a
    whole missing vertical cannot hide again.
    """
    ...
    root_pages = [
        ...
        "spriedzes.html", "about.html", "kontakti.html", "blog.html", "likumi.html",
        "statistika.html",
    ]
    for p in root_pages:
    ...
    for slug in law_slugs or []:
        urls.append(f"{BASE_URL}/likumi/{slug}.html")
