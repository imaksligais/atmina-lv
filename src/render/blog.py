...
``src.briefs``, and stdlib + extern (markdown, jinja2).
"""
...
def _compute_brief_footers(db: sqlite3.Connection) -> dict[str, dict[str, int]]:
    ...
    # Same gate as the brief body (briefs.py `COALESCE(confirmed, 1) = 1`) —
    # the footer must not count a candidate the brief itself did not report.
    contras = {
        d_: cnt for d_, cnt in db.execute(
            "SELECT date(detected_at), COUNT(*) FROM contradictions "
            "WHERE COALESCE(confirmed, 1) = 1 GROUP BY date(detected_at)"
        ).fetchall()
    }
...
def _fetch_blog_posts(db: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    for r in rows:
        ...
        # Virsraksts nāk no PAŠREIZĒJĀ satura, ne no denormalizētās kolonnas.
        # `visual_brief_json` aizpildās TIKAI rakstīšanas brīdī (src/tools.py →
        # parse_visual_brief), tāpēc vēlāks `## Vizuālais brief` bloka labojums
        # tajā nekad neparādās un noglabātais virsraksts uzvarēja og:title,
        # twitter:title, H1 un abus `alt` tekstus (pārskats #505, 2026-08-26).
        # Precedence: saturs → noglabātais JSON → no virsraksta atvasinātais
        # fallback. Tukšs virsraksts jebkurā līmenī krīt uz nākamo, tāpēc
        # 13 no 184 pārskatiem, kuru bloks nesaparsējas, renderējas kā līdz šim.
        headline = None
        vb_from_content = parse_visual_brief(d.get("content") or "")
        if vb_from_content:
            headline = (vb_from_content.get("headline") or "").strip() or None
        if not headline:
            vb_json = d.get("visual_brief_json")
            if vb_json:
                try:
                    vb = json.loads(vb_json)
                    stored_headline = vb.get("headline")
                    if isinstance(stored_headline, str):
                        stored_headline = stored_headline.strip() or None
                    if stored_headline:
    ...
    for p in posts:
        ...
        deduped.append(p)
    # Listing order follows the brief's CONTENT date, not created_at — the
    # same identity the slug uses. created_at moves whenever a published brief
    # is corrected through the store_context_note() UPSERT, and on 2026-09-14
    # two corrected old dailies (09-08, 09-10) jumped above the 09-13 daily and
    # pushed the just-published weekly to the homepage's third slot. A weekly
    # sorts by its week END and wins the tie against that day's daily (it is
    # published after it and covers it); created_at only breaks remaining ties.
    def _order_key(p: dict[str, Any]) -> tuple[str, int, str]:
        sort_date = p["date_slug"]
        if p["note_type"] == "weekly_brief":
            range_dates = _DATE_RE.findall(p["date"])
            if range_dates:
                sort_date = range_dates[-1]
        return (sort_date, 1 if p["note_type"] == "weekly_brief" else 0, p["created_at"])

    deduped.sort(key=_order_key, reverse=True)
    return deduped
