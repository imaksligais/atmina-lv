...
33 canonical topic groups from ``src.topic_map.TOPIC_GROUPS`` from a mere
...
# Curated topic adjacency for the "Citas tēmas" column. Keys and values are
# canonical ``TOPIC_GROUPS`` names (asserted by
# ``tests/test_topics.py::test_related_topics_map_is_canonical``); a mapped
# topic that is EMPTY in the current data (absent from ``all_topics``) is
# skipped, and the historic "5 busiest topics" fill tops the list back up to
# five. Topics without an entry here keep the pure top-5 behaviour.
    ...
    *,
    ...
    """, (name,)).fetchall():
        ...
        latest_positions.append({
            ...
            "source_domain": _domain_label(
                r["doc_source_domain"], r["source_url"]),
    ...
    # Profile links carry ?tema=<kanoniskā tēma>#pozicijas — ppv1.js reads
    # the hash as the tab to open and the param as the topic filter; without
    # the hash the profile lands on Pārskats with the topic highlighted but
    # the wrong tab (2026-09-07). ``quote(..., safe="/")`` matches Jinja's
    # ``urlencode`` byte-for-byte, so the template links and these agree.
    tema_param = "?tema=" + quote(name, safe="/")
    profile_suffix = tema_param + "#pozicijas"
    columns: list[dict[str, Any]] = []
    if top_politicians:
        columns.append({
            "title": "Visvairāk pozīciju",
            "links": [
                {
                    "label": p["name"],
                    "href": f"../politiki/{p['slug']}.html{profile_suffix}",
                    "sub": f"{p['count']} poz.",
                }
    ...
    # "Cita tēma" — curated neighbours first (RELATED_TOPICS), then the
    # busiest-topic fill, capped at five. Both stages skip the current topic
    # and anything already picked; a curated name that is empty today simply
    # isn't in ``all_topics`` and drops out.
    by_name = {t["name"]: t for t in all_topics}
    other_topics: list[dict[str, Any]] = []
    for related in RELATED_TOPICS.get(name, []):
        t = by_name.get(related)
        if t is not None and t["name"] != name and t not in other_topics:
            other_topics.append(t)
    for t in all_topics:
        if len(other_topics) >= 5:
            break
        if t["name"] != name and t not in other_topics:
            other_topics.append(t)
    other_topics = other_topics[:5]
    if other_topics:
        columns.append({
            "title": "Citas tēmas",
            "links": [
    ...
    return {
        ...
        "slug": _slugify(name),
        "color": TOPIC_COLORS.get(name, "#8b8fa3"),
        # The directory's own number, reused verbatim (never recomputed with
        # different conditions) so the "Skatīt visas N pozīcijas" archive link
        # can never disagree with the temas.html card. ``latest_positions`` is
        # capped at 15, so this is the only honest total on the page.
        "position_count": by_name[name]["position_count"] if name in by_name else len(
            latest_positions
        ),
