...
def _fetch_tensions(db: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    rows = db.execute("""
        SELECT t.id, t.topic, t.description, t.tension_type, t.source_url, t.target_url,
               t.created_at,
               -- created_at is UTC (schema.sql); the page shows the LV day.
               date(t.created_at, 'localtime') AS lv_date,
    ...
    for r in rows:
        ...
        # pop, don't keep: this same dict list is serialized verbatim into the
        # saites page's inline JSON (src/render/links.py — saites_data
        # ["tensions"]), where a stray key changes the page bytes and trips
        # tests/test_render_chars.py::test_saites_index_byte_identical. The LV
        # day is a display concern of this page only.
        d["date"] = d.pop("lv_date") or (d["created_at"] or "")[:10]
        result.append(d)
    return result
