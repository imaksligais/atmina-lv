...
def retrieve_context(
    ...
    try:
        ...
        if query:
            ...
            # NB `db.search_similar()` nosauc šo parametru `politician_id`, ne
            # `opponent_id`. Līdz 2026-08-02 šeit bija `opponent_id=`, tāpēc KATRS
            # izsaukums ar `query=` meta TypeError — ceļš bija miris kopš 2026-04-09,
            # un to nepamanīja, jo neviens ražošanas kods to nesauc; vienīgais
            # patērētājs ir `wiki/operations/wiki-tools.md` dokumentētais piemērs.
            results = db_search_similar(query_vec, top_k=max_results, politician_id=opponent_id)
