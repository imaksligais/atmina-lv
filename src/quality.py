...
# (2026-09-05, plāna 4.3 — tabula te bija baitu-identiska kopija).
_STRIP_DIACRITICS = LV_TRANS
...
def validate_lv_diacritics(
    ...
    *,
    ...
    # Short text: the ratio is too coarse — at 67-99 letters ONE diacritic is
    # under 1.5%, and correct Latvian whose words take none got refused at a
    # write boundary (2026-09-23, an `analyses` row lost). Below
    # `short_letters` stripping is judged by its own signature, ZERO
    # diacritics. Measured 2026-09-24 on 32 765 real 40-99-letter sentences:
    # 0.09% sit in the 1-diacritic-under-floor zone; 100-149 letters: 0.03%.
    # Trade-off accepted: a partly stripped short text that kept one
    # diacritic now passes; T4 drift strips everything.
    if letters < short_letters:
        if diacrit > 0:
            return True, "ok (short text, has diacritics)"
...
QuoteVerdict = Literal["verified", "mismatch", "cannot_verify"]


@dataclass(frozen=True)
class QuoteCheck:
    """Three-state result of comparing a stored quote to its source document.

    ``verdict`` is exactly one of:
      - ``'verified'`` — the quote was located in the source and matches it
        (modulo capitalisation, a quoting convention).
      - ``'mismatch'`` — the quote was located after folding diacritics away
        but differs before folding. That difference can only be diacritics, so
        the stored text is provably not what the document says.
      - ``'cannot_verify'`` — the quote could not be located in the source at
        all, so the check did not run. This is NOT a pass.

    **Why this is not a ``(bool, str)``.** Until 2026-08-24 the function
    returned ``(True, "quote not located in source (cannot verify)")`` — "I
    could not check" reported as "it passed". On 2026-08-23 that quietly waved
    through two quotes (#703944 Švinka, #703945 Vīksna); they turned out to be
    correct, but only because ``@quality-reviewer`` re-checked them by hand,
    not because the gate said anything true. CLAUDE.md § Working Conventions
    names this class: a green result stops people looking.

    The write boundary still allows ``cannot_verify`` — see ``blocks``. What
    changed is that callers can now COUNT the third state instead of adding it
    to the passes. Any caller that reports a pass rate must report the
    ``cannot_verify`` denominator beside it, or the rate is a fact about the
    corpus's locatability, not about our fidelity.

    Deliberately not a tuple: the old shape unpacked as ``ok, reason``, and a
    2-tuple whose first element silently changed from ``bool`` to a
    (always-truthy) verdict string would turn every stale call site into a gate
    that cannot fail — exactly the defect being fixed. Unpacking this raises.
    """
    ...
    def blocks(self) -> bool:
        """True only for a PROVEN mismatch — the sole state that refuses a write.

        ``cannot_verify`` must not block: measured over the live DB
        (2026-08-03, 4735 checkable rows) 1408 quotes are legitimately not
        locatable character-exactly — English quotes, elisions marked
        ``(..)``, typographic quotes, and re-fetched bodies whose wording
        changed. Refusing those would block real claims at a write boundary,
        and refusing a quote means storing none, which silently drops
        provenance (claim #555664).
        """
    ...
    def cannot_verify(self) -> bool:
        return self.verdict == "cannot_verify"


def check_quote_against_source(
    quote: str | None, source: str | None
) -> QuoteCheck:
    """Check a VERBATIM quote against the source document. Returns a ``QuoteCheck``.

    This is the right question to ask of
    ...
    Reports ``mismatch`` for EXACTLY ONE condition: the quote is absent from
    the source but present once diacritics are folded away. That difference can
    only be diacritics, so the stored text is provably not what the document
    says — either we stripped a mark, or we "corrected" the speaker's own
    spelling, and CLAUDE.md forbids both. The reason carries the source
    wording, because the fix is always to restore it.

    A quote that is simply absent from the source is ``cannot_verify``, never
    ``verified``: the comparison did not run. It still does not block a write
    (see ``QuoteCheck.blocks``) — 1408 live quotes are legitimately unlocatable
    — but it must be counted apart from the passes.
    ...
    if not quote or not source:
        # Nothing to compare — not a pass either. A claim with no quote is
        # outside the checkable set, and a claim whose document body is gone
        # (re-scrape, paywall stub) is precisely the class `/audit-integrity`
        # check 17 exists for.
        return QuoteCheck("cannot_verify", "no quote or no source to compare against")

    restored = restore_text_from_source(quote, source)
    if restored is None:
        return QuoteCheck("cannot_verify", "quote not located in source")
    if restored == quote:
        return QuoteCheck("verified", "verbatim")
    if restored.lower() == quote.lower():
        return QuoteCheck("verified", "differs only in capitalisation")

    return QuoteCheck("mismatch", (
