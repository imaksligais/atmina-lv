...
from src.db import _float_list_to_bytes, get_db, lv_cutoff, now_lv
from src.embeddings import embed_text
from src.preflight import ensure_analysis_env
...
def get_pending_politicians(days: int = 1) -> list[dict]:
    ...
    # signatures with no first-party position). The exclusion still holds for
    # the announcement class, which is most of the corpus: of 2 149 vestnesis
    # documents (2026-04-30..09-04) the twelve largest title classes — auctions,
    # inheritance notices, UR entries, court summons — are ~800, and 11 claims
    # have EVER come out of the whole platform (measured 2026-09-06).
    #
    # What used to stand here was false and had a cost. It claimed the content
    # "reaches the brief's 'Šodien izsludināts' section via document_politicians
    # junction". That section lived inside `generate_telegram_brief()` and was
    # deleted with the Telegram flow on 2026-08-03 (`60c878f4`); it has never
    # appeared in a published brief. So the exclusion had no compensating
    # surface, and doc 95670 — «Ministru kabineta krīzes vadības sēdes
    # protokols» (2026-08-26), seven government deadlines on the day whose brief
    # lead was that exact topic — was never read by anyone.
    #
    # The compensating surface now exists and is a REPORT, not a queue entry:
    # `src.routine.vestnesis_acts_for()` lists the day's documents whose title
    # starts with «Ministru kabineta» / «Saeimas» / «Valsts prezidenta» (24 of
    # 2 149, i.e. 0–1 a day) to the orchestrator in `print_routine`, with the
    # day's vestnesis total as the denominator. `wiki/log-ingest` still records
    # that the ingest ran. Substantive cases are still processed manually —
    # `get_politician_documents` below takes an explicit pid and is the way in.
    # Relay news accounts are excluded from the QUEUE (2026-08-02). The
    # predicate itself lives in `src/scope.py` — it is ALSO the routine status
    # denominator and the wiki backlog figure, and typing it inline is what let
    # those three drift apart inside a single commit. Read that module before
    # changing the rule; it carries the measurements and the scope boundary.
    #
    # Deliberately NOT mirrored in get_politician_documents(): that takes an
    # explicit pid, so it stays the escape hatch for reviewing a relay slot on
    # purpose. Nothing becomes unreachable — it only stops being proposed.
    rows = db.execute(
        f"""SELECT tp.id, tp.name, tp.party, tp.role, tp.relationship_type,
    ...
    result: dict[int, dict] = {r["id"]: dict(r) for r in rows}

    # OTRĀ JOSLA (2026-08-04, junction inversijas plāns § 5): citētie
    # `mentioned` runātāji. Doks, kur pid ir tikai 'mentioned', bet detektors
    # (src/quoted_speaker.py) viņu atzīmē kā vienīgo citēto runātāju, pirmajā
    # joslā neienāk nekad — un `reviewed_at` to maskē uz mūžu, tiklīdz subject
    # slots doku apstrādājis. Josla filtrē pēc per-pāra `extracted_at IS NULL`
    # un `reviewed_at` APZINĀTI neskatās. Scope predikāts (relay/inactive
    # izslēgšana) tas pats, kas pirmajai joslai.
    lane_pairs = pending_quoted_mentioned(db, days=days)
    if lane_pairs:
        lane_counts: dict[int, int] = {}
        for p in lane_pairs:
            lane_counts[p["politician_id"]] = lane_counts.get(p["politician_id"], 0) + 1
        ph = ",".join("?" for _ in lane_counts)
        allowed = {
            r["id"]: r for r in db.execute(
                f"""SELECT tp.id, tp.name, tp.party, tp.role, tp.relationship_type,
                          (SELECT MAX(created_at) FROM analyses
                           WHERE opponent_id = tp.id) AS last_analyzed
                   FROM tracked_politicians tp
                   WHERE tp.id IN ({ph}) AND {queue_politician_sql("tp.id")}""",
                list(lane_counts),
            ).fetchall()
        }
        for lane_pid, cnt in lane_counts.items():
            if lane_pid not in allowed:
                continue
            if lane_pid in result:
                result[lane_pid]["doc_count"] += cnt
                result[lane_pid]["mentioned_doc_count"] = cnt
            else:
                entry = dict(allowed[lane_pid])
                entry["doc_count"] = cnt
                entry["mentioned_doc_count"] = cnt
                result[lane_pid] = entry

    db.close()
    return sorted(result.values(), key=lambda e: e["doc_count"], reverse=True)
...
def get_politician_documents(
    ...
    When the pending queue holds MORE than ``max_results`` docs, the cut is
    reported instead of silent: stderr gets a warning and every returned doc
    carries ``queue_total`` (the real queue size). The 2026-08-03 LB case cut
    the operator's #1-priority doc as the 21st of 21 with no signal at all.
    """
    db = get_db()
    cutoff = lv_cutoff(days)

    # platform='vestnesis' filtered out — see get_pending_politicians comment
    # for why (announcement class) and for the report that compensates it
    # (`src.routine.vestnesis_acts_for`). Note the asymmetry: this function is
    # PID-EXPLICIT, so the filter here only stops a vestnesis doc arriving as a
    # side effect of asking for a politician's day. A doc named by the routine
    # report is reached by doc id, not through this queue.
    queue_sql = """FROM documents d
    ...
    queue_total = db.execute(
        f"SELECT COUNT(*) {queue_sql}", (pid, cutoff)
    ).fetchone()[0]
    rows = db.execute(
        f"""SELECT d.id, d.content, d.source_url, d.source_domain, d.platform, d.language,
    ...
    # OTRĀ JOSLA (2026-08-04, junction inversijas plāns § 5): citētie
    # `mentioned` runātāji ar `extracted_at IS NULL`. `reviewed_at` šeit
    # APZINĀTI netiek skatīts — dokumenta-līmeņa zīmogs ir tieši tas, kas šo
    # klasi maskēja (sk. src/quoted_speaker.py). Subject joslas vaicājums
    # augstāk paliek neizmainīts.
    subject_ids = {d["id"] for d in docs}
    lane_pairs = pending_quoted_mentioned(db, days=days, pid=pid)
    lane_ids = [
        p["document_id"] for p in lane_pairs
        if p["document_id"] not in subject_ids
    ]
    lane_total = len(lane_ids)
    budget = max(0, max_results - len(docs))
    take = lane_ids[:budget]
    if take:
        ph = ",".join("?" for _ in take)
        lane_rows = db.execute(
            f"""SELECT d.id, d.content, d.source_url, d.source_domain,
                      d.platform, d.language, d.scraped_at, d.published_at,
                      d.word_count, d.is_auto_caption, 'mentioned' AS role
        ...
        for r in lane_rows:
            ...
            if content and len(content.split()) > 2000:
                ...
                d["truncated"] = True
                d["total_words"] = len(words)
            d["quoted_mentioned_lane"] = True
            docs.append(d)

    total_pending = queue_total + lane_total
    if total_pending > len(docs):
        print(
            f"WARNING get_politician_documents(pid={pid}): queue holds "
            f"{total_pending} docs, returning only {len(docs)} "
            f"(max_results={max_results}) — the cut docs stay pending; "
            f"re-run or raise max_results.",
            file=sys.stderr,
        )
        for d in docs:
            d["queue_total"] = total_pending
...
def get_existing_claims(
    ...
    ``stated_around`` (ISO date/datetime — the article's date) replaces the
    ``created_at`` window with ``stated_at`` within ±5 days of it; ``days`` is
    then ignored. This is the extractor's same-event duplicate check: a claim
    backfilled today about a May event has a fresh ``created_at`` but a May
    ``stated_at``, so the ``created_at`` window both misses same-event claims
    stored long ago and floods the list with unrelated recent ones.
    """
    db = get_db()
    ...
    sql = (
        ...
        "source_url, claim_type "
        "FROM claims WHERE opponent_id = ? "
    )
    params: list = [pid]
    if stated_around is not None:
        sql += "AND ABS(julianday(stated_at) - julianday(?)) <= 5"
        params.append(stated_around)
    else:
        sql += "AND created_at >= ?"
        params.append(lv_cutoff(days))
...
# Gandrīz-dublikāta slieksnis `claim_vectors` attālumam (sqlite-vec vec0
# noklusējums = L2 uz normalizētiem e5-small vektoriem — tas pats skaitlis, ko
# atdod `search_similar_claims`). Kalibrēts 2026-09-24 ar
# `scripts/calibrate_dup_threshold.py`:
#   pozitīvie (backfill 1. partijas dublikāti, 10 pāri; dzēstā puse iegulta no
#     rollback faila): 0.2590 0.2751 0.3131 0.3751 0.3868 0.3969 | 0.4046
#     0.4241 0.4308 0.4695;
#   negatīvie (40 pāri: 30 nejauši ar citu tēmu 0.4613–0.6138, 10 tās pašas
#     tēmas ar citu nostāju 0.3995–0.4495): tuvākais 0.3995;
#   θ = 0.398 → recall 6/10 = 60 %, FP 0/40 (10 pāri, ne neatkarīgi: #18230
#     ir divos pāros; vektori atjaunoti no rollback teksta). Rezerve līdz tuvākajam
#     negatīvajam ir tikai 0.0015 — tāpēc vārts ir TIKAI ziņojošs.
#   Robežgadījumi (θ izvēlē neiekļauti): #718086↔#718085 0.3680 (zem θ),
#     #718081↔#718013 0.4366 (virs θ).
#   Bāzes biežums: 38 no 726 claims kopš 2026-09-10 (5,2 %) ar kaimiņu ≤ θ;
#     roku lasījumā aptuveni 3 no 5 ir tas pats notikums vai tā pati nostāja.
DUP_DISTANCE_MAX: float = 0.398


def _nearest_position_claim(
    db,
    *,
    opponent_id: int,
    claim_id: int,
    ...
    """Tuvākais tā paša politiķa `position` claim ar `stated_at` ±5 d.

    Tas pats vec0 k-NN ceļš, ko `db.search_similar_claims` (filtrs iebīdīts
    `claim_id IN (subquery)`, tāpēc k=1 ir budžets tikai šī politiķa logā).
    Atgriež rindu (claim_id, distance, stated_at) vai None.
    """
    import sqlite_vec
    ...
    excluded = [claim_id, *exclude_ids]
    placeholders = ",".join("?" for _ in excluded)
    return db.execute(
        f"""SELECT cv.claim_id, cv.distance, c.stated_at
            FROM claim_vectors cv JOIN claims c ON c.id = cv.claim_id
            WHERE cv.embedding MATCH ? AND k = 1
              AND cv.claim_id IN (
                  SELECT id FROM claims
                  WHERE opponent_id = ? AND claim_type = 'position'
                    AND id NOT IN ({placeholders})
                    AND ABS(julianday(stated_at) - julianday(?)) <= 5)
            ORDER BY cv.distance""",
        (embedding_bytes, opponent_id, *excluded, stated_at),
    ).fetchone()
...
def save_analysis(
    ...
    Raises:
        RuntimeError: if the interpreter cannot embed claims. Deliberately the
            FIRST thing this function does, before any claim-count branch —
            the failure mode being closed is a zero-claim call quietly
            succeeding in an environment where a claim-bearing call could not.
            See src.preflight.ensure_analysis_env.
    """
    ensure_analysis_env()
    ...
    failures: list[dict] = []
    claim_ids: list[int] = []
    # claim_id -> index of the batch entry that first produced it, so a second
    # entry collapsing onto the same id can be reported (see silent_dedup).
    seen_claim_ids: dict[int, int] = {}
    ...
    try:
        ...
        # sufficient. Precomputed here so slow embedding work happens OUTSIDE
        # the held SQLite write transaction (see db.store_claim docstring).
        ...
        with db:
            # Robeža "jaunam" claim: store_claim ir idempotents, tāpēc
            # atkārtots izsaukums atdod esošu id — tam dublikātu pārbaudi
            # nedara (tas nav jauns ieraksts).
            pre_call_max_id = db.execute(
            ...
            if claims:
                ...
                for idx, c in enumerate(claims):
                    ...
                    if new_claim_id:
                        # Silent idempotency merge (trap T2, Data Contract #3).
                        # store_claim is idempotent on
                        # (opponent_id, source_url, topic) and is first-write-
                        # wins, so a document carrying several DISTINCT stances
                        # in one topic collapses them into the first claim: the
                        # second stance is discarded, store_claim reports
                        # success, and ``failures`` stays empty. The only trace
                        # was a repeated id inside ``claim_ids`` — which nobody
                        # reads. Observed repeatedly (06-17/06-18: Vītols,
                        # Krusts, Rokpelnis, Abu Meri, Tāvars).
                        #
                        # A repeat WITHIN this batch is the loss signal. An id
                        # that merely already existed before the call is normal
                        # idempotence (a re-run of the same day) and is not
                        # reported.
                        first_idx = seen_claim_ids.get(new_claim_id)
                        if first_idx is not None:
                            first = claims[first_idx]
                            failures.append({
                                "type": "silent_dedup",
                                "opponent_id": pid,
                                ...
                                "topic": c["topic"],
                                "source_url": source_url,
                                "kept_stance": first["stance"],
                                "dropped_stance": c["stance"],
                                "hint": (
                                    "Two distinct stances collapsed onto one "
                                    "claim via the (opponent_id, source_url, "
                                    "topic) key. Differentiate the topic or "
                                    "consolidate the stances deliberately, "
                                    "then verify stored-count == intended-count."
                                ),
                            })
                            print(
                                f"[save_analysis] silent_dedup pid={pid} "
                                f"claim_id={new_claim_id} topic={c['topic']!r}: "
                                f"stance {idx + 1} collapsed onto stance "
                                f"{first_idx + 1}",
                                file=sys.stderr,
                            )
                        else:
                            seen_claim_ids[new_claim_id] = idx

                        # Gandrīz-dublikāts (tikai ziņo, nekad nebloķē): tas
                        # pats politiķis, cits avots, tas pats notikums —
                        # idempotences atslēga to nenoķer (cits source_url).
                        # Kļūda šeit nedrīkst sabojāt transakciju: izdrukā un
                        # turpina.
                        if (
                            new_claim_id > pre_call_max_id
                            and first_idx is None
                            and c.get("claim_type", "position") == "position"
                        ):
                            try:
                                own = db.execute(
                                    "SELECT stated_at FROM claims WHERE id = ?",
                                    (new_claim_id,),
                                ).fetchone()
                                near = None
                                if own and own["stated_at"]:
                                    near = _nearest_position_claim(
                                        db,
                                        opponent_id=pid,
                                        claim_id=new_claim_id,
                                        stated_at=own["stated_at"],
                                        embedding_bytes=precomputed_embeddings[idx],
                                        # Šī paša izsaukuma iepriekšējie claims
                                        # tiek salīdzināti apzināti: dublikāti
                                        # var nākt arī no viena izsaukuma.
                                        exclude_ids=[],
                                    )
                                if near is not None and near["distance"] <= DUP_DISTANCE_MAX:
                                    failures.append({
                                        "type": "possible_duplicate",
                                        "opponent_id": pid,
                                        "claim_id": new_claim_id,
                                        "similar_claim_id": near["claim_id"],
                                        "distance": float(near["distance"]),
                                        "stated_at": own["stated_at"],
                                        "similar_stated_at": near["stated_at"],
                                        "hint": (
                                            "Stored, not blocked. Same politician, "
                                            "stated_at within ±5 days, near-identical "
                                            "stance vector. Read both: if it is the "
                                            "same statement from another source, "
                                            "delete one (paired rollback); if it is "
                                            "a distinct position, leave both."
                                        ),
                                    })
                                    print(
                                        f"[save_analysis] possible_duplicate "
                                        f"pid={pid} claim_id={new_claim_id} ~ "
                                        f"#{near['claim_id']} "
                                        f"distance={near['distance']:.4f}",
                                        file=sys.stderr,
                                    )
                            except Exception as dup_err:  # noqa: BLE001
                                print(
                                    f"[save_analysis] possible_duplicate check "
                                    f"skipped pid={pid} claim_id={new_claim_id}: "
                                    f"{dup_err}",
                                    file=sys.stderr,
                                )
            ...
            # Mark processed documents (inside the same transaction).
            #
            # Divi zīmogi ar atšķirīgu granularitāti (2026-08-04, junction
            # inversijas plāns):
            #  - `documents.reviewed_at` paliek DOKUMENTA līmenī un tiek likts
            #    TIKAI dokiem, kur analizētajam politiķim ir 'subject' junction
            #    rinda. Mentioned joslas apstrāde (citēts runātājs caur rindas
            #    otro joslu) doku zīmogot NEDRĪKST — citādi subject slots to
            #    pazaudētu, pirms vispār palaists: tā pati maskēšana, ko šis
            #    plāns noņem, tikai spoguļvirzienā. Blakusefekts: doks, kurā
            #    pid nav NEVIENAS junction rindas, reviewed_at vairs nesaņem —
            #    tas arī agrāk slēpa doku no CITU politiķu rindām.
            #  - `document_politicians.extracted_at` ir PĀRA līmenī un tiek
            #    likts katrai šī izsaukuma apstrādātajai (doks, pid) junction
            #    rindai — gan claims, gan empty iznākumā (nezīmogots tukšais
            #    doks mentioned joslā atgrieztos mūžīgi, T5).
            if reviewed_doc_ids:
                doc_list = list(reviewed_doc_ids)
                placeholders = ",".join("?" for _ in doc_list)
                subject_docs = [
                    r["document_id"] for r in db.execute(
                        f"SELECT DISTINCT document_id FROM document_politicians "
                        f"WHERE politician_id = ? AND role = 'subject' "
                        f"AND document_id IN ({placeholders})",
                        [pid, *doc_list],
                    ).fetchall()
                ]
                if subject_docs:
                    mark_documents_reviewed(subject_docs, db=db)
                db.execute(
                    f"UPDATE document_politicians SET extracted_at = ? "
                    f"WHERE politician_id = ? AND extracted_at IS NULL "
                    f"AND document_id IN ({placeholders})",
                    [now_lv(), pid, *doc_list],
                )
