...
from src.x_pool import TRANSACTION_KEY_ATTEMPTS, get_pool, reset_transaction_key
...
async def _probe_search_slot_health() -> int:
    ...
    ``SEARCH_PROBE_TIMEOUT_S``, after up to ``TRANSACTION_KEY_ATTEMPTS``
    transaction-key rebuilds per slot. Uninitialized and rate-limited slots
    count as unhealthy.
    """
    pool = await get_pool()
    healthy = 0
    for slot in range(pool.slot_count):
        ...
        if await _search_probe_with_key_repair(client, slot):
            healthy += 1
    return healthy


async def _search_probe_with_key_repair(client, slot: int) -> bool:
    """Probe SearchTimeline, rebuilding the transaction key between failures.

    Most failures here are a bad transaction key rather than a degraded slot
    (see `src.x_pool.reset_transaction_key`), and a rebuild recovers the client
    within a few tries. Counting such a slot as unhealthy used to drop the pool
    below SEARCH_MIN_HEALTHY_SLOTS and fall the whole run back to the timeline
    strategy for a reason that had nothing to do with credentials.
    """
    for attempt in range(1, TRANSACTION_KEY_ATTEMPTS + 1):
        try:
            await asyncio.wait_for(
                client.search_tweet(SEARCH_PROBE_QUERY, "Latest", count=1),
                timeout=SEARCH_PROBE_TIMEOUT_S,
            )
            if attempt > 1:
                logger.info(
                    "_probe_search_slot_health: slot %d recovered after %d "
                    "transaction-key rebuild(s)", slot, attempt - 1,
                )
            return True
        except Exception as e:
            logger.debug(
...
async def fetch_mentions(
    ...
    *,
    ...
    if resolved == "search":
        ...
        # of 2.json (slot 2's account) — the same account in two slots, zero
