...
def link_audit_row(db, *, brief_image_id: int, prompt: str) -> int | None:
    """Stamp the newest UNLINKED ``image_audit`` row for ``prompt`` with the
    ``brief_images`` id it produced; return that row id, or None if nothing
    matched.

    Why this exists (2026-09-15): ``generate_image()`` writes its audit row
    before the caller knows the ``brief_images`` id, so the CLI path left
    ``source_id`` NULL — and the ``apply_migrations()`` backfill, whose
    idempotency guard is ``NOT EXISTS (source_table='brief_images' AND
    source_id=bi.id)``, could not see the row and inserted a second one.
    13 pairs in 2026-09 (0,507 USD counted twice in the month budget).
    Linking here closes the loop: the unique partial index on
    ``(source_table, source_id)`` then refuses the duplicate by construction.

    Match by prompt, newest first — the CLI writes exactly one audit row per
    generation and links it in the same process, so the newest unlinked row
    with this prompt is the one just produced. Already-linked rows are never
    restamped.
    """
    row = db.execute(
        "SELECT id FROM image_audit WHERE source_table IS NULL AND prompt = ? "
    ...
    if row is None:
        return None
    db.execute(
        "UPDATE image_audit SET source_table = 'brief_images', source_id = ? "
        "WHERE id = ? AND source_table IS NULL",
        (brief_image_id, row[0]),
    )
    db.commit()
    return int(row[0])
