...
- Negative constraints (no people, logos, party symbols, foreign flags; LV + EU
  flags are allowed as elements of the setting)
...
# Gaišais dvīnis SEPIA_STYLE-am: gaiša krēmpapīra fona ilustrācija ar retām
# tuša kontūrlīnijām un daudz tukša papīra — NE tumšais gravīras sepia. Formula
# nolasīta no 2026-09-06 airBaltic 2. sintēzes gaišā hero prompta
# (docs/tweet_bank/2026-09-06-sinteze-airbaltic-2-hero-light-prompt.py), kas
# līdz šim dzīvoja tikai vienreizējā skriptā, jo `cli thread` sepia pievienoja
# bez nosacījuma (verdikts 47b, 2026-09-07).
LIGHT_STYLE: str = (
    "Light, airy editorial illustration on a pale cream / ivory paper "
    "background, almost-white warm paper with a barely visible warm corner "
    "vignette. Sparse clean line drawing: confident thin dark-ink contours of "
    "varying weight with soft graphite pencil hatching used only for volume, "
    "and a great deal of untouched paper. No dense crosshatching, no engraved "
    "texture fields, no heavy blacks, no dark background. One single warm "
    "accent colour (muted amber / ochre), used very sparingly. Generous "
    "negative space, 16:9 composition, no text, no lettering, no numbers, "
    "no words, no captions, no logos."
)

# Pavediena / ilustrāciju stili (VIENMĒR bez teksta), atšķirībā no
# STYLE_VARIANTS, kas ir plakāta stili AR renderētu virsrakstu.
THREAD_STYLES: dict[str, str] = {
    "sepia": SEPIA_STYLE,
    "light": LIGHT_STYLE,
}
...
# Subject exclusions shared by BOTH constraint variants below, so the two lists
# cannot drift apart again. Until 2026-09-13 both carried a blanket "national
# flags" ban, which contradicted the agent prompt's Latvian-setting rule
# (.claude/agents/graphics-designer.md § «Vide attēlos ir LATVISKA», operator
# decision 2026-08-21: the Saeima hall is described WITH "Latvijas + ES
# karogi"); image #324 with both flags was approved that day. The flag rule is
# therefore explicit: Latvia + EU as setting elements yes, anything else no.
_SUBJECT_EXCLUSIONS: str = (
    "Do NOT include: people, faces, hands, party logos, photorealistic elements, "
    "cartoon style, decorative borders, watermarks, political party symbols, or any "
    "recognizable real-world individuals. "
    "FLAGS: the flags of Latvia and of the European Union MAY appear as elements of "
    "the setting (behind a rostrum, on a wall, on a desk stand) — no other national "
    "flag, no party symbol and no logo of any kind. "
)
...
# scripts/oneoff-content/regen_172_v2.py and
# scripts/oneoff-content/twitter_pack_designer.py — changing its contents would
# silently alter those posters too. The subject list itself IS shared
# (_SUBJECT_EXCLUSIONS) — only the text rule differs.
TEXT_FREE_CONSTRAINTS: str = (
    ...
    "signatures, no watermark lettering. NO AUTHORSHIP MARK of any kind: no artist "
    "signature, no autograph, no cursive monogram, no initials, no printed credit "
    "line, no plate-mark inscription — margins and corners stay blank. The "
    "composition must carry its meaning through imagery alone. Ignore any "
    "typography instruction in the style description above — this composition "
    "has no typographic element."
)
