...
def generate_image(
    ...
    *,
    ...
    """Call Gemini image API with retry logic. Returns image bytes — JPEG in
    practice. Brief masters are nevertheless stored under a ``.png`` name by
    design (BACKLOG § Ne-darīt, 2026-09-23) — readers sniff content, not suffix.
...
def _extract_image_bytes(response) -> bytes:
    """Pull image bytes (JPEG in practice) from the first inline_data part. Raises SafetyError otherwise."""
