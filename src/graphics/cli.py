...
from src.graphics.prompt import DEFAULT_STYLE, THREAD_STYLES
...
def build_parser() -> argparse.ArgumentParser:
    ...
    b.add_argument(
        ...
        help="Text-free poster: omit the rendered headline, metaphor carries it alone",
    )
...
def _run_brief(args) -> None:
    ...
    prompt_text = build_prompt(
        visual_brief, vm, style_key=style_key, no_text=getattr(args, "no_text", False)
    )
    budget_check(db)
    key = load_gemini_key()
    try:
