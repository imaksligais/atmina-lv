...
def _short_party(party: str | None) -> str:
    """Party short-code for a bar label; fallback truncates the raw name.

    Until 2026-09-05 this held its own 8-entry map (``Stabilitātei!`` → ``S!``,
    against the site's ``ST``); resolution now goes through
    ``parties.short_name`` via ``src.coalition``.
    """
    if not party:
        return "—"
    return lookup_party_short_name(party) or party[:5]
