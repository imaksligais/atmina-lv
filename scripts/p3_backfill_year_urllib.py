...
# Frakcijas etiķetes kanonizācija — VIENA karte visiem ingest ceļiem.
# Šis parseris lasa JS `voteFullListByNames`, ne HTML tabulu, tāpēc tas
# NEIET caur parse_vote_snapshot(); līdz 2026-09-04 tam nebija nekādas
# normalizācijas, un tas atkārtoti ielika `ST!` rindas, ko 2026-08-04
# migrācija jau bija iztīrījusi (7 448 rindas / 975 balsojumi).
from src.saeima.votes import normalize_faction  # noqa: E402
...
# Agenda vote links appear in FOUR layouts (union all — checking only one is
...
#  4. LIVE agenda (`DK?ReadForm&actual=1`, `IsActual="1"`) — since 2026-09-10
#     it renders NO vote link at all: patterns 1–3 each returned 0 on the
#     2026-09-10 sitting, which really had 16 votes, and the day read as a
#     formal 2.B STOP. The vote GUID sits in the **21st argument** of a
#     `drawDKP_*(...)` call; the URL is built exactly like pattern 3.
#     Same freshness caveat as pattern 3 — a live day must be ingested
#     same-day. (In the helper since 2026-09-16; before that it lived only in
#     `.claude/agents/saeima-tracker.md` Step 2.B.)
...
# Pattern 4. Same call shape as `src/saeima/parsing.py::_DRAW_DKP_RE` (that one
# reads the 9th argument for the outcome label; this one reads the vote GUID) —
# keep both in step if titania changes the call form.
#
# DELIBERATELY WIDER than the prompt's `drawDKP_Pr`: registration items are
# drawn by `drawDKP_UT(...)`, and 2 of the 16 votes on 2026-09-10 were
# attendance registrations. A narrower class would drop them silently — the T8
# class. Over-inclusion costs one visible `SKIP … empty data` line per false
# candidate (76 candidates → 16 real votes on 2026-09-10); omission costs a vote.
_DRAW_DKP_PR_RE = re.compile(r"drawDKP\w*\((.*?)\);", re.DOTALL)
_DRAW_DKP_ARG_RE = re.compile(r'"(.*?)"', re.DOTALL)
# Two live arg layouts measured on real pages:
#  - 21 args (the 2026-09-10 form): vote GUID at [20].
#  - 34 args (since 2026-09-17, ALL 71 calls on that sitting): [5] `unid` is
#    the vote GUID (== [21] `DKPid` on the live page), [20] is `urgentDep`
#    (empty — reading the GUID there is what returned 0/37 on 2026-09-17),
#    [28] `voteType` gates whether a vote exists ('0#0#0' on all 32 voted
#    items, '' on the rest — incl. items carrying a "Nod. kom." label but no
#    vote), [29] `hasTechChilds` marks sub-item votes (fetched via a separate
#    getTechDKP agent call — NOT followed by this helper).
#    NB: on the `nr={uuid}` form the same 34-arg call has the 32-hex DKP item
#    UNID at [5] instead — it fails the GUID check below, so pattern 4 adds
#    nothing there. That is correct: nr= renders the same votes via
#    addVotesLink, and emitting parentID= for them would double-load one vote
#    under two URL forms.
_DRAW_DKP_VOTE_GUID_ARG = 20  # 21st argument, 0-based (21-arg live form)
_DRAW_DKP_UNID_ARG = 5        # 34-arg live form: vote GUID
_DRAW_DKP_VOTETYPE_ARG = 28   # 34-arg live form: non-empty ⇔ vote exists
# The parentID form is a 36-char dashed GUID — same class as
# `_VOTING_READFORM_RE`, because pattern 4 builds the SAME URL. A bare 32-hex
# UNID is NOT accepted here: that id belongs to the `/0/{HEX}?OpenDocument`
# form, and feeding it to `parentID=` would manufacture a URL titania never
# serves.
_DKP_VOTE_GUID_RE = re.compile(r"^[A-Fa-f0-9-]{36}$")
...
def _extract_vote_urls_from_agenda(agenda_html: str) -> list[str]:
    """Extract unique vote URLs from all four agenda layouts (union, deduped).

    Patterns 1–2 yield HEX32 ids → built as `/0/{HEX}?OpenDocument`; patterns
    3–4 yield a GUID → built as `/Voting?ReadForm&parentID={GUID}` (the `./` in
    the agenda anchor resolves against the .nsf base). Preserves first-on-page
    order within each pattern, patterns concatenated 1→2→3→4.

    Pattern 4 (2026-09-16) reads the vote GUID out of each `drawDKP_*(...)`
    call, which is the ONLY place a LIVE (`IsActual="1"`) agenda carries the
    vote id — it renders no vote link at all. Two arg layouts exist (see the
    constants above): the 34-arg form is detected by the presence of the
    `voteType` position and is gated on it being non-empty, then the GUID is
    read from `unid` [5]; shorter calls keep the legacy 21st-arg read. A call
    whose vote position is absent, empty or non-GUID is skipped silently:
    most agenda items simply have no vote.
    """
    seen: set[str] = set()
    out: list[str] = []
    ...
    for call in _DRAW_DKP_PR_RE.finditer(agenda_html):
        args = _DRAW_DKP_ARG_RE.findall(call.group(1))
        if len(args) > _DRAW_DKP_VOTETYPE_ARG:
            # 34-arg form: voteType decides whether the item was voted at all
            # (the GUID sits at [5] even on items never voted on).
            if not args[_DRAW_DKP_VOTETYPE_ARG].strip():
                continue
            guid = args[_DRAW_DKP_UNID_ARG].strip()
        elif len(args) > _DRAW_DKP_VOTE_GUID_ARG:
            guid = args[_DRAW_DKP_VOTE_GUID_ARG].strip()
        else:
            continue
        if not _DKP_VOTE_GUID_RE.match(guid):
            continue
        _add(f"{SAEIMA_BASE}/Voting?ReadForm&parentID={guid}")
    return out
...
def _decode_entry(entry: str) -> tuple[str, str | None, str]:
    ...
    faction = normalize_faction(parts[2])
    # 2026-07-28 — vote value carries HTML entities in the raw page
    # ("Re&#291;istr&#275;jies"): the deputies block is parsed from RAW html,
    # and only the name went through URL-decoding. Par/Pret/Atturas/Nebalsoja
    # are ASCII so the gap was invisible until registration events (~31k rows,
    # backfilled 2026-07-28). ingest_saeima_missing_votes.py reuses this
    # parser, so the 2025 bulk load inherits the fix.
    vote = html_mod.unescape(parts[3].strip())
    name = urllib.parse.unquote(name_enc.replace("+", " "))
    return (name, faction, vote)
...
def _to_vote_result(parsed: dict, vote_url: str) -> VoteResult:
    ...
    vote = VoteResult(
        ...
        # `_RESULT_RE` lifts the red span verbatim = source label; the fallback
        # below is our arithmetic (result_source, 2026-08-18). On this urllib
        # path the span is empty in practice (titania writes it with JS), so
        # rows from here are 'computed' or NULL — never a source quote.
        result_source="agenda_label" if parsed["result"] else None,
        url=vote_url,
        individual_votes=individual_votes,
    )

    # Present = cast ballots only; `total_nebalso` is NOT in the denominator —
    # see scripts/audit_saeima_vote_results.py::compute_expected_result for the
    # evidence (2026-08-17). And a procedural row where nobody cast a ballot
    # (attendance registration, quorum check) has no result to compute: leave it
    # empty rather than fabricating "Noraidīts".
    if not vote.result:
...
def process_session(session: dict, year: int, log_file: Path) -> dict:
    ...
    if not uuid:
        # Dzīvā (`active=1`) manifesta rinda — 2026-09-16. UUID tai nav, tāpēc
        # `nr={uuid}` uzbūvētu `nr=None` un nolasītos kā tukša darba kārtība.
        # Dzīvu dienu ielādē @saeima-tracker tajā pašā dienā, ne šis backfill.
        _log(log_file, f"  SKIP dzīvā sēde {date_str} ({session_type}) — nav nr={{UUID}}")
        return {"skipped_live": True}
...
def main() -> int:
    ...
    # Sk. p3_backfill_year.py — balsojumi commit-ojas pirms claims, tāpēc
    # salauzts embedding steks atstāj balsojumus bez pozīcijām.
    ensure_embeddings_live()

    init_saeima_tables(DB_PATH)
    log_file = REPO_ROOT / "data" / f"p3_backfill_{args.year}.log"

    manifest, _generated_at = load_manifest(MANIFEST_PATH)
