...
# reads the 9th argument for the outcome label; this one reads the 21st for the
# vote GUID) — keep both in step if titania changes the call form.
#
...
_DRAW_DKP_VOTE_GUID_ARG = 20  # 21st argument, 0-based
...
def _extract_vote_urls_from_agenda(agenda_html: str) -> list[str]:
    ...
    Pattern 4 (2026-09-16) reads the 21st argument of each `drawDKP_*(...)`
    call, which is the ONLY place a LIVE (`IsActual="1"`) agenda carries the
    vote id — it renders no vote link at all. An absent, empty or non-GUID 21st
    argument is skipped silently: most agenda items simply have no vote.
    """
    seen: set[str] = set()
    out: list[str] = []
    ...
    for call in _DRAW_DKP_PR_RE.finditer(agenda_html):
        ...
        if len(args) <= _DRAW_DKP_VOTE_GUID_ARG:
            continue
