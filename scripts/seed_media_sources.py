...
MEDIA_SOURCES = [
    {
        ...
        "x_handle": "ltvzinas",
        "feed_type": "relay",
        # 2026-06-09 fikss pārcēla institūciju kontus uz 'organization'
        # (data/fix_media_feeds_organization_2026-06-09.sql) — personas paliek
        # 'journalist'. Bez šī lauka atkārtota seed palaišana lēmumu atceltu
        # (notika 2026-08-19, atgriezts tajā pašā dienā).
    ...
    {
        "name": "Katrīna Iļjinska",
        "x_handle": "katrinailjinska",
        "feed_type": "relay",
        "notes": "Inc. Baltics galvenā redaktore un izdevēja; iepriekš BNS, "
                 "\"Dienas Bizness\" un \"Forbes Latvija\". Raksta par publiskajiem "
                 "iepirkumiem, valsts pārvaldi un IT projektu tēriņiem. Uzmanību "
                 "ekstrakcijā: 2014–2019 vadīja partijas \"Saskaņa\" sabiedrisko "
                 "komunikāciju, 2024. gada septembrī–decembrī bija ekonomikas "
                 "ministra V. Valaiņa padomniece — viņas pašas vērtējumi ir "
                 "ieinteresētas puses komentārs, nevis neitrāla mediju ziņa; "
                 "feed_type='relay' nodrošina, ka citāti tiek attiecināti uz "
                 "citētajiem politiķiem, ne uz viņu. Konts apstiprināts Apollo.lv "
                 "2024-12-28 publikācijā (pievienots 2026-08-19).",
    },
]
...
def main() -> None:
    ...
    with db:
        for m in MEDIA_SOURCES:
            ...
            rel_type = m.get("relationship_type", "journalist")
            ...
            if existing:
                pid = existing["id"]
                if existing["relationship_type"] != rel_type:
                    db.execute(
                        "UPDATE tracked_politicians SET relationship_type = ? WHERE id = ?",
                        (rel_type, pid),
                    )
                    print(f"updated {m['name']} -> relationship_type={rel_type} (id={pid})")
                else:
                    print(f"skip {m['name']} — already {rel_type} (id={pid})")
            else:
                db.execute(
                    "INSERT INTO tracked_politicians (name, relationship_type, x_handle, notes, created_at) "
                    "VALUES (?, ?, ?, ?, ?)",
                    (m["name"], rel_type, m["x_handle"], m["notes"], now_lv()),
                )
