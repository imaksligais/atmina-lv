...
def main():
    ...
    # Generate
    try:
        # kind="synthesis" (2026-09-07, verdikts 47b) — audita rinda image_audit.
        png_bytes = generate_image(PROMPT, aspect_ratio="16:9", kind="synthesis")
    ...
    # Site-relative path (deploy root = output/atmina/), same convention as
    # briefs images — see src/schema.sql brief_images.image_path comment.
    image_id = save_image_row(
        db,
        note_id,
        image_path=f"images/synthesis/{SYNTHESIS_SLUG}.png",
        prompt=PROMPT,
        model=key["model"],
        seed=None,
        width=1792,
