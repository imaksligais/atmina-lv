...
def find_mismatched_rows(db, limit: int | None = None) -> tuple[list[dict[str, Any]], int]:
    """Return (mismatches, scanned) — mismatches is the flag list, scanned is the
    total number of role='subject' junction rows examined (the denominator)."""
...
def main():
    ...
    db = get_db()
    mismatches, scanned = find_mismatched_rows(db, limit=args.limit)
    ...
    sys.stderr.write(f"\n{len(mismatches)} mismatches found of {scanned} subject rows checked.\n")
