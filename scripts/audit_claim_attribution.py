...
def find_suspect_claims(db) -> tuple[list[dict[str, Any]], int]:
    """Return (suspects, scanned) — suspects is the flag list, scanned is the
    total number of claim rows examined (the denominator)."""
    ...
    return suspects, len(rows)


def main():
    db = get_db()
    suspects, scanned = find_suspect_claims(db)
    ...
    sys.stderr.write(
        f"\n{len(suspects)} suspect claims ({n_mismatch} mismatches, {n_unverifiable} unverifiable) of {scanned} scanned.\n"
    )
    db.close()
