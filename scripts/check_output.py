...
def check_publish_gate(allow: list[str], hits: dict[str, int] | None = None) -> list[str]:
    ...
    problems: list[str] = []
    n_pages = 0          # saucējs = TIKAI pārskata lapas, ne viss blog/*.html
    n_ok = 0
    n_no_approval = 0
    n_no_image = 0
    n_image_unchecked = 0
    for page in pages:
        ...
        # Abus vārtus IZVĒRTĒ neatkarīgi, blokē pēc prioritātes (2026-08-24).
        # Līdz tam šis bija `elif` ķēde: ja attēla vārti nostrādāja pirmie,
        # lapa nekad nesasniedza apstiprinājuma zaru un `n_no_approval` palika
        # 0 — 2026-08-23 palaidiens izdrukāja „0 bez publicēšanas
        # apstiprinājuma", kaut `publish_approvals` rindas nebija vispār.
        # Bloķēšana bija pareiza, ziņojums ne, un tieši šos skaitļus lasa
        # nākamā sesija (CLAUDE.md § Working Conventions — saucējs, kas nevar
        # nokrist). Apstiprinājuma vārti ir izvērtējami VIENMĒR (atslēga ir
        # lapas slugs, ne note_id); attēla vārti orfānam nav izvērtējami, un
        # tas ir „nepārbaudīts", nevis „izturēja".
        missing_approval = key not in approvals
        if missing_approval:
            n_no_approval += 1
        if note_id is None:
            n_image_unchecked += 1
        ...
        if _suppressed(allow, hits, p, rel):
            continue
        problems.append(p)
    print(f"    publish-gate: {n_pages} pārskata lapas (no {len(pages)} blog/*.html), "
          f"{n_ok} apstiprinātas, "
          f"{n_no_approval} bez publicēšanas apstiprinājuma, "
          f"{n_no_image} bez approved=1 attēla, "
          f"{n_image_unchecked} attēla vārti nepārbaudīti (orfāni), "
