...
def main() -> int:
    ...
    # Formas vajadzīgas marķiera godīgumam: ja nesakritušais vārds JAU IR pid
    # name_forms sarakstā, "add ASCII form" būtu maldinošs ieteikums — īstais
    # cēlonis tad ir veto/proximity loģika, ne trūkstoša forma (Ivanova
    # gadījums, 2026-08-04: forma bija, ieteikums noveda pie liekas izmeklēšanas).
    forms_by_pid: dict[int, set] = {}
    for p in politicians:
        try:
            forms_by_pid[p["id"]] = set(json.loads(p["name_forms"] or "[]"))
        except (ValueError, TypeError):
            forms_by_pid[p["id"]] = set()
    db.close()
    ...
    for name, candidates in unmatched:
        ...
        for pid, real_name, score in candidates:
            marker = ""
            if score >= 0.98:
                if name in forms_by_pid.get(pid, set()):
                    marker = ("  ⟵ forma JAU eksistē name_forms — veto/proximity "
                              "miss, NE datu robs (sk. BACKLOG § Ne-darīt)")
                else:
