...
def main() -> int:
    ...
    for inv in result["inversions"][:60]:
        print(
            f"  doc {inv['document_id']:<7} [{inv.get('platform', '?'):<9}] runā(mentioned): "
    ...
    # Sadalījums pa platformām, jo raža nav viendabīga: tvītu joslā satīras
    # konti dominē viltus pozitīvos (src/quoted_speaker.py docstring, mērīts
    # 2026-08-27). Kopskaitlis bez sadalījuma slēpj, cik daudz no atradumiem
    # nāk no joslas, kuru jālasa piesardzīgāk.
    by_platform: dict[str, int] = {}
    for inv in result["inversions"]:
        key = inv.get("platform") or "?"
        by_platform[key] = by_platform.get(key, 0) + 1
    if by_platform:
        parts = ", ".join(f"{k}={v}" for k, v in sorted(by_platform.items()))
        print(f"pa platformām: {parts}")
        print("NB: tvītu joslā gaidi satīras viltus pozitīvus — šķirotājs ir @claim-extractor.")
    if n == 0:
