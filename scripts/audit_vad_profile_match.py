...
def _strip_md(s: str) -> str:
    ...
    s = s.strip()
    s = re.sub(r"\*\*", "", s)
    s = re.sub(r"\[([^\]]+)\]\([^)]*\)", r"\1", s)  # [Vards](../politiki/x.html) -> Vards (saites kopš 2026-09-20)
    s = re.sub(r"[¹²³⁴⁵]", "", s)  # zemsvītras marķieri (¹ Kalniete/Vaidere, ² Treija, ³ Eglīts …)
    return s.strip()
...
class IncomeExpected:
    politician: str
    income_2024_eur: int  # nosaukums vēsturisks — lauks nes § 2 gada (INCOME_YEAR) summu


@dataclass
class YoYExpected:
    politician: str
    eur_2023: int  # nosaukums vēsturisks — lauks nes YOY_YEARS[0] summu
    eur_2024: int  # … un YOY_YEARS[1]


# Gadi tiek nolasīti no analīzes teksta (2026-09-20): § 2 virsraksts
# «… deklarētāji YYYY. gadā», § 4 kolonnas «YYYY EUR». Līdz tam 2023/2024
# bija iekodēti, un lapas pāreja uz 2025 vārtus padarītu aklus.
INCOME_YEAR = "2024"
YOY_YEARS: tuple[str, str] = ("2023", "2024")
...
def parse_analize() -> tuple[list[IncomeExpected], list[YoYExpected],
    ...
    m = re.search(r"deklarētāji\s+(\d{4})\.\s*gadā", sec2)
    if m:
        INCOME_YEAR = m.group(1)
    income = [
    ...
    yoy_rows = _parse_md_table(sec4)
    if yoy_rows:
        year_cols = [h for h in yoy_rows[0] if re.fullmatch(r"\d{4} EUR", h)]
        if len(year_cols) == 2:
            YOY_YEARS = (year_cols[0][:4], year_cols[1][:4])
    yoy = [
        YoYExpected(_strip_md(r["Politiķis"]),
                    _parse_int(r[f"{YOY_YEARS[0]} EUR"]),
                    _parse_int(r[f"{YOY_YEARS[1]} EUR"]))
        for r in yoy_rows
    ]
...
class DeclSnapshot:
    ...
    income_rows: list[tuple[str, float, str, str]] = None  # (type, amount, currency, source)
        ...
        for details in decl_div.select("details.vad-section"):
            ...
            if label.startswith("Nekustamie īpašumi"):
                ...
                for tr in details.select("tr"):
                    ...
                    rm = re.match(r"^(.+?):\s*([\d.,]+)\s*([A-Z]{3})\b(?:\s*no\s+(.*))?",
                                  cell_text)
                    if not rm:
                        continue
                    ...
                    cur = rm.group(3)
                    source = (rm.group(4) or "").strip().upper()
                    ...
                    snap.income_rows.append((inc_type, amt, cur, source))
                    if cur == "EUR":
    ...
    for entry in income:
        ...
        if prof is None:
            mismatches.append(Mismatch(
                "income", entry.politician, f"{INCOME_YEAR} EUR total",
        ...
        relevant = [s for s in snaps
                    if s.year_label == INCOME_YEAR and s.kind == "annual"]
        # Profile stores duplicate decls (parallel amati); dedup by
        # (income_type, amount, currency, source) WITHIN a single
        # politician by combining all rows then deduping — tā pati atslēga,
        # ko lapa apraksta § 3 un ko rēķina scripts/vad_analysis_numbers.py
        # (līdz 2026-09-20 avots atslēgā nebija: divi vienādi procentu
        # maksājumi no divām bankām saplūda vienā).
        seen: set[tuple] = set()
        actual_total = 0.0
        for s in relevant:
            for inc_type, amt, cur, source in s.income_rows:
                if cur != "EUR":
                    continue
                key = (inc_type, round(amt, 2), cur, source)
        ...
        if not ok:
            mismatches.append(Mismatch(
                ...
                f"{len(relevant)} annual {INCOME_YEAR} decl(s) in profile",
            ))
    return mismatches
...
def audit_yoy(yoy: list[YoYExpected], verbose: bool = False) -> list[Mismatch]:
    ...
    for entry in yoy:
        ...
        if prof is None:
            ...
            continue
        snaps = parse_profile(prof)
        for year, expected in ((YOY_YEARS[0], entry.eur_2023),
                                (YOY_YEARS[1], entry.eur_2024)):
            relevant = [s for s in snaps
...
def main() -> int:
    ...
    print(f"=== § 2: {INCOME_YEAR} ienākums (top 15) ===")
    all_mismatches.extend(audit_income(income, verbose=args.verbose))
    print()

    print(f"=== § 4: {YOY_YEARS[0]} → {YOY_YEARS[1]} YoY (top 10) ===")
