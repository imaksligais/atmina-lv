...
def run_audit(
    ...
    if ids:
        ph = ",".join("?" * len(ids))
        # `saeima_vote` rindas izslēdz no mērījuma (verdikts 2026-08-21,
        # eskalācija 8): to vektorus neaiztiek — ne re-embedo, ne veido —
        # tāpēc "stale" tām nav rīcības, un kandidātu sarakstā tās tikai
        # barotu rindas, ko `reembed_claims.py` nedrīkst pārrakstīt
        # (2026-09-07: 150 no 171 stale bija balsojumi). Testu stand-in
        # tabulai `claim_type` kolonnas var nebūt — tad neizslēdz neko.
        cols = {r[1] for r in db.execute("PRAGMA table_info(claims)").fetchall()}
        rows = db.execute(
            f"SELECT id, {'claim_type' if 'claim_type' in cols else 'NULL'} "
            f"FROM claims WHERE id IN ({ph})",
            ids,
        ).fetchall()
        alive = sorted(r[0] for r in rows if r[1] != "saeima_vote")
        vote_excluded = sum(1 for r in rows if r[1] == "saeima_vote")
    picked = alive
...
def main() -> int:
    ...
    print(
        ...
        f"saeima_vote_izslēgti={rep['vote_excluded']}"
    )
    print(
