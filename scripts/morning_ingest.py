...
# TypeSafe matcher veto: ingesta noklusējums ir ēnas režīms (operatora lēmums 2026-09-19).
# Per-process: backfill skripti un testi (conftest.py piesprauž "off") to neredz.
# Vides mainīgais, ja uzstādīts, pārspēj — `ATMINA_TYPESAFE_VETO=off|enforce`.
# Jāstāv PIRMS pirmā `from src…` — src/matcher.py mainīgo nolasa importa brīdī
# moduļa līmenī (arī importi s1()…s5() iekšpusē ir pēc šīs rindas).
# Gate: tests/test_morning_ingest_veto_default.py.
os.environ.setdefault("ATMINA_TYPESAFE_VETO", "shadow")
...
def main() -> int:
    ...
    """
    # Žurnālam saucējs: kurā veto režīmā šis ingests strādāja (shadow / off / enforce).
    print(f"=== TypeSafe veto mode: {os.environ['ATMINA_TYPESAFE_VETO']} ===", flush=True)
