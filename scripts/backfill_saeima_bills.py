...
def backfill(db_path: str = DB_PATH, dry_run: bool = False) -> dict:
    ...
    for doc_nr, vote_list in grouped.items():
        ...
        for v in vote_list:
            ...
            # Passing vote_id= is what binds the vote to the bill:
            # append_bill_stage sets saeima_votes.bill_id inside the SAME
            # transaction (src/saeima/bills.py:380-384) and its own comment
            # declares it the sole writer to that column. A raw
            # "UPDATE saeima_votes SET bill_id" used to follow this call —
            # removed 2026-08-15 as both redundant (same value, already
            # committed) and a direct breach of CLAUDE.md inv #12.
            append_bill_stage(
