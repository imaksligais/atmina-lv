...
# ---------------------------------------------------------------------------
def step_copy(*, allow_overwrite: bool = False) -> None:
    # Guard added 2026-08-22 (audits/2026-08-22-slop-un-bloat-audits.md, kāpnes #1).
    # This migration completed once, on 2026-04-06. The file is kept as documented
    # history — but `shutil.copy2` below is a WHOLE-FILE overwrite of the live
    # database, and both paths are still live on the maintainer machine (a 32 MB
    # 2026-04 politracker.db and a multi-GB atmina.db), so the copy SUCCEEDS rather
    # than erroring out. It is the only whole-file DB overwrite in scripts/.
    # Nothing in src/, tests/, .claude/, check.sh or deploy.sh calls this.
    if DST_DB.exists() and not allow_overwrite:
        raise SystemExit(
            f"REFUSED: {DST_DB} already exists ({DST_DB.stat().st_size:,} bytes).\n"
            f"  step_copy() would overwrite it wholesale with {SRC_DB}\n"
            f"  ({SRC_DB.stat().st_size:,} bytes), a one-off 2026-04 migration source.\n"
            "  This migration ran once on 2026-04-06; the script is kept as history.\n"
            "  There is no git recovery path — .gitignore excludes *.db.\n"
            "  To replay it anyway (DESTROYS the live database):\n"
            "      python scripts/migrate_db.py full-migrate --overwrite-live-db"
        )
...
# Main
# ---------------------------------------------------------------------------
def main(*, allow_overwrite: bool = False) -> None:
    step_copy(allow_overwrite=allow_overwrite)
...
USAGE = """\
scripts/migrate_db.py — one-off politracker.db → atmina.db migration (ran 2026-04-06).

Kept as documented history. A bare invocation used to fall through to main(), whose
first step overwrites the live data/atmina.db wholesale; it now prints this instead.

  document_politicians              migrate the document_politicians junction only
  full-migrate                      replay the full migration (refuses if data/atmina.db exists)
  full-migrate --overwrite-live-db  replay it and DESTROY the existing live database
"""
...
if __name__ == "__main__":
    ...
    argv = sys.argv[1:]
    if argv and argv[0] == "document_politicians":
        migrate_document_politicians()
    elif argv and argv[0] == "full-migrate":
        main(allow_overwrite="--overwrite-live-db" in argv)
    else:
