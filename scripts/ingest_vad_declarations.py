...
def main(argv=None) -> int:
    ...
    with VadClient() as client:
        for pid, name, _slug in politicians:
            ...
            total_skip_denylist += result.rows_skipped_denylist
            ...
            print(
                ...
                f"skip_denylist={result.rows_skipped_denylist} "
                f"errs={len(result.errors)} ({elapsed:.1f}s)"
            )

    total_elapsed = time.monotonic() - started
    if total_skip_denylist:
        # Denominatora noteikums: katrs deny-trāpījums ir redzams, ne kluss.
        print(f"[denylist] {total_skip_denylist} svešās rindas bloķētas — pārbaudi logu un data/vad_denylist.json dzīvīgumu")
    print(
        f"\n[done] new={total_new} present={total_present} "
        f"skip_role={total_skip_role} skip_legacy={total_skip_legacy} "
        f"skip_denylist={total_skip_denylist} "
    ...
    if not args.dry_run:
        try:
            append_ingest_entry(
                ...
                documents_skipped=(
                    total_present + total_skip_role + total_skip_legacy
                    + total_skip_denylist
                ),
