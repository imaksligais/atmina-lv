...
# Pēdējā rinda VIENMĒR nosauc iznākumu — arī tad, kad izejas kods pazūd.
#
# Kāpēc: konveijera izejas kods ir PĒDĒJĀS komandas kods, tāpēc
# `bash scripts/check.sh 2>&1 | tail -20` atgriež `tail` nulli arī tad, kad
# pytest ir kritis. `wiki/operations/commands.md` par to brīdina kopš
# 2026-08-16 — un 2026-08-27 tas notika VĒLREIZ, ar to pašu sekām: sarkans
# pytest tika nolasīts kā "exit 0". Piezīme, kas nokrita divreiz, ir vārts,
# kas paļaujas uz cilvēka atmiņu; šis trap to pārvērš izvadā.
#
# Pēc šī: `| tail` loga pēdējā rinda ir vai nu "==> all checks passed", vai
# "==> CHECK FAILED (exit N)". Nav jāsaskaņo ar izejas kodu, ko caurule apēd.
_check_exit_banner() {
    local rc=$?
    if [ "$rc" -ne 0 ]; then
        echo ""
        echo "==> CHECK FAILED (exit $rc) — skaties augstāk, kurš solis krita."
        echo "    NB: ja sauci šo caur konveijeru, \$? rāda konveijera pēdējo komandu, ne šo kodu."
    fi
    return "$rc"
}
trap _check_exit_banner EXIT
...
# `static` is in the default because it alone emits sitemap.xml: without it a
# new brief page lands in blog/ while the sitemap stays from the last static
# render, and check_output below fails "emitēta lapa nav sitemap" on every new
# brief day (2026-09-22). ~5 s.
echo "==> generate_public_site smoke (only=${CHECK_RENDER_ONLY:-dashboard,blog,static})"
"$PY" - "${CHECK_RENDER_ONLY:-dashboard,blog,static}" <<'PYSMOKE'
import sys
