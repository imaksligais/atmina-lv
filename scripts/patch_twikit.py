...
ERROR_CODE_OLD = "            error_code = response_data['errors'][0]['code']"
ERROR_CODE_NEW = "\n".join([
    "            # atmina patch 6 (2026-08-22): X sometimes returns an errors[] entry",
    "            # with no 'code' key. Upstream indexes it directly, so twikit's own",
    "            # error HANDLER raises KeyError: 'code' and destroys the real error —",
    "            # the caller sees a traceback instead of the message X actually sent.",
    "            # Measured 2026-08-22: 3 accounts silently lost (@realNepareizais",
    "            # timeline, @Kalniete and @ijabs replies) while morning_ingest still",
    "            # reported OK. .get() lets the None fall through to the status-code",
    "            # branch below, which raises a typed exception carrying response.text.",
    "            error_code = response_data['errors'][0].get('code')",
])


def patch_error_code(root: Path) -> bool:
    """Stop twikit's error handler from raising KeyError on a code-less error payload."""
    cp = root / "client" / "client.py"
    text = cp.read_text(encoding="utf-8")
    if "atmina patch 6" in text:
        return False  # already patched
    if ERROR_CODE_OLD not in text:
        print("  WARNING: error_code line not found in client.py — upstream shape changed")
        return False
    cp.write_text(text.replace(ERROR_CODE_OLD, ERROR_CODE_NEW, 1), encoding="utf-8")
    print("  client.py: errors[0]['code'] -> .get('code') (no more KeyError in the error handler)")
    return True
...
def main():
    ...
    if args.check:
        ...
        if "ON_DEMAND_HASH_PATTERN" not in ttext:
            ...
            ok = False
        cltext = (root / "client" / "client.py").read_text(encoding="utf-8")
        if "atmina patch 6" not in cltext:
            print("  MISSING: errors[0]['code'] KeyError guard (Patch 6)")
            ok = False
    ...
    print("\n--- Patch 6: errors[0]['code'] KeyError guard ---")
    changes += int(patch_error_code(root))
