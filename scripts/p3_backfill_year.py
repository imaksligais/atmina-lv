...
# Frakcijas etiķetes kanonizācija — VIENA karte visiem ingest ceļiem. Šis
# ceļš savāc šūnas ar Playwright JS snippetu (cells[i+1]) un līdz 2026-09-04
# glabāja tās verbatim, tāpat kā p3_backfill_year_urllib.py.
...
def _to_vote_result(data: dict, vote_url: str) -> VoteResult:
    ...
    for d in data.get("deputies") or []:
        iv = IndividualVote(
            deputy_name=d["name"],
            faction=normalize_faction(d.get("faction")),
            vote=d.get("vote") or "",
        )
    ...
    vote = VoteResult(
        ...
        # A label scraped off the rendered page is the source's own word;
        # the fallback below is ours (result_source, 2026-08-18).
        result_source="agenda_label" if data.get("result") else None,
        url=vote_url,
...
def process_session(page, session: dict, year: int, log_file: Path) -> dict:
    ...
    if not uuid:
        # Dzīvā (`active=1`) manifesta rinda — 2026-09-16; sk. p3_backfill_year_urllib.py.
...
def main() -> int:
    ...
    # Katrs balsojums ģenerē claims, un katrs claim tiek embedots. Salauzts
    # embedding steks te nozīmē daļēju rakstīšanu: store_vote() commit-o pirms
    # claim ģenerēšanas, tāpēc balsojumi paliek DB, bet claims pazūd.
