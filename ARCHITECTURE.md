...
`data/atmina.db` (SQLite + WAL mode) holds everything load-bearing. It carries **43 logical tables** (re-counted 2026-09-18; the earlier "40" had gone stale) — `grep -o 'CREATE TABLE[^(]*' src/schema.sql` (21, ignoring the comment on line 2) + `src/saeima/schema.py` (7) + `src/vad/schema.py` (11), plus `publish_approvals`, `external_profiles`, `tension_proposals` and `tension_judged` (4) created by the migration block in `src/db_migrations.py` (`apply_migrations()`, called from `init_db()`). Two more are `sqlite-vec` virtual tables (`document_vectors`, `claim_vectors`), each of which expands into several shadow tables, so a raw `sqlite_master` count reads much higher.
