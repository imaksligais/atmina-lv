...
- `## Koalīcija vs Opozīcija` tabula — verbatim (tagad ir 5-kolonnu tabula: Bloks | Pozīcijas | Partijas | Galvenie runātāji | Dominējošās tēmas). **Arī `<span class="bloc-explainer">…</span>` rinda tieši zem tabulas** (kopš 2026-09-05: paskaidro, ka «Opozīcija» = Saeimas frakcijas ārpus valdības, «Bez Saeimas frakcijas» = partijas statuss) — nedzēs un nepārraksti; renderis to tāpat pieliek atpakaļ. Tava sintēzes rindkopa iet ZEM tās.
...
- Under each topic's positions table — **cik prozas raksti, nosaka konteksta kastīte** (A+B noteikums, 2026-09-02; pilnā versija: [`brief-shared-rules.md`](../../wiki/operations/agenti/brief-shared-rules.md) § Prozas bloku forma):
  - **Tēmai NAV kastītes** → 2-3 teikumi, kas savieno pozīcijas: kur partijas sakrīt vai šķiras, kas uz spēles, kurš ir izolēts.
  - **Tēmai JAU IR kastīte** → ne vairāk kā **viens** teikums, un tikai par to, kā kastītē nav (kurš klusēja; kā dienas beigas maina kastītes iezīmēto loku). **Ja jaunā nav nekā — rindkopu izlaid pavisam.** Skelets kastītes ņem no `routine_day_window(date)`, tātad tās vienmēr ir TĀS PAŠAS dienas piezīmes — kastīte un tava sintēze pēc uzbūves stāsta vienu dienu, un divi bloki pēc kārtas ir dublējums, ne papildinājums.
  - **Forma abos gadījumos:** maksimums 120 vārdu blokā; 4+ nosaukti aktieri iet sarakstā (viens aktieris = viena rinda), ne semikolu virtenē; pirmais teikums ir tēmas būtība līdz ~25 vārdiem.
  - **Abus sedz `lint_lv_style()` 5. un 6. likums** (`prose-block-too-long`, `context-box-synthesis-duplication`; kopš 2026-09-02). Tie krīt automātiski — 13. pašpārbaudes punkts tos jau palaiž, tāpēc atsevišķs darbs te nav, bet ziņot skaitļus (18. punkts) joprojām ir tava atbildība.
  Use the `<!-- SINTĒZE: Koalīcija: ... | Opozīcija: ... -->` comment as guide. Delete the comment when done.
...
- If a topic has no context box, write a brief 2-3 sentence context yourself (in a `<div class="context-box">`) based on DB claims history. Tā ir tava proza — B forma (≤120 vārdi, aktieri sarakstā) uz to attiecas.
...
18. ✅ **Prozas bloku saucējs (A+B, 2026-09-02)** — pirms saglabāt saskaiti savus prozas blokus (tēmu sintēzes, paša rakstītās konteksta kastītes, Koalīcija vs Opozīcija rindkopa; NE skeleta kastītes, NE tabulas) un atskaitē nosauc **trīs** skaitļus: cik bloku, garākā bloka vārdu skaits, cik bloku pāri 120 vārdiem. Papildus nosauc, **cik tēmām bija skeleta kastīte un cik no tām tu zem tabulas raksti vairāk par vienu teikumu** — otrajam skaitlim jābūt 0. «Pārskats ir pārskatāms» bez šiem skaitļiem ir vārti, kas nevar nokrist.
    ```python
    blocks = [b for b in agent_prose.split("\n\n") if b.strip()]
    print("bloki:", len(blocks),
          "| garakais:", max(len(b.split()) for b in blocks),
          "| >120:", sum(1 for b in blocks if len(b.split()) > 120))
    ```
...
- **Kastīte NAV brīvs papildinājums tavai sintēzei — tā ir tās vietā.** `generate_daily_brief()` (skelets) piezīmes izvelk ar `routine_day_window(date)` (`src/briefs.py:431`), tāpēc kastītē nekad nav iepriekšējo dienu fona — tur ir ŠĪS dienas piezīme par ŠO dienu. Ja zem tabulas uzraksti pilnu sintēzi, lasītājs vienu un to pašu dienu izlasa divreiz pēc kārtas. 2026-09-01 § Imigrācija tā deva 355 vārdus nepārtrauktas prozas (206 + 149) ar tiem pašiem aktieriem tajā pašā secībā un gandrīz identisku noslēguma frāzi. Rīcības noteikums ir augstāk, PAPILDINI sadaļā.
...
- **Spriedzes apraksts pārskatā ir KOPIJA, ne skats (pievienots 2026-08-25).** Tabula nes `political_tensions.description` tekstu tā brīža redakcijā, tāpēc DB rindas labojums pēc pārskata uzrakstīšanas šeit NEIENĀK pats. Ja spriedzi vai tendenču piezīmi labo pēc tam, kad pārskats jau uzrakstīts, labojums ir **trīs vietās**: DB rinda · šī tabula (un konteksta kastītes, ja tās to pašu teikumu atkārto) · `wiki/dailies/` fails. 2026-08-24 labojums aizgāja tikai uz DB, un pārskats #496 vienlaikus saturēja abus variantus — konteksta kastīte pareizi, tabula nepareizi.
- **Laika apgalvojumus pārbaudi pret `claims.stated_at`, ne pret spriedzes tekstu.** «Vienā dienā», «tajā pašā dienā», «diennakts laikā» ir pārbaudāmi fakti: pirms tos raksti vai pārraksti, izvelc katra nosauktā politiķa claim `stated_at`. Šī klase 2026-08-24 deva 3 instances no 3, un trešo ieviesa pirmo divu labojums — pēc KATRA prozas labojuma pārlasi visu rindkopu, ne tikai laboto frāzi.
...
- **Tēma:** ir viens no 33 kanoniskajiem tēmu nosaukumiem, ko definē `src.topic_map.get_all_group_names()`. Ja nepārliecināts par precīzu nosaukumu, palaid:
  ```
  .venv/Scripts/python.exe -c "from src.topic_map import get_all_group_names; print(sorted(get_all_group_names()))"
  ```
