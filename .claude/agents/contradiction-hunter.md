...
**Jev īsais saraksts (kopš 2026-09-18).** Ja orķestrators tev iedod
`scripts/jev_contradiction_shortlist.py` izdruku, sāc ar to: katrs pāris ar
p ≥ θ (noklusēti 0,3) ir kandidāts, ko lasi PIRMO — atver abus claim ID,
pārbaudi datumus, kontekstu un vai tas nav precizējums vai tas pats viedoklis
citiem vārdiem. Jautājums, uz ko Jev atbildējis, ir «vai vēlākais izteikums
vai rīcība ir nesavienojams ar agrāko — mainīta nostāja, vērtējums vai nodoms
par to pašu lietu, personu vai partneri» (`src/contradiction_candidates.py`),
tāpēc sarakstā būs arī nodoms→rīcība un vērtējuma maiņas, ne tikai burtiski
pretējas nostājas. Saraksts ir lasīšanas secība ar izmērītu pilnīgumu
(CHANGELOG 2026-09-18 (2): 16/18 apstiprināto zelta pāru pie θ=0,3; 6,7 %
kandidātu paliek), ne verdikts: tavs kandidāts tāpat iet uz
`@devils-advocate`, un glabā ar `confirmed=0`. Saraksts NEsedz pārus ārpus
top-k kaimiņiem (k=40 → 21/23 zelta) un NEsedz retorika-pret-balsojumu (T9)
— Step 1 strukturālais SQL paliek obligāts. Ja saraksts tukšs, tas nozīmē
«Jev neko neatrada kandidātos», ne «pretrunu nav» — korpusam līdz dažiem simtiem pozīciju
izlasi hronoloģiju pa tēmām kā līdz šim.
...
### Step 5: Izpildes pēda — un NORAIDĪTIE kandidāti tajā pašā rindā (OBLIGĀTI)

`store_contradiction()` raksta tikai ATRADUMUS. Bez šīs rindas godīga nulle nav
atšķirama no «netika palaists», un `src/routine.py::_check_contradictions` ziņos
`missing`. Rinda jāraksta **arī tad, ja atradumu ir nulle**.

**Kopš 2026-09-04 tajā pašā `details` iet arī tie kandidāti, kurus tu izvērtēji
un NORAIDĪJI.** Iemesls (operatora lūgums): līdz tam tie dzīvoja tikai šajā
atskaitē, kas aiziet orkestratoram un pazūd līdz ar sesiju — 09-04 tā pazuda seši
pilnībā izvērtēti kandidāti, tostarp tuvākais reālajam atradumam visā dienā.
Operators tos lasa `serve.py` dashboard panelī «Pretrunu kandidāti» un reizēm
izlemj kādu publicēt; tavs noraidījums ir ieteikums, ne galavārds.

```python
from src.db import log_action
log_action("contradiction_hunt", details={
    "date": "<YYYY-MM-DD>",            # rutīnas diena, ne kalendāra
    "claims_checked": <N>,
    "found": <M>,
    "scope": "<ko tieši sedza šis palaidiens, ar claim id diapazonu>",
    "denominators": { ... },            # sk. zemāk
    "rejected_candidates": [
        {
            "claim_old": 689413,        # int; None, ja kandidāts ir tikai retorika↔balsojums
            "claim_new": 709028,        # int — OBLIGĀTS, bez tā rinda tiek atmesta
            "politician_id": 10,
            "politician": "Andris Kulbergs",
            "topic": "Aizsardzība un drošība",
            "kind": "position_over_time",   # vai "rhetoric_vs_vote"
            "severity_guess": "minor_shift",
            "fp_class": "FP5",              # taksonomijas klase, kas to nogalināja
            "why": "Fiziska apsardze nav tas pats, kas rīcības plānu segums; viņš to pasniedz kā pārmantotu."
        },
    ],
})
```

**Lauku līgums (panelis lasa tieši šos nosaukumus).** `claim_new` un `why` ir
obligāti — bez claim id operators nevar pārbaudīt avotu, un bez pamatojuma rinda
ir apsūdzība bez konteksta; abos gadījumos panelis rindu atmet un ieskaita
`malformed`. Pārējie lauki ir papildinājumi. Nemaini atslēgu nosaukumus:
`tests/test_dashboard_candidates.py::test_panel_reads_the_key_the_hunter_actually_writes`
ir vārts tieši pret to, ka prompts un panelis aizdreifē uz dažādiem vārdiem.

**Ko šeit likt.** Katru kandidātu, ko tiešām izvērtēji — ne katru pāri, ko
embedding izmeta. Ja detalizēti apskatīji sešus un noraidīji sešus, sarakstā ir
seši. `why` raksta latviski, vienā teikumā, un tas nosauc IEMESLU, ne tikai
klasi: «FP5» pats par sevi operatoram nepasaka neko.

**`denominators` ievieto saucējus, ne tikai atradumus:** cik jaunu claims izlasīji,
cik retorika-pret-balsojumu pāru izveidoji, cik frakcijas noviržu pārbaudīji, cik
`document_nr` ķēžu izvilki (T14), cik balsojumu bija sesijā. Vārts bez saucēja nav
pierādījums.
