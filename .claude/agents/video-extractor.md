...
> **STATUS: ceļš pārbaudīts, produkcijā nekad nav izmantots.** `SELECT COUNT(*) FROM documents WHERE platform='video'` = **0**. Reāla gadījuma nav bijis: vienīgais, kas šo ceļu izgāja, ir viens E2E tests 2026-07-22 (2 minūšu klips ar karstu pārrunāšanos), kurā atribūcijas stop-gate korekti apturēja ekstrakciju — 0 saglabātu pozīciju, `reviewed_at` neaiztikts; testa dokuments pēc tam dzēsts. Tātad stop-gate ir pierādīts, laimīgais ceļš nav. Zināmā robeža, ko tests atklāja: diarizācija uz karstas pārrunāšanās jauc runātāju robežas — pirmajam īstajam ingestam izvēlies mierīgu interviju, nevis debašu klipu. Ja transkripta saturs nesakrīt ar piešķirto runātāju (pirmās personas ministra frāzes zem `@host` u. tml.), APSTĀJIES un ziņo; sk. CLAUDE.md Data Contract #13 un BACKLOG § Video ingest.
