...
> **Pass/fail kritēriji Saeimas sesijai — [`wiki/operations/quality-bars.md`](../../wiki/operations/quality-bars.md). Izlasi PIRMS glabāšanas/publicēšanas, ne pēc.** CLAUDE.md § Quality Bars sauc šo failu par kanonisko nesēju; līdz 2026-08-09 uz to saistīja 1 no 17 nesējiem.
...
- Bash with Python — run `src/saeima/` package functions (`votes.py`, `parsing.py`, `bills.py`) to parse and store data
...
#### Step 2.0: Enumerate ALL sitting UUIDs for the date (BEFORE any scraping)

**One date can carry several sittings**, each with its own
`DK?ReadForm&nr={SESSION_UUID}` page — and a sitting that CONTINUES an earlier
extraordinary session appears under the later date as **its own UUID**, not as
part of that day's regular sitting (the 23.07.2026 extraordinary session
continued on 20.08.2026 exactly this way). So the unit of work is the DATE, not
one agenda page.

Open the calendar (`DK?ReadForm&calendar=1`) first, collect **every** UUID whose
label carries the target date, then run Step 1 + Steps 2.A–2.C for each of them
and take the **vote-URL union across all UUIDs** into Step 3. The calendar
prints five label forms, and all five are sittings:

| Label    | `session_type`    | Sēdes veids |
|----------|-------------------|-------------|
| `20`     | `regular`         | kārtējā sēde |
| `20(J)`  | `jautajumi`       | jautājumu sēde — has votes (2025-10-30 had 2); never assume empty |
| `20(A)`  | `arkartas`        | ārkārtas sēde |
| `20(As)` | `arkartas_sesija` | ārkārtas sesijas sēde |
| `20(S)`  | `sviniga`         | svinīgā sēde (no roll-call votes) |

The canonical parser is `scripts/_p3_extract_sessions_2026-05-26.py` — its
docstring carries these five forms and the story of how `(As)` and `(S)` were
silently dropped for a year, taking 16 sittings with them. An unreadable label
is a STOP there, and the same discipline applies here: a UUID the calendar shows
for the date but you did not scrape means the DATE is not ingested.

**Why this is a hard rule:** on 2026-08-20 the DB held one of the date's TWO
sitting UUIDs (61 votes, last at 18:58:29) and the day was reported done; the
second UUID was absent from the session manifest, so its 25 votes (17:40–19:56,
including the airBaltic 1495/Lp14 final vote) stayed missing for 13 days.
2026-07-23 carried THREE sittings.
...
#### Step 2.B: Extract voting URLs (FOUR patterns — check ALL)

Vote IDs appear in the agenda snapshot in **four** layouts. Older sessions
expose static `OpenDocument` links; newer DK sessions embed the IDs in a
JavaScript `addVotesLink(...)` call with **no static href**; from
...
formal 2.B STOP if the new layout had not been spotted); and since 2026-09-10
a **LIVE** (`IsActual="1"`) agenda renders **no vote link at all** — the GUID
sits in the 21st argument of a `drawDKP_Pr(...)` call. Extract from all
four and take the union — checking only the first is how the **2026-06-04
session (70 votes) was silently missed**.

**2026-09-10, measured:** patterns 1, 2 and 3 each returned **0** on the live
agenda, i.e. the canonical three-pattern union was empty and the day read as a
formal 2.B STOP. Pattern 4 found 76 candidates → 16 real votes. A live sitting
therefore CANNOT be cleared by the three-pattern union alone.
...
# Pattern 4 — LIVE agenda (2026-09-10+): the page renders NO vote link; the
# vote GUID is the 21st argument of drawDKP_Pr(...). Build
#   .../Voting?ReadForm&parentID={GUID}
# from it, exactly like pattern 3 (same freshness caveat applies).
grep -oE 'drawDKP_Pr\([^)]*\)' snapshot_file        # then take arg #21 per call
```

**Fetch BOTH agenda forms for a live day.** `DK?ReadForm&actual=1` is richer
than `DK?ReadForm&nr={uuid}`: on 2026-09-10 it carried 56+17 blocks against 52,
and both attendance registrations plus the 15:33 vote appeared ONLY there. Take
the union of both.

Union all four, dedupe; patterns 1–2 build `/0/{HEX}?OpenDocument`, patterns 3–4
use the `Voting?ReadForm&parentID={GUID}` form (after `&amp;`→`&` normalization
if present). The canonical regexes for **all four** patterns live in
`scripts/p3_backfill_year_urllib.py` (`_STATIC_VOTE_RE`, `_ADD_VOTES_RE`,
`_VOTING_READFORM_RE`, `_DRAW_DKP_PR_RE` + `_DRAW_DKP_VOTE_GUID_ARG`; union
helper `_extract_vote_urls_from_agenda`, added 2026-06-12). **Pattern 4 is in
that helper since 2026-09-16** — it reads the 21st argument of every
`drawDKP_*(...)` call (deliberately wider than `drawDKP_Pr`: registration items
are drawn by `drawDKP_UT`, and 2 of the 16 votes on 2026-09-10 were attendance
registrations), validates it as a 36-char GUID, and builds the pattern-3 URL.
Locked by `tests/test_saeima_live_agenda_pattern4.py`, whose mutation test makes
the suite red if pattern 4 is removed. Two things that did NOT change: the
helper still counts only what the fetched agenda renders, and the parity audit
now REFUSES a live day outright (`audit_saeima_agenda_parity.py` gate 4, exit 2)
— a live page does not render sub-item votes, so it undercounts (2026-09-10: 16
against 21 in the `nr={DkId}` form). **Freshness caveat (verified 2026-06-12):** pattern-3 pages serve
...
#### Step 2.C: Harvest the agenda outcome labels (result provenance)

The outcome label ("Pieņemts", "Nod. kom.", "Likums", "Paziņojums", …) exists
**only here**, on the agenda page — the vote page's red `<span>` is empty. It
sits in the 9th argument of the `drawDKP_*(...)` call, keyed to the agenda ITEM,
and `addVotesLink(DKP_HEX, VOTE_HEX)` (the same call Step 2.B reads) bridges it
to the vote. Harvest it here, in the same visit, and pass it to Step 3.C — that
is the ONLY way `saeima_votes.result` becomes a source quote instead of our
arithmetic (`result_source='agenda_label'` vs `'computed'`; BACKLOG § Saeima).

```bash
cd ~/atmina
PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -c "
from src.saeima import extract_agenda_result_labels
html = open('path/to/agenda.html', encoding='utf-8').read()
labels = extract_agenda_result_labels(html)
print(len(labels), 'no', len(vote_urls), 'balsojumiem ar etiķeti')
for url, label in labels.items(): print(label, url)
"
```

The helper needs the agenda **HTML source** (the `drawDKP_*` calls are script
text), not the accessibility snapshot — fetch `DK?ReadForm&nr={uuid}` directly if
the Playwright snapshot has already stripped it.

**A missing label is a legitimate outcome, not a gap to fill.** Sub-item votes
("Par priekšlikumu Nr.N", "Par debašu laika pagarināšanu") and attendance
registration carry none; those votes go to Step 3.C WITHOUT `agenda_result` and
keep NULL or computed provenance. Never invent a label to make the map complete.
Report the denominator: how many of the session's votes got a label.
...
**"Par priekšlikumu Nr.N" votes with an interesting split (operatora noteikums 2026-08-21):**
A proposal vote normally inherits its bill chain's summary (same substance, T14). BUT when
the ballot pattern itself is the story — a rejected proposal with real support, a coalition
faction voting apart from its bloc, cross-bloc alignment — open the stage's **Priekšlikumu
tabula** on the bill card, find row N, and summarize THAT proposal: who submitted it, what
it would change, the committee verdict, and the ballot line. Reference case: 56/Lp14
otrreizējā caurlūkošana, priekšlikums Nr.7 (vote 7900) — ekonomikas ministra Valaiņa
«zelta vīzu» norma, komisijā neatbalstīta, Saeimā 14/53/12, paša ministra ZZS frakcija
atturējās; the generic bill summary told the reader nothing about what was actually rejected.
Heuristic for "interesting": result `Noraidīts` with ≥10 Par, OR any faction voting against
its own bloc's majority, OR the proposal is what the day's media debate is about.

**Māsas balsojumu noteikums — kopsavilkumā nedrīkst būt CITA balsojuma iznākums
(operatora lēmums 2026-08-17, atkārtoti 08-18; šeit vēsturiski Step 3.5).** Viena
`document_nr` balsojumi mēdz dalīt vienu `summary`, un `generate_claims_from_votes`
to ieauž katra deputāta stancē. Tāpēc jaunam ne-lasījuma balsojumam (priekšlikums
Nr.N, steidzamība, nodošana komisijai) kopsavilkums apraksta TIKAI to, ko šis
balsojums izšķir — bez teikuma par lasījuma iznākumu. Reference: 6916
(priekšlikums Nr.1, 2. lasījums, **Noraidīts**) mantoja 6103 (1. lasījums)
kopsavilkumu ar «pirmajā lasījumā vienbalsīgi pieņemts (84 par)», un visi 84
claims aprakstīja noraidīto priekšlikumu ar cita balsojuma iznākumu. Vēsturiskās
rindas (246 balsojumi, ~20k claims) NETIEK pārrakstītas — tur šī frāze lasāma kā
likumprojekta konteksts; noteikums attiecas uz jaunām rindām.
...
result = process_vote_snapshot(
    ...
    agenda_result=labels.get(vote_url),  # from 2.C; None when the item has no label
)
print(result)  # includes result + result_source — check the provenance you wrote
"
```

`agenda_result` is the VERBATIM label from Step 2.C. Passing it makes the stored
`result` a source quote (`result_source='agenda_label'`) and overrides the
parser's computation. Pass the label **exactly as the agenda prints it** — do not
map `Nod. kom.` / `Likums` / `Paziņojums` onto `Pieņemts`/`Noraidīts`; the corpus
dictionary is wider than two values (operator verdict 2026-08-18). Omit the
argument when the item has no label; a fabricated one is worse than NULL.
...
#### Step 5.bis: Agenda parity — the DATE's votes, not one sitting's

Summary completeness only inspects the rows you DID store. This second gate
checks that you stored them all:

> **DB votes by `(vote_date, vote_time)` ⊇ the union of agenda votes across ALL
> sitting UUIDs of that date.** Any agenda vote without a DB row = **STOP**:
> list the missing ones (URL, date, time, motif) and report. Never round the
> date off as done while one of its sittings is short.

Compare by `(vote_date, vote_time)`, **never by URL** — titania re-archives vote
pages under new UNIDs, so a URL diff quietly answers "nothing missing"
(Important Notes #4).

```bash
cd ~/atmina
.venv/Scripts/python.exe scripts/audit_saeima_agenda_parity.py --year YYYY --dates YYYY-MM-DD
```

The tool is read-only, prints one row per SITTING (darba kārtībā / DB / TRŪKST),
so a date with several UUIDs yields several rows — a single row for a date you
know had two sittings is itself the signal that the manifest is short.

**The manifest must be fresh.** The tool reads
`data/saeima_backfill_sessions.json`, which is a stored snapshot of the
calendar, not a live read: a manifest generated before the date's sittings
existed shows the date with too few UUIDs, or not at all, and the audit then
prints "trūkst 0" over a denominator that never included them. That is precisely
what hid 08-20 — the 08-01 manifest saw 07-23 as 6+3 votes and 08-20 not at all,
a clean report across 118 stored rows it had never looked at. The tool therefore
gates on manifest age and emits a STOP naming the stale manifest; when you see
it, re-capture the calendar snapshot and regenerate the manifest with
`scripts/_p3_extract_sessions_2026-05-26.py`, then re-run the audit. That STOP
is never something to pass over — an unaudited date is an un-ingested date.
...
| Date has more sitting UUIDs in the calendar than were scraped                         | STOP, scrape all (2026-08-20: one of two sittings, 25 votes lost for 13 days) |
...
### T12 — a zero result is a format change until proven otherwise

The general rule behind every STOP row above (CLAUDE.md T12): **when an
upstream source starts returning nothing, assume the format changed — never
that the content was removed.** saeima.lv, VID and X have each done exactly
this. A vanished selector, an empty list and a 404 are all the same signal
class: the page moved, not the politics.

Concretely: do not "handle" an empty result by writing an empty day. Check the
upstream page by hand, look for a new layout, keep the existing fallback
patterns in place alongside any new one (the three-template union in Step 2.B
exists because of this class), and alert + retry rather than fail hard. If you
cannot tell whether it is a format change or a genuinely empty session, that is
a STOP, not a judgement call — an empty day recorded once looks identical to an
empty day forever after.
...
`src/saeima/` (the loader) sets it automatically — you should never need to set it manually.
...
(`src/wiki.py`, `src/briefs.py`, `src/render/`) will treat the row as
...
**Party ≠ faction (2026-07-28, from the 2026-07-26 note #373 incident).** The
`faction` column on an individual vote records who sat in the faction FOR THAT
VOTE — it is not party membership. A party member can sit outside the faction
(Kiršteins: LPV member, `faction=NULL` on votes — legitimate, never "fix" his
party to match). Any statement about how "the faction" voted must be derived
from the `faction` values in that vote's own rows, never by grouping tracked
politicians by their `party` field. Deputies with `faction=NULL` in a vote are
not part of that faction's tally.
