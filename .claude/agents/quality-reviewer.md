...
# NB: an even earlier version of this query carried `date(c.created_at) >=
...
tension_rows = db.execute("""
    ...
    WHERE DATE(created_at, 'localtime') = ?     -- UTC kolonna, sk. CLAUDE.md
""", (ROUTINE_DAY,)).fetchall()
note_rows = db.execute("""
    SELECT id, content FROM context_notes
    WHERE note_type = 'context' AND DATE(created_at) = ?   -- LV kolonna, BEZ localtime
""", (ROUTINE_DAY,)).fetchall()
