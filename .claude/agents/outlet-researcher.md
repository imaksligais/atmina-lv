...
- **Do not touch the editorial `framing:` field.** It lives on the `sources:`
  feed rows, is an INTERNAL `@claim-extractor` confidence signal, and is never
  published on an outlet page — publishing it would turn a sourced-facts page
  into a bias rating, which is the one thing this feature was designed not to
  be. You neither read it, copy it, nor propose changes to it. (This rule was
  carried only by the human-readable twin `wiki/operations/agenti/outlet-researcher.md`
  until 2026-08-15; the canonical prompt is this file, so it belongs here.)
