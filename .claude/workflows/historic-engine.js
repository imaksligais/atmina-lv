...
if (!RESOLVED.length) {
  ...
  return { engine: 'historic-engine', report: [], totalDocs: 0, totalClaims: 0, totalSurvivors: 0, totalFailures: 0, hunt: HUNT, note: 'empty resolved set' }
}
...
B) RHETORIC-vs-ACTION (STRUCTURAL — new saeima_vote rows get no vector since 2026-08-21 and similarity never shows STANCE mismatch, so vector search is not evidence here
...
const report = clean.map((r) => ({
  ...
  // save_analysis reports every DROPPED claim here (missing_source_url,
  // inactive politician, silent_dedup) and reports it nowhere else. The schema
  // declared it and the extractor prompt demands it, but until 2026-08-15
  // nothing downstream read it — so a bulk historic load could discard claims
  // and still return a clean-looking summary. CLAUDE.md § Working Conventions:
  // "any new operation that can drop input must report what it dropped".
  failures: r.extract?.failures || [],
...
const totalFailures = report.reduce((n, r) => n + r.failures.length, 0)
log(`historic-engine done: ${report.length} politicians, ${totalDocs} docs linked, ${totalClaims} new claims, ${totalSurvivors} survivors, ${totalFailures} dropped claims (hunt=${HUNT}).`)
if (totalFailures > 0) {
  log(`WARNING: ${totalFailures} claims were DROPPED by save_analysis — read report[].failures before treating this run as complete.`)
}
