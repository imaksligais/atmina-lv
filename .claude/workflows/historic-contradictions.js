...
// See historic-backfill.js: dropped claims must survive the wrapper boundary.
const totalFailures = eng?.totalFailures || 0
log(
  `Done. ${report.length} politicians, ${totalSurvivors} contradiction survivors stored confirmed=0` +
...
return {
  ...
  next_steps:
    totalSurvivors > 0
      ? 'Review confirmed=0 contradictions, UPDATE confirmed=1 the keepers, then: .venv/Scripts/python.exe -m src.render --only=pretrunas && bash scripts/deploy.sh --no-delete'
