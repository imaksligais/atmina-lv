...
// Dropped claims must not stop at the engine boundary — a wrapper that omits
// them re-creates the silent-loss defect one level up (CLAUDE.md § Working
// Conventions: stored-count == intended-count is the check that catches it).
const totalFailures = eng?.totalFailures || 0
log(
  ...
  }${totalFailures > 0 ? `, ${totalFailures} DROPPED claims — read report[].failures` : ''}.`
)
...
return {
  ...
  next_steps:
    totalClaims > 0
      ? `New historic position claims stored with stated_at=published_at. Render the affected pages narrowly + deploy additively: ${PY} -m src.render --only=pretrunas,pozicijas,dashboard,politiki,blog && bash scripts/deploy.sh --no-delete. ${
          HUNT
            ? totalSurvivors > 0
