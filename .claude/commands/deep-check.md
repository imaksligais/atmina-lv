...
- Render the contradictions domain narrowly when publishing: `python -m src.render --only=pretrunas` then `deploy.sh --no-delete` (full-tree Cloudflare deploy; the flag is a no-op).
