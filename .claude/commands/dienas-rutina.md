...
**TypeSafe matcher veto (2026-09):** `match_politicians()` judges surname-only matches when `ATMINA_TYPESAFE_VETO=shadow|enforce` is set in the environment of the ingest process (per-process, never machine-wide — `tests/conftest.py` pins `off` for the suite). During the shadow week run `.venv/Scripts/python.exe scripts/typesafe_veto_report.py --days 1` and flag any vetoed match that is a real politician (a gold loss → BACKLOG); switch to `enforce` only after seven shadow days with zero such cases. Backfill scripts keep the deterministic matcher (no variable set).
