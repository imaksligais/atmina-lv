...
description: Read-only DB integrity sweep — matcher-collision risks (≤4-char forms), x_handle↔social_accounts divergence, orphaned contradiction refs, aging unreviewed/unconfirmed rows (aged from the marker, not the claim), stale-party language, party↔faction-record divergence plus the ministers-without-vote-coverage denominator, same-day duplicate claims, missing brief-image variants, truncated stubs, unresolvable provenance URLs + hand-written-row signatures, non-canonical position topics. Emits a triage table + BACKLOG-ready blocks; fixes only with operator approval + paired rollback.
...
1. **Matcher collision risk (T1).** For every active politician, list stored `name_forms` ≤4 chars AND generated forms ≤4 chars (apply `src/matcher.py::_latvian_surname_inflections` to each surname form). For each, sample-grep recent `documents.content` to show what it currently collides with. Output candidate `negative_patterns` — proposals only. Since the 2026-07-27 B2+D2+H package, single-word forms only match at word boundaries, so a ≤4-char form is a namesake risk rather than a substring bomb — keep flagging, but say which class.
   ...
   ```
2. **x_handle divergence.** `tracked_politicians.x_handle` vs `social_accounts.handle` (`platform='twitter'`). **Report THREE numbers, always — the denominator and BOTH divergence branches.** A bare `LOWER(a) <> LOWER(b)` mismatch test is SQL-NULL-blind: a politician with `x_handle IS NULL` and a live `social_accounts` row never satisfies the comparison, so the class is invisible by construction. That is how 7 NULL rows sat unreported for months while the check returned `flagged=1` and looked clean (found 2026-08-23; 5 backfilled, `data/fix_x_handle_backfill_2026-08-23.sql`). `x_handle` is what `templates/politician.html.j2` gates the profile's X link on, so each NULL row is a missing link on a live page.

   ```sql
   -- (a) DENOMINATOR — never report a branch without it
   ...
   -- (b) NULL branch: handle exists in social_accounts, profile link renders nothing
   ...
   -- (c) VALUE branch: both set, they disagree (profile link points at the wrong account)
   ...
   Report as `saucējs N | NULL klase X | vērtību nesakritība Y`. Baseline 2026-08-25: `111 | 1 | 1` — the 1 NULL row is id 27 Bordāns, deliberately left NULL because `relationship_type='inactive'` and no profile page renders for him (196 pages built, `bordans` not among them), so filling it would change no public surface; the 1 value mismatch is id 62 Svirskis (`ESvirskis` vs `realNepareizais`, needs someone to establish which handle is live). Previous baseline `111 | 2 | 1` (2026-08-23) included id 204 Latvijas armija (NBS), filled 2026-08-25 after measuring that its page carried 82 tweet links but no header X link (`data/{fix,rollback}_x_handle_nbs_2026-08-25.sql`). A run reporting `0 | 0 | 0` means the join broke, not that the data is clean.
...
4. **Aging review queues.** `contradictions` with `reviewed=0` older than 14 days; `confirmed=0` survivors (deep-check output) older than 30 days **AND `reviewed=0`** — a `reviewed=1 confirmed=0` row is a documented rejection that legitimately sits at confirmed=0 forever (#40 Judins is the reference: its summary carries its own verdict, and the unfiltered query re-flagged it on 2026-08-04 as if it were queue debt); open review claims older than 14 days.
   ...
   **Age the queue from the MARKER, not the claim** — `COALESCE(review_status_at, created_at)` (`src.db.REVIEW_AGE_EXPR`, since 2026-09-07). `created_at` is when the CLAIM was written, which is a different fact: on 2026-08-22 a retro-marking pass put 19 April–August claims into the queue and the 14-day gate immediately reported a BAR BREACH over work done the previous day. `review_status_at` is stamped by the same two triggers when the derived status changes; rows written before 2026-09-07 carry NULL and fall back to `created_at`, so nothing gets younger — expect the distribution to shift only as the old rows are resolved.
   ...
   The gate's own form lives in `src.db.open_review_queue()` (`age_days` per row, `REVIEW_QUEUE_AGE_DAYS = 14`) — `@quality-reviewer` § A calls it rather than re-writing this SQL, so the two can never disagree.
...
9. **Party ↔ Saeima faction record (T6, wrong-from-the-start variant).** Check 5 only fires when a claim *says* someone switched; a `party` that was wrong the day it was seeded never produces such a claim and can sit for months. Cross-check against the vote record instead: for every active politician, emit the faction timeline (`label, n, first, last`) when their party's `short_name` covers **<20 %** of their faction-labelled ballots. Skip parties whose `short_name` never appears as a faction label at all — alliances (AS members carrying LZP/LRA) and non-Saeima parties are silent by construction, not findings.
   ...
   **9b. The blind zone, reported as its own denominator (2026-09-07, verdict 42).** The query above GROUPs BY `iv.faction`, so a politician with **no faction-labelled ballot at all** never appears in it — not as a finding, and not as a checked row. The check is therefore silent about them, and silence reads as clean. Ministers who are not deputies are exactly that class: there is nothing to compare their `party` against, so T6 can never fire on them. That is how pid=224 R. Melnis carried a wrong party for two months and 26 published briefs while this check reported clean every run.

   ```bash
   .venv/Scripts/python.exe scripts/audit_minister_vote_coverage.py
   ```

   Reports `checked=N flagged=M` over active politicians whose `role` contains `ministr` (substring, case-insensitive — `role` is free text and its forms drift: "Ministru prezidente (demisionējusi)", "Iekšlietu ministrijas parlamentārais sekretārs"), and lists each flagged row as `pid + name + party + role + ballots (labelled)`. A row here is **not a defect** — it is a name automated verification cannot reach, so its party has to be checked by hand (CVK, ministry page, a fresh source).

   **Baseline 2026-09-07: `checked=25 flagged=6`** — pid 15 Braže, 64 Vītols (`role='Finanšu ministra biroja ekonomists'`: the substring's known false positive, and unverifiable by votes anyway), 155 Melbārde, 158 Lāce, 159 Uzulnieks, 224 Melnis. pid 155 is an open verdict row (12); pid 212 Labanovskis sits in the same blind zone for the OTHER reason — `relationship_type='inactive'` drops him from check 9 and from this denominator alike, and his 2 898 labelled ballots all end 2025-02-20, so even a coverage read would be about a Saeima term he has left. `checked=0` means the role filter broke, not that the government has no ministers; the script exits 2 on it.
...
18. **Non-canonical `position` topics (idempotency-key hygiene).** `src/tools.py::store_claim` normalizes `topic` through `normalize_topic()`; `src/db.py::store_claim` does not. Both write the same table, and `topic` is one third of the idempotency key `(opponent_id, source_url, topic)`. Turning normalization on inside the key without a dry run is precisely the class that produced 4 087 duplicates on 2026-08-02 (§ Escalation 8: an `UPDATE claims SET topic` can MANUFACTURE duplicates), so the operator's 2026-09-06 verdict was **no code change, a counted gate instead** — if the un-normalized path ever admits a new topic form, it shows up here as one row rather than as a topic page nobody can find a year later.

    ```bash
    .venv/Scripts/python.exe scripts/audit_topic_canonical.py
    ```

    Compares every distinct `claim_type='position'` topic against the 33 canonical groups in `src/topic_map.py` (the script reads `get_all_group_names()`, so adding a 34th group needs no edit here). Prints `checked=N distinct=D/33 flagged=F` plus each offending topic with its claim count and lowest id.

    Scope is `position` on purpose: `saeima_vote` topics come from `_motif_to_topic()` and are designed to evolve, and `program_promise` renders only on the party page — the same `claim_type='position'` gate every render and brief query uses.

    **Baseline 2026-09-07: `checked=6564 distinct=33/33 flagged=0`.** `checked` drifts upward every routine day — the number that must hold is `flagged=0` and `distinct ≤ 33`. Expected value is 0. `checked=0` is a broken denominator, not a clean run (exit 2). Tests: `tests/test_audit_topic_canonical.py`.
