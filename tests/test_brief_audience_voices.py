...
def test_active_politicians_ordering_is_deterministic():
    ...
    rindām ar 4 no 18 pozīcijām. Vaicājumam jānes `p.name` kā PĒDĒJAIS
    kritērijs. Kopš 2026-09-15 starp `cnt` un `p.name` stāv `MAX(c.salience)
    DESC` — alfabēts viens pats 09-12/13 nogrieza dienas galveno runātāju
    (`tests/test_briefs.py::TestAktivakieTieBreakBySalience`); vārds paliek
    stabilais, ar partiju nekorelētais izšķīrējs precīziem neizšķirtiem.
    """
    import re
    ...
    assert "ORDER BY cnt DESC, MAX(c.salience) DESC, p.name ASC" in block.group(0), (
        "trūkst deterministiska tie-break ķēdes: " + block.group(0)[-120:]
    )
