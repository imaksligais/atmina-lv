...
def test_quote_of_day_mid_sentence_flag():
    """Fragments ar mazo sākuma burtu dabū mid_sentence=True (kartē „..” prefikss);
    pilns teikums ar lielo burtu — False. Citāts pats paliek verbatim."""
    from src.render.focus import _quote_of_day
    db = make_db()
    seed_pol(db, 1, "Anna Bērza", "Partija A")
    seed_claim(db, 1, "Budžets", 0.9, quote="par saviem līdzekļiem es netaisos maksāt tādu summu nekad")
    q = _quote_of_day(db)
    assert q["mid_sentence"] is True
    assert q["quote"].startswith("par saviem")  # verbatim, bez prefiksa datos

    db2 = make_db()
    seed_pol(db2, 1, "Anna Bērza", "Partija A")
    seed_claim(db2, 1, "Budžets", 0.9, quote="Šis ir pilns teikums ar lielo sākuma burtu pietiekamā garumā")
    assert _quote_of_day(db2)["mid_sentence"] is False
...
def test_focus_quote_cards_reads_c_stack_quote():
    from src.render.focus import _focus_quote_cards
    ...
    assert {c["source_url"] for c in _focus_quote_cards(focus)} == {
        "https://x.com/hot/1", "https://x.com/q/9"}
...
def test_hero_feed_stale_contradictions_show_none():
    """Bez svaigas (≤14 d) pretrunas karuselī pretrunu NAV — vecs „enkurs"
    2026-09-20 turēja jūlija pretrunu sākumlapas pirmajā kartītē (spec
    2026-09-20-landing-jaunakais-design.md)."""
    from src.render.focus import hero_feed
    db = make_db()
    feed = hero_feed(db, [con(1, 60), con(2, 90)], [], EMPTY_FOCUS)
    assert [i for i in feed if i["kind"] == "contradiction"] == []
