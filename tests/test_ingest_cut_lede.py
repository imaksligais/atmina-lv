...
class TestLedePrependedIsCounted:
    """`lede_prepended` — the prevention side of the same class.

    `_enrich_rss_items_fulltext` flags an item when the fetched body dropped
    the RSS lede and the lede was kept in front of it (trafilatura omits the
    first paragraph on lsm.lv/diena.lv). `_ingest_source` must surface the
    count next to `truncated_lede_fallback` so the denominator stays visible.
    """

    def test_flagged_items_count_in_result_and_log_row(self, monkeypatch):
        items = [
            {"text": "Virsraksts — ievads, kuru trafilatura nometa.\n\n"
                     "Pilns raksta teksts bez ievada, kas turpinās un turpinās.",
             "url": "https://lsm.lv/a/1", "lede_prepended": True},
            {"text": "Parasts raksts ar pilnu tekstu no paša sākuma, bez "
                     "nekādiem trūkumiem vai pielikumiem pēc tam.",
             "url": "https://lsm.lv/a/2"},
        ]
        result, _, logged = _run_source(monkeypatch, items)
        assert result["lede_prepended"] == 1, result
        ingest_rows = [r for r in logged if r["action"] == "ingest"]
        assert len(ingest_rows) == 1, logged
        assert ingest_rows[0]["details"]["lede_prepended"] == 1
