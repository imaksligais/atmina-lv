...
from src.confidence_drift import (
...
@pytest.fixture
def small_sample_db():
    """Tāds pats drifts kā `drift_db` NATO tēmai, bet tikai 4 claims katrā pusē.

    2026-08-24 brīdinājums salīdzināja divus n<=3 paraugus — vidējais uz tik
    mazas izlases nav mērījums. Detektoram šeit jāklusē (operatora verdikts
    2026-09-07, rinda 26).
    """
    fd, path = tempfile.mkstemp(suffix=".db")
    os.close(fd)
    ...
    for i in range(4):
        db.execute(
            "INSERT INTO claims (topic, confidence, source_url, stated_at) VALUES (?,?,?,?)",
            ("Ārpolitika", 0.55 + i * 0.02, "https://source-a.lv", first_half_date),
        )
    for i in range(4):
        db.execute(
            "INSERT INTO claims (topic, confidence, source_url, stated_at) VALUES (?,?,?,?)",
            ("Ārpolitika", 0.85 + i * 0.02, "https://source-a.lv", second_half_date),
        )
    db.commit()
    db.close()
    yield path
...
class TestSmallSampleIsSilent:
    """Brīdinājums bez saucēja ir vārti, kas nevar nokrist (verdikts 2026-09-07)."""

    def test_four_claims_per_half_produces_no_alert(self, small_sample_db):
        alerts = check_confidence_drift(db_path=small_sample_db, days=7, threshold=0.15)
        assert alerts == [], f"n=4 katrā pusē nav mērījums, bet troksnis: {alerts}"

    def test_five_claims_per_half_still_alerts(self, drift_db):
        # Robeža ir n<5, ne n<=5 — `drift_db` NATO tēmai ir tieši 5 abās pusēs.
        alerts = check_confidence_drift(db_path=drift_db, days=7, threshold=0.15)
        nato = next(a for a in alerts if a["topic"] == "NATO")
        assert nato["first_half_claims"] == 5
        assert nato["second_half_claims"] == 5

    def test_min_half_claims_is_overridable(self, small_sample_db):
        alerts = check_confidence_drift(
            db_path=small_sample_db, days=7, threshold=0.15, min_half_claims=4
        )
        assert [a["topic"] for a in alerts] == ["Ārpolitika"]


class TestDriftLineNamesTheDenominator:
    def test_line_shows_n_on_both_sides(self):
        line = format_drift_line({
            "topic": "Ārpolitika",
            "drift": 0.25,
            "first_half_avg": 0.6,
            "second_half_avg": 0.85,
            "first_half_claims": 7,
            "second_half_claims": 9,
            "source_growth": 0,
        })
        assert "n 7 → 9" in line

    def test_routine_status_prints_the_same_line(self):
        """Rutīnas statuss lasa to pašu formatētāju, ko atskaite.

        Bez šī vārta `src/routine.py` var atgriezties pie rindas bez saucēja,
        un tests par `format_drift_line` to nepamanītu.
        """
        src = (Path(__file__).parent.parent / "src" / "routine.py").read_text(
            encoding="utf-8"
        )
        assert "format_drift_line" in src
