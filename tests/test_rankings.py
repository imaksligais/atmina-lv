...
def test_biggest_reversals_one_row_per_person(tmp_path):
    """One person = one row in the 'Lielākie apvērsumi' column.

    Contract for the 2026-09-23 fix: the same politician must not occupy
    several rows — each person appears once, carrying their largest
    day-gap contradiction, and the column still fills up to ``limit``
    rows when enough distinct people qualify.
    """
    from src.db import get_db, init_db
    from src.render.rankings import fetch_rankings
    from src.saeima.schema import init_saeima_bills, init_saeima_tables

    db_path = str(tmp_path / "reversals.db")
    init_db(db_path)
    init_saeima_tables(db_path)
    init_saeima_bills(db_path)
    db = get_db(db_path)

    def c(name: str, delta: int, cid: int) -> dict:
        return {
            "id": cid,
    ...
    contradictions = [
        c("Persona Viens", 900, 1),
        c("Persona Divi", 800, 2),
        c("Persona Tris", 700, 3),
        c("Dubulta Persona", 600, 4),
        c("Dubulta Persona", 500, 5),  # second, smaller reversal, same person
        c("Persona Četri", 400, 6),
        c("Persona Pieci", 300, 7),
    ]
    items = fetch_rankings(db, contradictions, limit=5)["biggest_reversals"]
    db.close()

    names = [i["name"] for i in items]
    assert len(names) == len(set(names)), f"duplicate person rows: {names}"
    # 6 distinct people must still fill all 5 rows.
    assert len(items) == 5, names
    # The surviving row carries that person's largest gap.
    by_name = {i["name"]: i for i in items}
    assert by_name["Dubulta Persona"]["delta_days"] == 600
