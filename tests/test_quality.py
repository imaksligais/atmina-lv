...
from src.quality import (
    QuoteCheck,
    check_quote_against_source,
    restore_text_from_source,
    validate_lv_diacritics,
)
...
class TestValidateLvDiacritics:
    ...
    # ------------------------------------------------------------------
    # Short texts (<100 letters): one diacritic is enough (2026-09-24).
    # At 67-99 letters a single diacritic is under the 1.5% floor, so correct
    # Latvian whose words simply take no diacritics was refused at a write
    # boundary — 2026-09-23 an `analyses` row vanished this way. Measured on
    # 32 765 real 40-99-letter sentences from web articles: 29 (0.09%) sit in
    # that zone, and the old gate rejected 5 of 14 sampled; at 100-149 letters
    # the zone is 0.03%, so the ratio keeps ruling there. Stripping (T4) is
    # ZERO diacritics and stays refused at every length.
    # ------------------------------------------------------------------

    @pytest.mark.parametrize("text", [
        "Dok. 114540: humoristiska atbilde par augumu Threads ierakstam — "
        "nav politiskas pozīcijas",
        "Starp tiem ir gan premjers un ministri, gan Saeimas deputāti un "
        "virkne citu valsts amatpersonu.",
    ])
    def test_short_latvian_with_one_diacritic_passes(self, text):
        letters = sum(c.isalpha() for c in text)
        assert 66 < letters < 100  # the zone where 1 diacritic is < 1.5%
        ok, reason = validate_lv_diacritics(text)
        assert ok is True, reason

    def test_long_latvian_with_one_diacritic_still_uses_the_ratio(self):
        text = ("Starp tiem ir gan premjers un ministri, gan Saeimas deputāti "
                "un virkne citu valsts amatpersonu no Rigas un citam pilsetam "
                "visa valsti")
        assert sum(c.isalpha() for c in text) >= 100
...
class TestQuoteCheckThreeStates:
    """`check_quote_against_source` must separate "passed" from "could not check".

    Until 2026-08-24 the function returned `(True, "...cannot verify")` when the
    quote was not locatable in the source — a gate that could not fall, and one
    that on 2026-08-23 waved through two quotes (#703944, #703945) whose
    correctness was established by hand, not by the gate (BACKLOG § agenti-pipeline).
    """
    ...
    def test_verbatim_quote_is_verified(self):
        quote = "Daudz tiek runāts par airBaltic izmaksu sadaļu"
        check = check_quote_against_source(quote, self.SOURCE)
        assert check.verdict == "verified"
        assert check.verified is True
        assert check.blocks is False
        assert check.cannot_verify is False

    def test_capitalisation_only_difference_is_verified(self):
        quote = "publiskajā telpā mēs dzirdam par ieņēmumu sadaļu."
        check = check_quote_against_source(quote.capitalize(), self.SOURCE)
        assert check.verdict == "verified"
        assert check.blocks is False

    def test_stripped_diacritics_is_mismatch_and_blocks(self):
        quote = "Daudz tiek runats par airBaltic izmaksu sadalu"
        check = check_quote_against_source(quote, self.SOURCE)
        assert check.verdict == "mismatch"
        assert check.blocks is True
        # The reason must carry the source wording — the fix is to restore it.
        assert "Daudz tiek runāts par airBaltic izmaksu sadaļu" in check.reason

    def test_absent_quote_is_cannot_verify_not_verified(self):
        """The load-bearing assertion: absent ≠ passed."""
        check = check_quote_against_source(
            "Pilnīgi cits teikums, kura avotā nav vispār.", self.SOURCE
        )
        assert check.verdict == "cannot_verify"
        assert check.verified is False
        # It still must not refuse the write — 1408 live quotes are legitimately
        # unlocatable, and refusing means storing no quote at all (#555664).
        assert check.blocks is False

    def test_missing_quote_or_source_is_cannot_verify(self):
        for quote, source in ((None, self.SOURCE), ("kaut kas", None), (None, None)):
            check = check_quote_against_source(quote, source)
            assert check.verdict == "cannot_verify", (quote, source)
            assert check.blocks is False

    def test_result_cannot_be_unpacked_as_a_pair(self):
        """A stale `ok, reason = ...` call site must CRASH, not silently pass.

        Both `'verified'` and `'mismatch'` are truthy strings, so had this
        stayed a 2-tuple, every un-migrated caller would have become a gate
        that always passes — the same defect one level down.
        """
        check = check_quote_against_source("kaut kas nesameklējams", self.SOURCE)
        with pytest.raises(TypeError):
            _ok, _reason = check  # noqa: F841 — the unpacking IS the assertion

    def test_verdict_values_are_the_documented_three(self):
        assert {QuoteCheck("verified", "").verdict,
                QuoteCheck("mismatch", "").verdict,
                QuoteCheck("cannot_verify", "").verdict} == {
            "verified", "mismatch", "cannot_verify"
        }
...
def test_fasttext_unavailable_is_logged_not_silent(monkeypatch, caplog):
    ...
    reliable model load (CHANGELOG arhīvs 2026-08-19 «Vārtu vilnis 2»);
