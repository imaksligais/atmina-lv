...
class TestScopeIsDefinedOnce:
    ...
    # `src/wiki.py` sadalīts 2026-09-05 (plāna 5.9): abi backlog skaitītāji
    # pārcēlās uz `_build_index`, tātad uz `src/wiki_index.py`. Šis saraksts ir
    # ceļu saraksts, tāpēc moduļa sadalīšana bez šīs rindas būtu klusi izņēmusi
    # wiki backlog vaicājumus no vārtiem.
    QUEUE_MODULES = ["src/analyze.py", "src/routine.py", "src/wiki_index.py"]
