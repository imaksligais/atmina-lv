...
class TestMotifToTopic20260910Load:
    """2026-09-10 sēdes ielāde deva četrus nepareizus `topic` (backlog/saeima.md
    § `_motif_to_topic()` divas nepareizas tēmas): 332 balsojumu claims mantoja
    komisijas nosaukumu, `pretstatā` iekšējo «ets» un divus fallback. Pieraksts
    CHANGELOG 2026-09-15 (1); dati migrēti `data/fix_vote_topics_2026-09-15.sql`.
    """

    def test_health_crisis_motion_is_health_not_klimats(self):
        # «pretstatā» satur «ets» — topic_map bare-substring bugs deva Klimats.
        motif = ("Par katastrofālo medicīniskās palīdzības un pakalpojumu pieejamības "
                 "krīzi Latvijā pretstatā Satversmes 111. pantā garantētajam minimumam (136/P14)")
        assert _motif_to_topic(motif) == "Veselības aprūpe"

    def test_pacientu_tiesibu_likums_is_health_not_fallback(self):
        motif = "Grozījumi Pacientu tiesību likumā (1513/Lp14), nodošana komisijām"
        assert _motif_to_topic(motif) == "Veselības aprūpe"

    def test_vestures_macisana_agenda_motion_is_izglitiba(self):
        motif = "Par iekļaušanu nākamās kārtējās sēdes darba kārtībā. Par vēstures prioritāru mācīšanu (1104/Lm14)"
        assert _motif_to_topic(motif) == "Izglītība"

    def test_committee_name_does_not_pick_topic_over_subject(self):
        # Sibling vote of the one above: the referral names the Izglītības,
        # KULTŪRAS un zinātnes komisija — the bill's subject must still win.
        motif = "Par nodošanu Izglītības, kultūras un zinātnes komisijai. Par vēstures prioritāru mācīšanu (1104/Lm14)"
        assert _motif_to_topic(motif) == "Izglītība"
