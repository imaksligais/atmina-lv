...
class TestGetPoliticianDocuments:
    ...
    def test_truncation_is_reported_not_silent(self, analyze_db, capsys):
        """BACKLOG 2026-08-03 (LB pid=243): the queue held 21 docs, the cut
        21st was the operator's top priority, and nothing signalled the loss.
        When the queue exceeds max_results, every returned doc must carry
        queue_total and stderr must name what was cut."""
        with patch("src.analyze.get_db", lambda: get_db(analyze_db)):
            from src.analyze import get_politician_documents
            docs = get_politician_documents(1, days=1, max_results=1)
        assert len(docs) == 1
        assert docs[0]["queue_total"] == 2
        err = capsys.readouterr().err
        assert "max_results" in err

    def test_no_truncation_no_marker(self, analyze_db, capsys):
        """The marker is conditional on purpose — its presence MEANS loss."""
        with patch("src.analyze.get_db", lambda: get_db(analyze_db)):
            from src.analyze import get_politician_documents
            docs = get_politician_documents(1, days=1)
        assert len(docs) == 2
        assert all("queue_total" not in d for d in docs)
            ...
            claims=[
                {
                    ...
                    "stance": "Atbalsta aizsardzības budžeta palielināšanu līdz 5 % no IKP",
                    "confidence": 0.5,
                    "reasoning": "ok",
                    "salience": 0.5,
                },
                {
                    "document_id": 2,
                    "topic": "Aizsardzība un drošība",
                    "stance": "Iebilst pret valsts aizsardzības dienesta termiņa pagarināšanu",
                    "confidence": 0.5,
                    "reasoning": "ok",
                    "salience": 0.5,
                },
...
class TestRelayOrganizationsLeaveTheQueue:
    """Relay news slots never hold a first-party position — keep them out.

    Measured 2026-08-02: 11 entities that are `relationship_type='organization'`
    AND carry a `feed_type='relay'` social account held 2267 `role='subject'`
    documents and produced **0 claims, ever, of any claim_type**. LETA alone
    topped the pending queue in every window (310 docs) and burned 141
    `analyses` passes on guaranteed-empty content.

    The narrow AND is the whole point: filtering on `organization` alone would
    also silence LDDK, NBS, LVM and Valsts kontrole — institutions whose
    positions are the reason they were seeded, and which briefs.py's audience
    filter already drops from the topic tables.
    """

    @staticmethod
    def _seed(path, *, pid, name, rel_type, feed_type):
        db = get_db(path)
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        db.execute(
        ...
        if feed_type:
            db.execute(
                ...
                "VALUES (?, 'twitter', ?, ?)",
                (pid, f"h{pid}", feed_type),
            )
        db.commit()
        db.close()

    def test_relay_organization_is_not_proposed(self, analyze_db):
        self._seed(analyze_db, pid=195, name="LETA", rel_type="organization",
                   feed_type="relay")
        with patch("src.analyze.get_db", lambda: get_db(analyze_db)):
            from src.analyze import get_pending_politicians
            names = [p["name"] for p in get_pending_politicians(days=1)]
            assert "LETA" not in names, names

    def test_first_party_organization_still_appears(self, analyze_db):
        """The case the broad filter would have broken."""
        self._seed(analyze_db, pid=241, name="Valsts kontrole",
                   rel_type="organization", feed_type="first_party")
        with patch("src.analyze.get_db", lambda: get_db(analyze_db)):
            from src.analyze import get_pending_politicians
            names = [p["name"] for p in get_pending_politicians(days=1)]
            assert "Valsts kontrole" in names, names

    def test_organization_without_a_social_account_still_appears(self, analyze_db):
        self._seed(analyze_db, pid=242, name="Bez konta",
                   rel_type="organization", feed_type=None)
        with patch("src.analyze.get_db", lambda: get_db(analyze_db)):
            from src.analyze import get_pending_politicians
            names = [p["name"] for p in get_pending_politicians(days=1)]
            assert "Bez konta" in names, names

    def test_relay_person_is_untouched(self, analyze_db):
        """Only `organization` + relay is excluded — a relay feed on a tracked
        person (a politician whose tweets arrive via a relay account) must not
        be swept in by the same clause."""
        self._seed(analyze_db, pid=243, name="Relay Persona",
                   rel_type="tracked", feed_type="relay")
        with patch("src.analyze.get_db", lambda: get_db(analyze_db)):
            from src.analyze import get_pending_politicians
            names = [p["name"] for p in get_pending_politicians(days=1)]
            assert "Relay Persona" in names, names

    def test_explicit_access_still_reaches_a_relay_slot(self, analyze_db):
        """get_politician_documents is deliberately NOT filtered — nothing
        becomes unreachable, it only stops being proposed."""
        self._seed(analyze_db, pid=196, name="TV3 Ziņas",
                   rel_type="organization", feed_type="relay")
        with patch("src.analyze.get_db", lambda: get_db(analyze_db)):
            from src.analyze import get_politician_documents
            docs = get_politician_documents(196, days=1)
