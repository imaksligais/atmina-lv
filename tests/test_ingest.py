...
class TestEnrichRssItemsFulltext:
    ...
    def _run(self, items, monkeypatch, *, fetch_returns=None, extract_returns=None):
        ...
        async def _fake_fetch_page(client, url, **_kw):
    ...
    # -- 2026-09-23: trafilatura drops the lede -------------------------------
    # On lsm.lv/diena.lv the extracted body routinely omits the article's first
    # paragraph (7 of 8 checked docs). When that happens the lede must be kept
    # in front of the body — the same rule
    # scripts/backfill_truncated_docs.py::_compose applies to the backfill —
    # not silently replaced away.

    def test_dropped_lede_is_prepended_not_lost(self, monkeypatch):
        lede = "Apdraudējuma līmenis Latvijā un reģionā nav mainījies, pauž NBS."
        full = "Pudāns uzsver, ka draudi joprojām pastāv." + " Teksts." * 30
        items = [{"text": lede, "url": self.ARTICLE, "published_at": None}]
        out = self._run(items, monkeypatch,
                        fetch_returns="<html>body</html>", extract_returns=full)
        assert out[0]["text"] == lede + "\n\n" + full
        assert out[0]["lede_prepended"] is True

    def test_lede_already_in_fulltext_is_not_duplicated(self, monkeypatch):
        # Containment is normalized (html.unescape, whitespace-collapsed,
        # case-insensitive): a re-paragraphed lede still counts as present.
        lede = "Saeima šodien pieņēma budžetu."
        full = "Saeima   šodien\n\npieņēma   BUDŽETU." + " Vairāk." * 30
        items = [{"text": lede, "url": self.ARTICLE, "published_at": None}]
        out = self._run(items, monkeypatch,
                        fetch_returns="<html>body</html>", extract_returns=full)
        assert out[0]["text"] == full
        assert not out[0].get("lede_prepended")

    def test_prepended_lede_is_html_unescaped(self, monkeypatch):
        lede = "Viņš&nbsp;teica: &quot;Sveiki&quot;."
        full = "Pilns raksta teksts bez ievada." + " Teksts." * 30
        items = [{"text": lede, "url": self.ARTICLE, "published_at": None}]
        out = self._run(items, monkeypatch,
                        fetch_returns="<html>b</html>", extract_returns=full)
        assert "&nbsp;" not in out[0]["text"]
        assert "&quot;" not in out[0]["text"]
        assert out[0]["text"].startswith('Viņš teica: "Sveiki".')

    def test_empty_lede_stores_fulltext_only(self, monkeypatch):
        full = "Pilns raksta teksts." + " Teksts." * 30
        items = [{"text": "", "url": self.ARTICLE, "published_at": None}]
        out = self._run(items, monkeypatch,
                        fetch_returns="<html>b</html>", extract_returns=full)
        assert out[0]["text"] == full
        assert not out[0].get("lede_prepended")

    def test_lede_present_without_title_is_not_duplicated(self, monkeypatch):
        # RSS items are "title — description". The probe covers the
        # description part (trafilatura keeps the headline, drops the lede) —
        # a body that carries the description but not the headline is NOT
        # lede-missing; the title lives on in item["title"].
        lede = "Kāds virsraksts — Ministrs paziņoja par jaunu programmu šodien."
        full = "Ministrs paziņoja par jaunu programmu šodien." + " Vairāk." * 30
        items = [{"text": lede, "url": self.ARTICLE, "published_at": None}]
        out = self._run(items, monkeypatch,
                        fetch_returns="<html>body</html>", extract_returns=full)
        assert out[0]["text"] == full
...
# ---------------------------------------------------------------------------
# 2026-07-28 — consecutive duplicate path segments collapse in crawl discovery
# ("/neatkariga/neatkariga/izpete/…" nra.lv relative-href artifact ingested the
# same article twice under two URLs; store_claim idempotence keys on source_url,
# so the twins would double-store claims. BACKLOG § nra.lv dublēti ingesti.)
# ---------------------------------------------------------------------------

def test_site_links_collapse_duplicate_path_segments():
    from src.ingest import _extract_site_article_links

    html = (
        '<a href="https://nra.lv/neatkariga/neatkariga/izpete/'
        '526354-tramvaju-iepirkuma-drama-skoda-santaze.htm">x</a>'
        '<a href="https://nra.lv/neatkariga/izpete/'
        '526354-tramvaju-iepirkuma-drama-skoda-santaze.htm">y</a>'
    )
    links = _extract_site_article_links(html, "https://nra.lv/")
    assert links == [
        "https://nra.lv/neatkariga/izpete/"
        "526354-tramvaju-iepirkuma-drama-skoda-santaze.htm"
    ], "doubled segment must collapse AND both hrefs must dedupe to one URL"


def test_site_links_normal_paths_untouched():
    from src.ingest import _extract_site_article_links

    html = (
        '<a href="https://nra.lv/neatkariga/lasamgabali/'
        '523907-saeimas-priekssedetajas-vasaras-velejums.htm">x</a>'
    )
    links = _extract_site_article_links(html, "https://nra.lv/")
    assert links == [
        "https://nra.lv/neatkariga/lasamgabali/"
        "523907-saeimas-priekssedetajas-vasaras-velejums.htm"
    ]


# --- 2026-09-05: `lstrip("www.")` strips a character SET, not a prefix -------
from src.ingest import _is_relevant_section as _irs  # noqa: E402


@pytest.mark.parametrize("url", [
    "https://www.x.com/someone/status/1",
    "https://x.com/someone/status/1",
    "https://www.twitter.com/someone/status/1",
])
def test_social_domain_detected_with_and_without_www(url):
    assert _irs(url) is True


def test_www_prefix_removal_does_not_eat_leading_w(monkeypatch):
    """`"www.wp.lv".lstrip("www.")` → "p.lv" (the 2026-09-05 bug); removeprefix
    keeps "wp.lv". The stripped domain only matters when it is compared against
    _SOCIAL_DOMAINS, and no real entry starts with "w" — so the old version of
    this test passed with the bug put back (mutation sample 2026-09-24). Plant a
    w-domain in the set and use a URL the section rules would otherwise reject
    (3+ non-politics segments): only a correct strip keeps it relevant."""
    import src.ingest_rules as rules
    monkeypatch.setattr(rules, "_SOCIAL_DOMAINS", rules._SOCIAL_DOMAINS | {"wp.lv"})
    url = "https://www.wp.lv/sports/futbols/raksts"
    assert _irs(url) is True
