...
3. ``_rejected_brief_stems()`` + ``skip_stems`` KOPĒŠANAS pusē — noraidītais
   pārskata plakāts nedrīkst nonākt publiskajā kokā. Vecā forma vaicāja tikai
   ``approved = 2`` un padeva rezultātu tikai ģenerēšanas solim, tāpēc
   2026-08-16 mērījums atrada 27 noraidītus plakātus (108 faili / 5,44 MB) dzīvi
   serverī, 0 no tiem atsaukti kādā no 916 uzbūvētajām lapām. Additīvais deploy
   tos vairs nevar noņemt, tāpēc vārtam jānostrādā PIRMS sūtīšanas.
"""
...
# ── 2b. Noraidīto plakātu vārts (2026-08-16) ────────────────────────────────

def test_is_variant_of_matches_by_prefix_not_equality():
    """Diskā guļ ``<stem>-og.jpg``, tāpēc pilnas sakritības filtrs neko nedarītu."""
    from src.render._orchestrator import _is_variant_of

    base = {"2026-08-15-dienas-parskats-11fe6159"}
    assert _is_variant_of("2026-08-15-dienas-parskats-11fe6159-og", base)
    assert _is_variant_of("2026-08-15-dienas-parskats-11fe6159-hero", base)
    assert _is_variant_of("2026-08-15-dienas-parskats-11fe6159", base)
    # Cits hash ar to pašu prefiksu NEDRĪKST tikt notverts — tāpēc atdalītājs.
    assert not _is_variant_of("2026-08-15-dienas-parskats-11fe6159x-og", base)
    assert not _is_variant_of("2026-08-15-dienas-parskats-97cd89d3-og", base)


def test_copy_brief_images_skips_rejected_variants(tmp_path):
    """Vārts sedz KOPĒŠANU, ne tikai ģenerēšanu — tā bija 08-16 sakne."""
    from src.render._orchestrator import _copy_brief_images
    ...
    dest = tmp_path / "dest"
    dest.mkdir()
    for stem in ("good-97cd89d3", "bad-11fe6159"):
        for suf in ("-hero.webp", "-card.webp", "-thumb.webp", "-og.jpg"):
            (src / f"{stem}{suf}").write_bytes(b"x")

    n = _copy_brief_images(src, dest, skip_stems={"bad-11fe6159"})

    names = sorted(p.name for p in dest.iterdir())
    assert n == 4, "tikai apstiprinātā komplekts"
    assert all("bad-11fe6159" not in x for x in names), names
    assert names == [
        "good-97cd89d3-card.webp",
        "good-97cd89d3-hero.webp",
        "good-97cd89d3-og.jpg",
        "good-97cd89d3-thumb.webp",
    ]


def test_copy_brief_images_empty_skip_stem_does_not_block_everything(tmp_path):
    """Tukšs ``image_path`` -> stem "" -> prefiksa filtrs apēstu VISU.

    Divas dzīvas rindas (id 54, 72) ir tieši tādas, tāpēc šis ir vārta
    bīstamākais nepareizais variants, ne teorētisks gadījums.
    """
    from src.render._orchestrator import _copy_brief_images
    ...
    dest = tmp_path / "dest"
    dest.mkdir()
    (src / "brief-1-og.jpg").write_bytes(b"x")
    (src / "brief-2-hero.webp").write_bytes(b"x")

    # `_rejected_brief_stems` tukšos izmet; ja kāds tos tomēr padod, filtrs
    # nedrīkst tos uztvert kā "sakrīt ar visu".
    assert _copy_brief_images(src, dest, skip_stems={""}) == 2


def test_rejected_brief_stems_covers_all_three_encodings(tmp_path):
    """Trīs kodējumi vienai nozīmei + saucējs, ka vaicājums tiešām kaut ko skatīja."""
    from src.db import get_db, init_db
    from src.render._orchestrator import _rejected_brief_stems

    db_path = str(tmp_path / "img.db")
    init_db(db_path)
    db = get_db(db_path)
    db.execute(
        "INSERT INTO context_notes (id, opponent_id, note_type, topic, content) "
        "VALUES (1, NULL, 'daily_brief', 'dienas analīze 2026-08-15', 'x')"
    )
    db.execute(
        "INSERT INTO context_notes (id, opponent_id, note_type, topic, content) "
        "VALUES (2, NULL, 'daily_brief', 'dienas analīze 2026-08-16', 'y')"
    )
    rows = [
        # (id, note_id, image_path, approved)
        (1, 1, "images/briefs/approved-a.png", 1),      # apstiprinātais
        (2, 1, "images/briefs/unapproved-b.png", 0),    # 1->0 ar brāli  -> NORAIDĪTS
        (3, 1, "images/briefs/superseded-c.png", -1),   # aizstāts        -> NORAIDĪTS
        (4, 1, "images/briefs/cancelled-d.png", 2),     # atcelts         -> NORAIDĪTS
        (5, 2, "images/briefs/pending-e.png", 0),       # 0 BEZ brāļa     -> PALIEK
        (6, 2, "", 2),                                  # tukšs ceļš      -> IZMESTS
    ]
    for r in rows:
        db.execute(
            "INSERT INTO brief_images (id, note_id, image_path, prompt, model, "
            "generated_at, approved) VALUES (?, ?, ?, 'p', 'm', '2026-08-16', ?)",
            r,
        )
    db.commit()
    db.close()

    stems = _rejected_brief_stems(db_path)

    assert stems == {"unapproved-b", "superseded-c", "cancelled-d"}, stems
    # Saucējs: vārts, kas neko neatrod, nav pierādījums — fiksē abus galus.
    assert len(stems) == 3, "trīs kodējumi jāsedz visi"
    assert "pending-e" not in stems, (
        "approved=0 BEZ apstiprināta brāļa ir kandidāts, ne noraidījums — "
        "to izlaist nozīmētu nekad nepublicēt gaidošu attēlu"
    )
    assert "" not in stems, "tukšs image_path sakristu ar KATRU failu"


def test_rejected_brief_stems_unreadable_db_fails_open(tmp_path):
    """DB kļūme atkāpjas uz 'sūta visu', ne uz 'nesūta neko'."""
    from src.render._orchestrator import _rejected_brief_stems

    assert _rejected_brief_stems(str(tmp_path / "nav.db")) == frozenset()
