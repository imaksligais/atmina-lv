...
def memdb():
    ...
    """)
    # 2026-09-07 (verdikts 47b): mēneša budžeta saucējs pārcēlās uz image_audit,
    # jo brief_images rindu raksta tikai `cli brief` — pavediena un tiešie
    # generate_image() izsaukumi tur neparādās nekad.
    db.execute("""
...
def test_monthly_cost_usd_sums_current_month(memdb):
    ...
    memdb.execute(
        ...
        "(generated_at, kind, model, aspect, prompt, cost_usd) "
        "VALUES (?, 'brief', 'm', '16:9', 'p', 0.04)",
        (this_month + "-10 12:00:00",),
    )
    memdb.execute(
        "INSERT INTO image_audit "
        "(generated_at, kind, model, aspect, prompt, cost_usd) "
        "VALUES (?, 'thread', 'm', '16:9', 'p', 0.05)",
...
def test_monthly_cost_usd_excludes_other_months(memdb):
    memdb.execute(
        "INSERT INTO image_audit "
        "(generated_at, kind, model, aspect, prompt, cost_usd) "
        "VALUES ('2025-01-01 00:00:00', 'brief', 'm', '16:9', 'p', 0.10)"
    )
    memdb.commit()
    total = storage.monthly_cost_usd(memdb)
    assert total == 0.0
...
def test_monthly_cost_usd_uses_year_month_prefix_format(memdb, monkeypatch):
    ...
    memdb.execute(
        "INSERT INTO image_audit "
        "(generated_at, kind, model, aspect, prompt, cost_usd) "
        "VALUES ('2026-04-01 08:00:00', 'brief', 'm', '16:9', 'p', 0.07)"
    )
    memdb.execute(
        "INSERT INTO image_audit "
        "(generated_at, kind, model, aspect, prompt, cost_usd) "
        "VALUES ('2026-05-01 08:00:00', 'brief', 'm', '16:9', 'p', 0.11)"
    )
    memdb.commit()
...
class TestLinkAuditRow:
    """`link_audit_row()` stamps the `generate_image()` audit row with the
    `brief_images` id so the migration backfill's NOT EXISTS guard sees it.

    Regresija (2026-09-15): CLI ceļš rakstīja audita rindu ar `source_id NULL`
    un tad `brief_images` rindu; nākamais `apply_migrations()` backfill to
    neredzēja un pielika OTRU audita rindu — 13 pāri septembrī, 0,507 USD
    dubulti mēneša budžetā (`data/fix_image_audit_dedup_2026-09-15.sql`).
    """

    def _audit(self, db, prompt, kind="brief", at="2026-09-15 10:00:00"):
        return storage.save_audit_row(db, kind=kind, model="m", prompt=prompt) if at is None else db.execute(
            "INSERT INTO image_audit (generated_at, kind, model, aspect, prompt, cost_usd) "
            "VALUES (?, ?, 'm', '16:9', ?, 0.039)", (at, kind, prompt)).lastrowid

    def test_links_latest_unlinked_row_with_same_prompt(self, memdb):
        older = self._audit(memdb, "P", at="2026-09-15 09:00:00")
        newest = self._audit(memdb, "P", at="2026-09-15 10:00:00")
        other = self._audit(memdb, "Q", at="2026-09-15 10:30:00")
        memdb.commit()
        linked = storage.link_audit_row(memdb, brief_image_id=42, prompt="P")
        assert linked == newest
        rows = {r[0]: (r[1], r[2]) for r in memdb.execute(
            "SELECT id, source_table, source_id FROM image_audit")}
        assert rows[newest] == ("brief_images", 42)
        assert rows[older] == (None, None)
        assert rows[other] == (None, None)

    def test_already_linked_row_is_not_restamped(self, memdb):
        rid = self._audit(memdb, "P")
        memdb.execute("UPDATE image_audit SET source_table='brief_images', source_id=7 WHERE id=?", (rid,))
        memdb.commit()
        assert storage.link_audit_row(memdb, brief_image_id=8, prompt="P") is None
        assert memdb.execute("SELECT source_id FROM image_audit WHERE id=?", (rid,)).fetchone()[0] == 7

    def test_no_matching_row_returns_none_and_writes_nothing(self, memdb):
        assert storage.link_audit_row(memdb, brief_image_id=1, prompt="missing") is None
        assert memdb.execute("SELECT COUNT(*) FROM image_audit").fetchone()[0] == 0

    def test_backfill_guard_sees_the_link(self, memdb):
        """The migration backfill inserts from brief_images WHERE NOT EXISTS
        (audit row with that source) — after linking, it must add nothing."""
        memdb.execute(
            "INSERT INTO brief_images (id, note_id, image_path, prompt, model, generated_at) "
            "VALUES (5, 1, 'images/briefs/x.png', 'P', 'm', '2026-09-15 10:00:00')")
        self._audit(memdb, "P")
        memdb.commit()
        storage.link_audit_row(memdb, brief_image_id=5, prompt="P")
        missing = memdb.execute(
            "SELECT COUNT(*) FROM brief_images bi WHERE NOT EXISTS ("
            "SELECT 1 FROM image_audit ia WHERE ia.source_table='brief_images' AND ia.source_id=bi.id)"
        ).fetchone()[0]
        assert missing == 0
