...
def test_audit_flags_claim_with_mismatched_author(tmp_db):
    ...
    suspects, _scanned = find_suspect_claims(tmp_db)
...
def test_audit_reports_scanned_denominator(tmp_db):
    """The denominator counts every twitter-sourced position claim examined."""
    from scripts.audit_claim_attribution import find_suspect_claims
    ...
    for i in (1, 2, 3):
        tmp_db.execute(
            ...
            VALUES (45, 'Degviela', 'Own stance', 'own quote', 0.8, 0.5,
                    ?, '2026-04-18', 'position')
            """,
            (f"https://x.com/krusts/status/{i}",),
        )
    # A non-twitter claim is NOT part of this audit's denominator.
    tmp_db.execute(
        """
        INSERT INTO claims (opponent_id, topic, stance, quote, confidence, salience,
                            source_url, stated_at, claim_type)
        VALUES (45, 'Ekonomika', 'web', 'quote', 0.7, 0.5,
                'https://www.la.lv/par-partiku', '2026-04-18', 'position')
        """
    )
    tmp_db.commit()

    suspects, scanned = find_suspect_claims(tmp_db)
    assert scanned == 3
    assert suspects == []
