...
class TestSecondLaneIsNotWebOnly:
    """The lane must see relayed TWEETS, not just web articles (2026-08-27).

    Cost of the web-only filter, measured: on 2026-08-26 the LTV1 pre-election
    leaders' debate was relayed in tweets with explicit attribution ("Juris Pūce
    (LA) aicina palielināt iemaksas 2. pensiju līmenī"). Eight documents
    (95235, 95236, 95238, 95241, 95243, 95245, 95246, 95247) carried
    `role='mentioned'` on tracked politicians, `reviewed_at IS NULL`, and
    produced **0 claims** — on the day of the debate the published brief recorded
    only the one leader who happened to retweet a tweet about himself.

    Three gates missed it and two shared one blind spot: `get_pending_politicians()`
    walks `role='subject'`, this detector filtered `platform='web'`, and the
    orchestrator's survey query repeated the same `platform='web'` filter. Two
    gates with one blind spot are not two gates.
    """

    RELAY_TWEET = (
        "Juris Pūce (LA) aicina palielināt iemaksas 2. pensiju līmenī, "
        "savukārt Svetlana Čulkova uzskata pretējo."
    )

    def _add_tweet(self, path, doc_id=20):
        db = get_db(path)
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        db.execute(
            "INSERT INTO documents (id, content, content_hash, scraped_at, "
            "platform, source_url) VALUES (?, ?, ?, ?, 'twitter', ?)",
            (doc_id, CONTENT_10, f"h{doc_id}", now, "https://x.com/relay/status/1"),
        )
    ...
    def test_relayed_tweet_reaches_the_second_lane(self, lane_db):
        self._add_tweet(lane_db)
        db = get_db(lane_db)
        pairs = pending_quoted_mentioned(db, days=3)
        db.close()
        assert (20, 2) in {(p["document_id"], p["politician_id"]) for p in pairs}, (
            "an attributed statement relayed in a tweet must enter the second lane"
        )

    def test_denominator_counts_the_tweet_too(self, lane_db):
        """checked=2 after adding a third candidate would be the silent half."""
        self._add_tweet(lane_db)
        db = get_db(lane_db)
        result = find_inversions(db, days=3)
        db.close()
        assert result["checked"] == 3, result["checked"]

    def test_platform_travels_with_the_finding(self, lane_db):
        """Triage needs the lane: satire dominates false positives on the tweet side."""
        self._add_tweet(lane_db)
        db = get_db(lane_db)
        result = find_inversions(db, days=3)
        db.close()
        by_doc = {inv["document_id"]: inv["platform"] for inv in result["inversions"]}
        assert by_doc[20] == "twitter", by_doc
        assert by_doc[10] == "web", by_doc

    def test_platforms_argument_still_narrows(self, lane_db):
        """Explicit narrowing stays available — the default is what changed."""
        self._add_tweet(lane_db)
        db = get_db(lane_db)
        result = find_inversions(db, days=3, platforms=("web",))
        db.close()
        assert result["checked"] == 2
        assert 20 not in {inv["document_id"] for inv in result["inversions"]}
