...
class TestCheckAnalysis:
    ...
    def test_tz_shift_mandatory_regression(self, empty_routine_db):
        ...
        # Raw UTC compare (no UTC→LV shift): doc 07:00 > analysis 05:00 → would
...
class TestUnreviewedSubjectDocsBeforeTheDay:
    """Old subject docs that never entered the queue must be visible somewhere.

    `get_pending_politicians(days=1)` walks a scrape-time window, so a document
    that GAINS a `subject` slot after its scrape day — reactivation (Štekerhofs,
    doc 90283, 2026-09-07), a junction re-attribution, a `feed_type` change —
    stays `reviewed_at IS NULL` outside every future window. Measured
    2026-09-23: 3 such docs from August–September (86399, 95719, 100458) next
    to 595 pre-August historic backlog docs. This gate does not queue them; it
    lists them with a denominator, like the Vēstnesis acts line.
    """

    DAY = "2026-09-23"

    def _setup(self, db):
        db.execute("INSERT INTO tracked_politicians (id, name, party) VALUES (1, 'A Politiķis', 'TP')")
        db.execute(
            "INSERT INTO tracked_politicians (id, name, party, relationship_type) "
            "VALUES (2, 'Neaktīvs', 'TP', 'inactive')"
        )

    def _doc(self, db, doc_id, scraped, *, pid=1, role="subject", platform="web", reviewed=None):
        db.execute(
            """INSERT INTO documents (id, content, content_hash, platform, scraped_at, reviewed_at)
               VALUES (?, 'saturs', ?, ?, ?, ?)""",
            (doc_id, f"h{doc_id}", platform, scraped, reviewed),
        )
        db.execute(
    ...
    def test_old_unreviewed_subject_doc_is_listed(self, empty_routine_db):
        from src.routine import unreviewed_subject_docs_before
        db = get_db(empty_routine_db)
        self._setup(db)
        self._doc(db, 95719, "2026-08-27 20:37:42")
        self._doc(db, 95720, "2026-08-28 10:00:00", reviewed="2026-08-28 18:00:00")
        result = unreviewed_subject_docs_before(db, self.DAY)
        db.close()
        assert result["total"] == 2, result
        assert [d["id"] for d in result["docs"]] == [95719], result
        assert result["docs"][0]["name"] == "A Politiķis"

    def test_today_and_out_of_lookback_are_excluded(self, empty_routine_db):
        """Today is the queue's job; older than the lookback is the historic sweep."""
        from src.routine import unreviewed_subject_docs_before
        db = get_db(empty_routine_db)
        self._setup(db)
        self._doc(db, 1, f"{self.DAY} 09:00:00")
        self._doc(db, 2, "2026-07-01 09:00:00")
        self._doc(db, 3, "2026-09-22 23:59:59")
        result = unreviewed_subject_docs_before(db, self.DAY, lookback_days=30)
        db.close()
        assert [d["id"] for d in result["docs"]] == [3], result
        assert result["total"] == 1

    def test_scope_matches_the_queue(self, empty_routine_db):
        """Same exclusions as get_pending_politicians: vestnesis, non-subject, inactive."""
        from src.routine import unreviewed_subject_docs_before
        db = get_db(empty_routine_db)
        self._setup(db)
        self._doc(db, 1, "2026-09-10 09:00:00", platform="vestnesis")
        self._doc(db, 2, "2026-09-10 09:00:00", role="mentioned")
        self._doc(db, 3, "2026-09-10 09:00:00", pid=2)
        result = unreviewed_subject_docs_before(db, self.DAY)
        db.close()
        assert result["docs"] == [], result
        assert result["total"] == 0

    def test_denominator_is_reported_when_nothing_is_left(self, empty_routine_db):
        from src.routine import unreviewed_subject_docs_before
        db = get_db(empty_routine_db)
        self._setup(db)
        self._doc(db, 1, "2026-09-10 09:00:00", reviewed="2026-09-10 18:00:00")
        result = unreviewed_subject_docs_before(db, self.DAY)
        db.close()
        assert result["total"] == 1, result
        assert result["docs"] == []
...
class TestDeployStep:
    """Solis 11 «Deploy»: rutīna nedrīkst būt zaļa, ja nekas netika publicēts.

    `logs.action='deploy'` raksta `scripts/deploy.sh` pēc īsta wrangler deploy
    (`timestamp` = LV, `now_lv()`); pārskata atļauja ir `publish_approvals`
    (atslēga = slugs = subjekta datums, `approved_at` LV). Logs = rutīnas diena
    (`routine_day_window`), ne kalendārā diena.
    """

    DAY = "2026-04-07"

    @staticmethod
    def _approve(db, slug, ts):
        db.execute(
    ...
    def _check(self, path):
        from src.routine import _check_deploy
        db = get_db(path)
        try:
            return _check_deploy(db, self.DAY)
        finally:
            db.close()

    def test_na_without_deploy_or_approval(self, empty_routine_db):
        res = self._check(empty_routine_db)
        assert res["status"] == "n/a", res
        assert "deploy" in res["details"].lower()

    def test_missing_when_brief_approved_but_never_deployed(self, empty_routine_db):
        db = get_db(empty_routine_db)
        self._approve(db, self.DAY, "2026-04-07 22:00:00")
        # Iepriekšējās dienas deploy nesedz šodienas pārskatu.
        _log(db, "deploy", "2026-04-06 23:00:00", details={"version_id": "old-v"})
        db.close()
        res = self._check(empty_routine_db)
        assert res["status"] == "missing", res
        assert "old-v" in res["details"]

    def test_done_when_deploy_after_approval(self, empty_routine_db):
        db = get_db(empty_routine_db)
        self._approve(db, self.DAY, "2026-04-07 22:00:00")
        _log(db, "deploy", "2026-04-07 22:10:00",
             details={"version_id": "v-123", "approved_slugs": [self.DAY]})
        db.close()
        res = self._check(empty_routine_db)
        assert res["status"] == "done", res
        assert "v-123" in res["details"] and "22:10" in res["details"]

    def test_deploy_after_approval_without_the_slug_is_not_done(self, empty_routine_db):
        """Deploy pēc atļaujas, bet kokā nebija pārskata lapas → pārskats NAV dzīvs."""
        db = get_db(empty_routine_db)
        self._approve(db, self.DAY, "2026-04-07 22:00:00")
        _log(db, "deploy", "2026-04-07 22:10:00",
             details={"version_id": "v-noslug", "approved_slugs": ["2026-04-06"]})
        # Vecs ieraksts bez approved_slugs arī nepierāda, ka lapa aizgāja.
        _log(db, "deploy", "2026-04-07 22:20:00", details={"version_id": "v-legacy"})
        db.close()
        res = self._check(empty_routine_db)
        assert res["status"] == "stale", res
        assert "nebija nevienā izvietotajā kokā" in res["details"]

    def test_later_deploy_without_the_slug_is_stale(self, empty_routine_db):
        """Deploy nesa D, bet vēlāks deploy (pilns koks) vairs nenesa → lapa nav live."""
        db = get_db(empty_routine_db)
        self._approve(db, self.DAY, "2026-04-07 22:00:00")
        _log(db, "deploy", "2026-04-07 22:10:00",
             details={"version_id": "v-with", "approved_slugs": [self.DAY]})
        _log(db, "deploy", "2026-04-08 11:00:00",
             details={"version_id": "v-without", "approved_slugs": ["2026-04-06"]})
        db.close()
        res = self._check(empty_routine_db)
        assert res["status"] == "stale", res
        assert f"nesatur {self.DAY}" in res["details"]
        assert "v-without" in res["details"]

    def test_next_day_deploy_carrying_the_slug_is_done(self, empty_routine_db):
        """Apstiprināts D, izdeployots D+1 dienā (ārpus loga) ar slugu → D ir done."""
        db = get_db(empty_routine_db)
        self._approve(db, self.DAY, "2026-04-07 22:00:00")
        _log(db, "deploy", "2026-04-08 10:00:00",
             details={"version_id": "v-next", "approved_slugs": ["2026-04-06", self.DAY]})
        db.close()
        res = self._check(empty_routine_db)
        assert res["status"] == "done", res
        assert "v-next" in res["details"]

    def test_stale_when_approval_newer_than_deploy(self, empty_routine_db):
        db = get_db(empty_routine_db)
        _log(db, "deploy", "2026-04-07 16:00:00", details={"version_id": "v-early"})
        self._approve(db, self.DAY, "2026-04-07 22:00:00")
        db.close()
        res = self._check(empty_routine_db)
        assert res["status"] == "stale", res
        assert "v-early" in res["details"]

    def test_stale_when_data_written_after_deploy(self, empty_routine_db):
        db = get_db(empty_routine_db)
        _log(db, "deploy", "2026-04-07 16:00:00", details={"version_id": "v-early"})
        db.execute(
            "INSERT INTO context_notes (note_type, content, topic, created_at) "
            "VALUES ('context', 'Tendence', 'NATO', '2026-04-07 21:00:00')"
        )
        db.commit()
        db.close()
        res = self._check(empty_routine_db)
    ...
    def test_position_after_deploy_is_stale_vote_claim_is_not(self, empty_routine_db):
        db = get_db(empty_routine_db)
        _log(db, "deploy", "2026-04-07 16:00:00", details={"version_id": "v"})
        db.execute("INSERT INTO tracked_politicians (id, name, party) VALUES (1, 'P', 'TP')")
        db.execute(
            "INSERT INTO claims (opponent_id, topic, stance, stated_at, created_at, claim_type) "
            "VALUES (1, 'NATO', 'Balsoja', '2026-04-07', '2026-04-07 20:00:00', 'saeima_vote')"
        )
        db.commit()
        assert self._check(empty_routine_db)["status"] == "done"
        db.execute(
            "INSERT INTO claims (opponent_id, topic, stance, stated_at, created_at, claim_type) "
            "VALUES (1, 'NATO', 'Atbalsta', '2026-04-07', '2026-04-07 21:00:00', 'position')"
        )
        db.commit()
        db.close()
        assert self._check(empty_routine_db)["status"] == "stale"

    def test_next_days_data_does_not_stale_this_day(self, empty_routine_db):
        db = get_db(empty_routine_db)
        _log(db, "deploy", "2026-04-07 23:00:00", details={"version_id": "v"})
        db.execute(
            "INSERT INTO context_notes (note_type, content, topic, created_at) "
            "VALUES ('context', 'Tendence', 'NATO', '2026-04-08 14:00:00')"
        )
        db.commit()
        db.close()
        assert self._check(empty_routine_db)["status"] == "done"

    def test_after_midnight_deploy_counts_for_previous_routine_day(self, empty_routine_db):
        """Deploy 00:40 LV nākamajā kalendārajā dienā = iepriekšējās rutīnas dienas darbs."""
        db = get_db(empty_routine_db)
        self._approve(db, self.DAY, "2026-04-08 00:20:00")
        _log(db, "deploy", "2026-04-08 00:40:00",
             details={"version_id": "v-night", "approved_slugs": [self.DAY]})
        db.close()
        res = self._check(empty_routine_db)
        assert res["status"] == "done", res
        from src.routine import _check_deploy
        db = get_db(empty_routine_db)
        try:
            # ...un nākamajai rutīnas dienai tas NEskaitās.
            assert _check_deploy(db, "2026-04-08")["status"] == "n/a"
        finally:
            db.close()

    def test_missing_deploy_blocks_completion_na_does_not(self, empty_routine_db):
        db = get_db(empty_routine_db)
        self._approve(db, self.DAY, "2026-04-07 22:00:00")
        db.close()
        res = check_routine(self.DAY, db_path=empty_routine_db,
                            now=datetime(2026, 4, 7, 23, 0))
        assert res["steps"]["deploy"]["status"] == "missing"
