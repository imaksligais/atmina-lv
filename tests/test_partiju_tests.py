...
def test_render_uses_fixture_counts_not_hardcoded_12():
    ...
    # pp_stats padots: bez tā build() lasa ražošanas DB, kuras publiskajā CI nav
    # (2026-09-23 CI sarkans — `no such table: claims`).
    html = ptr.build(questions=questions, coding=coding, parties=parties,
                     saraksti=saraksti, og_image_exists=False, pp_stats=(7, 2))
    assert "3 apgalvojumi" in html
    assert "12 apgalvojumi" not in html
    assert "no 3<" in html or "no 3 " in html


def test_render_methodology_numbers_come_from_data():
    """B2.1: metodoloģijas skaitļi (pozīcijas, tēmas) nāk no datiem, nevis literāļi;
    avotu līmeņi ir trīs, nevis divi — atgriezts literālis padara testu sarkanu."""
    questions, coding, parties, saraksti = _render_fixture(3)
    html = ptr.build(questions=questions, coding=coding, parties=parties,
                     saraksti=saraksti, og_image_exists=False, pp_stats=(7, 2))
    assert "7 pozīcijās 2 tēmās" in html
    assert "276 pozīcijās" not in html and "31 tēmā" not in html
    assert "divi dokumenti" not in html and "no abiem" not in html
    assert "neviens no abiem" not in html
    assert "neviens no trim avotiem" in html


def test_render_fullread_sentence_comes_from_flags():
    """C3: teikums par pilnībā pārlasītajām programmām nāk no saraksti.yaml
    karodziņiem, nevis literāļa."""
    questions, coding, parties, _ = _render_fixture(2)
    flagged = [{"short_name": sn, "pilna_programma_url": f"https://{sn.lower()}.lv/p",
                "pilna_programma_parlasita": "2026-09-20"} for sn in STARTING14[:3]]
    flagged += [{"short_name": sn, "pilna_programma_url": f"https://{sn.lower()}.lv/p"}
                for sn in STARTING14[3:6]]
    flagged += [{"short_name": sn} for sn in STARTING14[6:]]
    html = ptr.build(questions=questions, coding=coding, parties=parties,
                     saraksti=flagged, og_image_exists=False, pp_stats=(7, 2))
    assert "Pilnu programmu atradām 6 no 14" in html
    assert "Visas 3 atrastās ir pārlasītas pilnībā" in html
    plain = ptr.build(questions=questions, coding=coding, parties=parties,
                      saraksti=[{"short_name": sn} for sn in STARTING14],
                      og_image_exists=False, pp_stats=(7, 2))
    assert "pārlasītas pilnībā" not in plain


def test_render_gate_table_comes_from_data():
    """Metodoloģijas vārtu tabula + bilance nāk no kodējuma, nevis literāļi."""
    questions, coding, parties, saraksti = _render_fixture(2)
    html = ptr.build(questions=questions, coding=coding, parties=parties,
                     saraksti=saraksti, og_image_exists=False, pp_stats=(7, 2))
    assert "kopā 0 piekrīt : 0 iebilst" in html
    assert "2 no 2 jautājumiem mazākums ir 0 vai 1" in html
    assert "Nolasāmas" in html and "66 piekrīt" not in html
    assert "Programmas sola, neiebilst" in html


def test_render_leader_links_come_from_data():
    """Līderu saites uz profiliem + partiju saites šūnās nāk no datiem, nevis literāļi."""
    questions, coding, parties, saraksti = _render_fixture(2)
    leaders = {sn: {"name": f"Vārds {sn}", "slug": f"vards-{sn.lower()}-aj"}
               for sn in STARTING14}
    html = ptr.build(questions=questions, coding=coding, parties=parties,
                     saraksti=saraksti, og_image_exists=False, pp_stats=(7, 2),
                     leaders=leaders)
    assert "../politiki/vards-jv-aj.html" in html
    assert "Līderis:" in html
    assert '<div class="pt-name"><a href="../partijas/jv.html#programma">' in html
    no_leaders = ptr.build(questions=questions, coding=coding, parties=parties,
                           saraksti=saraksti, og_image_exists=False, pp_stats=(7, 2),
                           leaders={})
    assert "Līderis:" not in no_leaders


def test_d1_note_comes_from_saskana_output(tmp_path):
    """D1 teikums metodoloģijā nāk no saskaņas faila, nevis literāļa; bez faila — nav teikuma."""
    f = tmp_path / "saskana.md"
    f.write_text("# Kodētāju saskaņa — 3 kodētāji × 168 šūnas\n\n"
                 "- Pilnīga sakritība (kodētāji + atsauce): **143 no 168 šūnām**.\n"
                 "- Fleisa kappa pār kodētājiem: **0.893** (1 = pilnīga saskaņa; 0 = nejaušība).\n",
                 encoding="utf-8")
    s = ptr._d1_note(f)
    assert "sakrita 143 no 168 (85 %, kappa 0.893)" in s
    assert "Domstarpību šūnas izskata operators." in s
    assert ptr._d1_note(tmp_path / "nav.md") == ""


def test_d1b_note_comes_from_draft_comment(tmp_path, monkeypatch):
    """D1b teikums metodoloģijā nāk no kodejums_draft.md komentāra, ne literāļa."""
    draft = tmp_path / "kodejums_draft.md"
    draft.write_text('<!-- d1b: {"datums": "2026-09-22", "parceltas": 15, "zari": 7} -->\n',
                     encoding="utf-8")
    monkeypatch.setattr(ptr, "K_PATH", tmp_path / "kodejums.json")
    s = ptr._d1b_note()
    assert "2026-09-22" in s and "7 no tiem" in s and "15 atbildes" in s
    draft.unlink()
    assert ptr._d1b_note() == ""
...
def test_og_image_meta_follows_flag():
    ...
    kw = dict(questions=questions, coding=coding, parties=parties, saraksti=saraksti,
              pp_stats=(7, 2))  # bez tā build() lasa ražošanas DB (publiskajā CI nav)
