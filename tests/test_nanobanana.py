...
def _no_production_audit_rows(monkeypatch):
    """``generate_image()`` kopš 2026-09-07 raksta ``image_audit`` rindu.

    Bez šī vārta katrs šī faila tests ieliktu rindu ražošanas ``data/atmina.db``
    — un tā nav troksnis, bet reāla mēneša budžeta (5,00 USD) izlietojuma
    uzskaite. Audita saturu pārbauda ``tests/test_image_audit.py``; te to tikai
    savāc atmiņā. tests/conftest.py tripwire tabulu tagad arī sargā.
    """
    written: list[dict] = []
    monkeypatch.setattr(
        nanobanana, "_write_audit_row",
        lambda kind, model, prompt, aspect, cost, status, error_message, db_path:
            written.append({"kind": kind, "status": status, "cost": cost}),
    )
    return written
