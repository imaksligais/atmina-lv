...
class TestSaeimaKeywordWordBoundary:
    """`_SAEIMA_KEYWORD_MAP` matches at a word START, never mid-word (2026-09-15).

    Regresija: «ets» (ES emisiju tirdzniecība) trāpīja «pretstatā» iekšpusē,
    un veselības aprūpes deklarācija 136/P14 (2026-09-10) atvasināja `Klimats`.
    """

    def test_ets_inside_pretstata_is_not_klimats(self):
        raw = "Par palīdzības pieejamību pretstatā Satversmē garantētajam minimumam"
        assert normalize_topic(raw) != "Klimats"

    def test_real_ets_word_still_routes_to_klimats(self):
        assert normalize_topic("Par ETS sistēmas paplašināšanu") == "Klimats"
