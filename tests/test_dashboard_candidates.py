...
def test_panel_renders_on_index_without_leaking_to_public_render(cand_db):
    ...
    # Panelis rāda pēdējās 14 dienas no ŠODIENAS (today_lv) — fiksēts datums
    # šeit nokrita 2026-09-19, kad 09-04 izkrita no loga. Sēj vakardienu.
    day = (today_lv() - __import__("datetime").timedelta(days=1)).isoformat()
    db = sqlite3.connect(cand_db)
    _seed_hunt(db, timestamp=f"{day} 23:00:00",
               details={"date": day, "rejected_candidates": [_candidate()]})
    db.commit()
    db.close()
