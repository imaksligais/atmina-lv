...
class TestGetDb:
    def test_returns_connection(self):
        ...
        os.close(fd)
        # get_db refuses an EXISTING 0-byte file (see
        # tests/test_db_refuses_empty_file.py); a nonexistent path is the
        # legitimate lazy-create case this test wants.
        os.unlink(path)
        try:
            db = get_db(path)
...
class TestInsertDocument:
    ...
    def test_url_dedup_merges_politician_links(self):
        """An edited article must still link politicians named only in the new text.

        The URL-first dedup branch used to UPDATE the row and return without
        writing `politician_links`. Because the later mention scan
        (matcher.py::link_politicians_to_documents) only offers documents that
        have NO junction rows at all, a politician who first appears in the
        EDITED version was never linked — permanently and silently.
        """
        fd, path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        try:
            ...
            seed.execute(
                ...
                "VALUES (10, 'Andris Kulbergs', 'tracked')"
            )
            seed.execute(
                "INSERT INTO tracked_politicians (id, name, relationship_type) "
                "VALUES (17, 'Ainārs Šlesers', 'tracked')"
            )
            seed.commit()
            seed.close()

            url = "https://www.lsm.lv/raksts/edited-article"
            first_id = insert_document(
                content="Pirmā versija par Kulbergu. " * 20,
                source_id=None, platform="web", source_url=url,
                politician_links=[(10, "subject")], db_path=path,
            )
            assert first_id is not None

            # Publisher edits the article; a second politician is now named.
            second_id = insert_document(
                content="Labotā versija par Kulbergu un Šleseru. " * 20,
                source_id=None, platform="web", source_url=url,
                politician_links=[(10, "subject"), (17, "mentioned")], db_path=path,
            )
            assert second_id == first_id, "URL dedup must reuse the same row"

            db = get_db(path)
            # Prove we went through the URL branch, not content_hash dedup.
            content = db.execute(
                "SELECT content FROM documents WHERE id=?", (first_id,)
            ).fetchone()["content"]
            assert content.startswith("Labotā versija")
            links = {
                (r["politician_id"], r["role"])
                for r in db.execute(
            ...
            db.close()
            assert links == {(10, "subject"), (17, "mentioned")}
        finally:
            _safe_unlink(path)

    def test_url_dedup_overwrites_title_on_edit(self):
        """Doc-72446 class: a re-published URL must carry the NEW title.

        The UPDATE branch used `title=COALESCE(title, ?)`, so a publisher who
        put a different story behind the same URL left the row with the OLD
        headline over the NEW body — the exact signature (title disagreeing
        with the first content line) that made the class invisible.
        """
        fd, path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        try:
            init_db(path)
            url = "https://www.leta.lv/raksts/re-used-url"
            first_id = insert_document(
                content="Pirmais stāsts par budžetu. " * 20,
                source_id=None, platform="web", source_url=url,
                title="Pirmais virsraksts", db_path=path,
            )
            assert first_id is not None

            second_id = insert_document(
                content="Pilnīgi cits stāsts par veselību. " * 20,
                source_id=None, platform="web", source_url=url,
                title="Otrais virsraksts", db_path=path,
            )
            assert second_id == first_id

            db = get_db(path)
            row = db.execute(
                "SELECT title, content FROM documents WHERE id=?", (first_id,)
            ).fetchone()
            db.close()
            assert row["title"] == "Otrais virsraksts"
            assert row["content"].startswith("Pilnīgi cits stāsts")
        finally:
            _safe_unlink(path)

    def test_url_dedup_keeps_old_title_when_new_is_none(self):
        """A re-fetch that carries no title must not WIPE the stored one."""
        fd, path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        try:
            init_db(path)
            url = "https://www.leta.lv/raksts/no-title-refetch"
            first_id = insert_document(
                content="Sākotnējais teksts par nodokļiem. " * 20,
                source_id=None, platform="web", source_url=url,
                title="Saglabājamais virsraksts", db_path=path,
            )
            insert_document(
                content="Labotais teksts par nodokļiem. " * 20,
                source_id=None, platform="web", source_url=url,
                title=None, db_path=path,
            )
            db = get_db(path)
            title = db.execute(
                "SELECT title FROM documents WHERE id=?", (first_id,)
            ).fetchone()["title"]
            db.close()
            assert title == "Saglabājamais virsraksts"
        finally:
            _safe_unlink(path)

    def test_url_dedup_resets_reviewed_at_on_content_change(self):
        """Changed content behind the same URL must re-enter the extraction queue.

        Claims extracted from the OLD text stay attached to the row; the NEW
        story was never reviewed, and `reviewed_at` kept it out of every
        pending queue forever (doc 72446, signature scraped_at > reviewed_at).
        """
        fd, path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        try:
            init_db(path)
            url = "https://www.leta.lv/raksts/reviewed-then-edited"
            first_id = insert_document(
                content="Vecais stāsts par izglītību. " * 20,
                source_id=None, platform="web", source_url=url,
                title="Vecais", db_path=path,
            )
            db = get_db(path)
            db.execute(
            ...
            insert_document(
                content="Jaunais stāsts par aizsardzību. " * 20,
                source_id=None, platform="web", source_url=url,
                title="Jaunais", db_path=path,
            )

            db = get_db(path)
            reviewed_at = db.execute(
                "SELECT reviewed_at FROM documents WHERE id=?", (first_id,)
            ).fetchone()["reviewed_at"]
            db.close()
            assert reviewed_at is None, (
                "edited content must clear reviewed_at so the doc re-enters the queue"
            )
        finally:
            _safe_unlink(path)

    def test_identical_refetch_keeps_reviewed_at(self):
        """Counter-trap: an unchanged re-fetch must NOT clear the review stamp.

        Otherwise every routine re-scrape would un-review the whole corpus and
        the extraction queue would grow without bound. Identical bytes are
        caught by the content_hash branch before the URL branch runs, so no
        ...
        try:
            init_db(path)
            url = "https://www.leta.lv/raksts/unchanged"
            content = "Nemainīgs teksts par tiesu sistēmu. " * 20
            first_id = insert_document(
                content=content, source_id=None, platform="web",
                source_url=url, title="Virsraksts", db_path=path,
            )
            stamp = now_lv()
            db = get_db(path)
            db.execute(
                "UPDATE documents SET reviewed_at = ? WHERE id = ?", (stamp, first_id)
            )
            db.commit()
            db.close()
            ...
            assert again is None, "identical bytes are content_hash duplicates"
            ...
            db.close()
            assert reviewed_at == stamp
        finally:
            _safe_unlink(path)

    def test_non_web_platform_does_not_take_url_branch(self):
        """The URL-first UPDATE (and its reviewed_at reset) is web-only."""
        fd, path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        try:
            init_db(path)
            ...
            first_id = insert_document(
                content="Tvīta teksts par ekonomiku. " * 20,
                source_id=None, platform="x", source_url=url,
                title="Tvīts", db_path=path,
            )
            db = get_db(path)
            db.execute(
                "UPDATE documents SET reviewed_at = ? WHERE id = ?",
            ...
            second_id = insert_document(
                content="Cits tvīta teksts par drošību. " * 20,
                source_id=None, platform="x", source_url=url,
                title="Tvīts 2", db_path=path,
            )
            assert second_id is not None and second_id != first_id, (
                "non-web platforms must insert a new row, not update in place"
            )

            db = get_db(path)
            row = db.execute(
                "SELECT title, reviewed_at FROM documents WHERE id=?", (first_id,)
            ).fetchone()
            db.close()
            assert row["title"] == "Tvīts"
            assert row["reviewed_at"] is not None
        finally:
            _safe_unlink(path)

    def test_url_dedup_preserve_scraped_at(self):
        """preserve_scraped_at=True keeps the stored ingest-day bucket.

        The truncated-doc backfill re-fetches through this UPDATE branch; a
        historic doc must not jump to today's scraped_at, or every day-scoped
        reader would misattribute it to the backfill day.
        """
        fd, path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        try:
            init_db(path)
            url = "https://www.lsm.lv/raksts/backfill-me"
            first_id = insert_document(
                content="Apgrieztais īsais teksts. " * 5,
                source_id=None, platform="web", source_url=url, db_path=path,
            )
            old_stamp = "2020-01-01 00:00:00"
            db = get_db(path)
            db.execute(
                "UPDATE documents SET scraped_at = ? WHERE id = ?",
                (old_stamp, first_id),
            )
            db.commit()
            db.close()
            ...
            second_id = insert_document(
                content="Pilnais teksts ir daudz garāks par veco. " * 20,
                source_id=None, platform="web", source_url=url,
                preserve_scraped_at=True, db_path=path,
            )
            assert second_id == first_id
            db = get_db(path)
            scraped_at = db.execute(
                "SELECT scraped_at FROM documents WHERE id=?", (first_id,)
            ).fetchone()["scraped_at"]
            db.close()
            assert scraped_at == old_stamp
        finally:
            _safe_unlink(path)

    def test_url_dedup_default_refreshes_scraped_at(self):
        """Default (preserve_scraped_at=False) keeps the old behaviour:
        an edited article's re-fetch restamps scraped_at to now."""
        fd, path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        try:
            init_db(path)
            url = "https://www.lsm.lv/raksts/normal-refetch"
            first_id = insert_document(
                content="Īsā versija. " * 5,
                source_id=None, platform="web", source_url=url, db_path=path,
            )
            old_stamp = "2020-01-01 00:00:00"
            db = get_db(path)
            ...
            insert_document(
                content="Labotā versija ir garāka. " * 20,
                source_id=None, platform="web", source_url=url, db_path=path,
            )
            db = get_db(path)
            scraped_at = db.execute(
...
class TestStoreClaimEmbeddingBytes:
    ...
    (write-contention design; see db.store_claim docstring).
    """
    ...
    def test_saeima_vote_not_auto_embedded(self, seeded_db):
        """2026-08-21 operatora verdikts: saeima_vote claims netiek auto-embedoti
        (98% no claim_vectors bija mehāniskas balsojumu rindas, kuras neviens
        kNN lasītājs neatgriež — T10; retorika↔balsojums prasa strukturālu SQL
        vaicājumu — T9). Eksplikīti padots embedding_bytes tiek saglabāts
        (test_provided_embedding_bytes_stored... sedz pozīcijas ceļu)."""
        from src.db import store_claim

        claim_id = store_claim(
            opponent_id=1, document_id=None, topic="NATO",
            stance="Atbalsta X likumprojektu — balso PAR",
            quote=None, confidence=1.0, reasoning="Saeimas balsojums 2026-08-21.",
            salience=0.5, source_url="https://titania.saeima.lv/v/9", stated_at=None,
            claim_type="saeima_vote", db_path=seeded_db,
        )
        assert _read_claim_vector(seeded_db, claim_id) is None, (
            "saeima_vote claim got an auto-vector — the 2026-08-21 no-embed "
            "verdict regressed"
        )
...
class TestStoreClaimDiacriticGuardrail:
    ...
    def test_rejects_stripped_quote_against_the_source(self, seeded_db):
        """A stripped quote is refused by SOURCE comparison, not by the ratio.

        This test used to assert `match="diacritic"`. `claims.quote` is VERBATIM
        (CLAUDE.md), so the ratio gate was asking the wrong question of it and
        failed both ways — it erased claim #555664's authentic low-diacritic
        quote (that row still carries an empty quote today, because a quote may
        not be edited, so refusing it dropped the provenance outright), while a
        ratio cannot see one damaged word in a diacritic-rich sentence.
        See tests/test_quote_verbatim_gate.py for the full rationale.
        """
        from src.db import store_claim
        good = (
            "Daudz tiek runāts par airBaltic izmaksu sadaļu, bet ne "
            "tik daudz par ieņēmumu sadaļu un eksporta potenciālu."
        )
        bad_quote = (
        ...
        db = get_db(seeded_db)
        db.execute("UPDATE documents SET content = ? WHERE id = 1", (good,))
        db.commit()
        db.close()
    ...
    def test_allows_unverifiable_quote(self, seeded_db):
        """When the source cannot confirm the wording, the write proceeds.

        Measured 2026-08-03: 1408 of 4735 live quotes are not locatable
        character-exactly (English, elisions, typographic quotes, re-fetched
        bodies). The old ratio gate rejected 6 of those 1408 — four are already
        the open operator triage set (#6966, #7003, #7018, #7518) and one
        (#7512) is authentic dialectal speech it refused wrongly. Reinstating a
        ratio fallback here would buy almost nothing and would rebuild the
        silent-provenance-loss path.
        """
        from src.db import store_claim
        claim_id = store_claim(
            opponent_id=1, document_id=1, topic="airBaltic",
            stance="OK", quote="Izsviest kriveji uora nu ANO!!! Es par tu pasaraksteju.",
            confidence=0.5, reasoning="", salience=0.5,
            source_url="https://test.lv/a", stated_at=None,
            db_path=seeded_db,
        )
...
class TestSearchSimilarClaimsFilter:
    ...
    def seeded_db(self):
        ...
        # Store a saeima_vote: "Atbalsta X" (voting record). Kopš 2026-08-21
        # verdikta store_claim vairs neauto-embedo saeima_vote claims — kNN
        # tipa-filtra mehānikas testam vektors jāpadoda eksplikīti.
        store_claim(
            ...
            claim_type="saeima_vote",
            embedding_bytes=_float_list_to_bytes(
                embed_text("Aizsardzība un drošība: Atbalsta X likumprojektu — balso PAR")
            ),
            db_path=path,
        )
        yield path
...
class TestStoreClaimProvenanceGuard:
    """Only `saeima_vote` claims may omit document_id.

    Vote claims carry provenance through saeima_individual_votes → parent
    saeima_votes.url, so they have no document row to point at (CLAUDE.md Data
    Contract #6). Every other claim_type without a document has no re-fetchable
    provenance at all.

    The rule can only live here: the Pydantic `Claim` model does not carry
    claim_type, so it cannot distinguish a legal NULL from a lost one. Until
    2026-08-01 the two layers actively disagreed — `src/models.py` declared
    `document_id: int`, so `tools.store_claim()` raised ValidationError on a
    legal vote claim while `db.store_claim()` accepted the identical row, and
    nothing enforced the requirement for position/commentary at all.
    """
    ...
    def test_saeima_vote_may_omit_document_id(self, seeded_db):
        from src.db import store_claim
        claim_id = store_claim(
            opponent_id=1, document_id=None, topic="Budžets",
            stance="Saeimas balsojums 2026-01-01: Test balsoja Par",
            quote=None, confidence=1.0, reasoning="", salience=0.7,
            source_url="https://titania.saeima.lv/vote/1", stated_at=None,
            claim_type="saeima_vote", db_path=seeded_db,
        )
        db = get_db(seeded_db)
        row = db.execute(
            "SELECT document_id FROM claims WHERE id=?", (claim_id,)
        ).fetchone()
        db.close()
        assert row["document_id"] is None

    @pytest.mark.parametrize("claim_type", ["position", "commentary", "program_promise"])
    def test_other_claim_types_require_document_id(self, seeded_db, claim_type):
        from src.db import store_claim
        with pytest.raises(ValueError, match="requires a document_id"):
            store_claim(
                opponent_id=1, document_id=None, topic="NATO",
                stance="atbalsta paplašināšanu", quote=None, confidence=0.8,
                reasoning="pamatojums", salience=0.5,
                source_url="https://test.lv/a", stated_at=None,
                claim_type=claim_type, db_path=seeded_db,
            )

    def test_position_with_document_id_still_stores(self, seeded_db):
    ...
    def test_model_accepts_null_document_id(self):
        """src/models.py must not reject what db.store_claim accepts."""
        from src.models import Claim
        claim = Claim(
            opponent_id=1, document_id=None, topic="Budžets",
            stance="balsoja Par", confidence=1.0, reasoning="", salience=0.7,
        )
        assert claim.document_id is None

    def test_model_still_requires_the_field(self):
        """Nullable must not become optional — a dropped key is a bug, not a vote."""
        import pydantic
        from src.models import Claim
        with pytest.raises(pydantic.ValidationError):
            Claim(
                opponent_id=1, topic="Budžets", stance="balsoja Par",
                confidence=1.0, reasoning="", salience=0.7,
            )


class TestInsertChunksReplaces:
    """insert_chunks must REPLACE a document's chunks, never append to them.

    insert_document's URL-first branch returns the EXISTING doc id when a
    re-scrape brings a changed body, and every caller then re-embeds the full
    text and calls insert_chunks. While that was a plain INSERT, each re-scrape
    stacked another full set of chunks on the same document — 586 documents and
    690 surplus rows by 2026-08-02, each with a live document_vectors row, so
    semantic search returned text the article no longer contained.
    """

    @staticmethod
    def _chunks(texts):
        return [(i, t, [0.0] * 384) for i, t in enumerate(texts)]

    @staticmethod
    def _count(path, doc_id):
        db = get_db(path)
        chunks = db.execute(
        ...
        db.close()
        return [(r["chunk_index"], r["content"]) for r in chunks]

    def test_second_call_replaces_rather_than_appends(self):
        from src.db import insert_chunks
        ...
        try:
            init_db(path)
            doc_id = insert_document(
                content="sākotnējais teksts " * 20, source_id=None,
                platform="web", source_url="https://test.lv/a", db_path=path,
            )
            insert_chunks(doc_id, self._chunks(["vecais a", "vecais b"]), db_path=path)
            assert len(self._count(path, doc_id)) == 2

            insert_chunks(doc_id, self._chunks(["jaunais a"]), db_path=path)

            rows = self._count(path, doc_id)
            assert rows == [(0, "jaunais a")], rows

        finally:
            _safe_unlink(path)

    def test_stale_vectors_go_with_the_stale_chunks(self):
        """document_vectors is a vec0 virtual table — it does NOT cascade."""
        import sqlite_vec
        from src.db import insert_chunks
        ...
        try:
            ...
            doc_id = insert_document(
                content="sākotnējais teksts " * 20, source_id=None,
                platform="web", source_url="https://test.lv/b", db_path=path,
            )
            insert_chunks(doc_id, self._chunks(["vecais a", "vecais b"]), db_path=path)
            insert_chunks(doc_id, self._chunks(["jaunais a"]), db_path=path)
            ...
            sqlite_vec.load(db)
            db.enable_load_extension(False)
            live = db.execute("SELECT id FROM document_chunks WHERE document_id=?",
                              (doc_id,)).fetchall()
            live_ids = {r["id"] for r in live}
            vec_ids = {
                r["chunk_id"] for r in db.execute("SELECT chunk_id FROM document_vectors")
            }
            db.close()

            assert vec_ids == live_ids, f"orphaned vectors: {vec_ids - live_ids}"
        finally:
            _safe_unlink(path)
