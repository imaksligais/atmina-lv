...
# Teksta formāti, ko šis vārts sargā. PNG un citi binārie NAV sarakstā:
# tie pēc formāta satur NUL baitus, tāpēc to skenēšana padara vārtu pastāvīgi
# sarkanu (2026-09-10: 20 `wiki/synthesis/images/*.png` pēc `5c488fa5`, kad
# sintēžu hero attēli kļuva versionēti). Pastāvīgi sarkans vārts ir tā paša
# defekta spogulis kā vārts, kas nevar nokrist — abus sāk ignorēt.
_WIKI_TEXT_SUFFIXES = {".md", ".base", ".html", ".json", ".yaml", ".yml"}

# Apakšējā robeža skenētajiem failiem: bez tās suffix filtrs var kļūt par
# vārtiem, kas neko neskatās (pārsauc paplašinājumu → 0 skenētu → zaļš).
# 2026-09-11 mērījums: 370 teksta faili no 390 izsekotajiem `wiki/`.
_WIKI_TEXT_FLOOR = 300
...
def test_no_tracked_wiki_page_contains_nul_bytes():
    ...
    for rel in listed:
        if not rel.strip():
            continue
        p = root / rel
        if p.suffix.lower() not in _WIKI_TEXT_SUFFIXES or not p.is_file():
            continue
        scanned += 1
        if b"\x00" in p.read_bytes():
            bad.append(rel)

    assert scanned >= _WIKI_TEXT_FLOOR, (
        f"vārts neko neskatījās: skenēti tikai {scanned} teksta faili "
        f"(gaidīts >= {_WIKI_TEXT_FLOOR}) — pārbaudi _WIKI_TEXT_SUFFIXES"
    )
    assert not bad, f"NUL bytes in tracked wiki pages ({scanned} skenēti): {bad}"
...
def test_wiki_sync_prunes_topic_page_when_claims_gone(tmp_path):
    """2026-08-23 defekts: dzēšot tēmas pēdējo claim, lapa palika kokā.

    `wiki_sync` pārrakstīja lapas, kurām dati IR, bet mapi nepārstaigāja, tāpēc
    `topics/uznemejdarbiba-un-valsts-iejauksanas.md` kļuva par `orphan_page` +
    `isolated_topic` (lints: orphans 0→1). Vārtiem jābūt simetriskiem ar
    izveidošanu — izmet tieši to, ko tas pats vaicājums vairs neizveidotu.
    """
    import sqlite3
    ...
    db.execute(
        "INSERT INTO claims (opponent_id, topic, stance, confidence, salience,"
        " stated_at, created_at) VALUES"
        " (1, 'Uzņēmējdarbība un valsts iejaukšanās', 'Atbalsta', 0.8, 0.6,"
        " '2026-08-21', datetime('now'))"
    )
    db.commit()
    db.close()
    ...
    wiki_dir = tmp_path / "wiki"
    result = wiki_sync(db_path=db_path, wiki_dir=str(wiki_dir))
    page = wiki_dir / "topics" / "uznemejdarbiba-un-valsts-iejauksanas.md"
    assert page.exists(), "lapai jāeksistē, kamēr claim ir"
    assert result["topics_pruned"] == []

    db = sqlite3.connect(db_path)
    db.execute("DELETE FROM claims WHERE topic = 'Uzņēmējdarbība un valsts iejaukšanās'")
    db.commit()
    db.close()

    result = wiki_sync(db_path=db_path, wiki_dir=str(wiki_dir))
    assert not page.exists(), "lapa bez neviena claim jāizmet"
    assert result["topics_pruned"] == ["uznemejdarbiba-un-valsts-iejauksanas.md"]

    # Dzīvā tēma NEDRĪKST aiziet līdzi, un apakšindekss paliek.
    assert (wiki_dir / "topics" / "budzets-un-finanses.md").exists()
    assert (wiki_dir / "topics" / "temas.md").exists()


def test_wiki_sync_prune_does_not_touch_person_or_party_pages(tmp_path):
    """Tikai `topics/` — person/party lapām semantika ir cita."""
    import sqlite3
    ...
    wiki_dir = tmp_path / "wiki"
    wiki_sync(db_path=db_path, wiki_dir=str(wiki_dir))
    person_pages = {p.name for p in _person_stub_files(wiki_dir)}
    party_pages = {p.name for p in (wiki_dir / "parties").glob("*.md")}
    assert person_pages and party_pages
    ...
    wiki_sync(db_path=db_path, wiki_dir=str(wiki_dir))
    assert {p.name for p in _person_stub_files(wiki_dir)} == person_pages
    assert {p.name for p in (wiki_dir / "parties").glob("*.md")} == party_pages


def test_index_personas_label_counts_the_pages_actually_written(tmp_path):
    """`- [[persons/personas|Politiķi]] — N profili` must equal N persona pages.

    Regression for the 2026-09-05 docs-vs-code audit § 5: the label used
    `relationship_type='tracked'` (167 live) while both `_build_persons_index`
    and the `wiki_sync()` person loop write a page for every
    `relationship_type != 'inactive'` row (196 live). A link's count must be
    the count of the page it points at, so this asserts against the two
    independent witnesses — the returned `persons` counter AND the files on
    disk — rather than re-running the label's own query.

    The fixture leans on this on purpose: `_create_test_db` inserts both
    politicians with the schema default `relationship_type='neutral'`, so the
    old `tracked`-only denominator rendered `0 profili` next to two written
    pages.
    """
    import re
    ...
    index = (wiki_dir / "index.md").read_text(encoding="utf-8")
    match = re.search(r"\[\[persons/personas\|Politiķi\]\] — (\d+) profili", index)
    assert match, f"personas rinda nav atrasta indeksā:\n{index}"
    labelled = int(match.group(1))

    pages_written = len(_person_stub_files(wiki_dir))
    assert pages_written >= 2, f"saucējs sabrucis — uzrakstītas {pages_written} lapas"
    assert labelled == pages_written, (
        f"indekss sola {labelled} profilus, bet wiki_sync uzrakstīja "
        f"{pages_written} personu lapas"
    )
    assert labelled == result["persons"]


def test_index_footnote_states_the_query_behind_each_politician_count(tmp_path):
    """CLAUDE.md korolārijs (b): skaitlis bez vaicājuma nav pierādījums.

    Divi dažādi politiķu skaitļi vienā lapā ir likumīgi tikai tad, ja lapa
    pati pasaka, ar ko tie atšķiras.
    """
    from src.wiki import wiki_sync
    ...
    index = (wiki_dir / "index.md").read_text(encoding="utf-8")
    assert "**Saucēji.**" in index
    assert "relationship_type='tracked'" in index
    assert "relationship_type!='inactive'" in index
