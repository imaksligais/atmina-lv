...
def test_publish_gate_blocks_unapproved_brief(tree, publish_setup, capsys):
    ...
    out = capsys.readouterr().out
    assert "1 pārskata lapas" in out, "saucējs obligāts — cik lapas pārbaudītas"


def test_publish_gate_counts_both_gates_independently(tree, publish_setup, capsys):
    """2026-08-23: `elif` ķēde drukāja „0 bez publicēšanas apstiprinājuma",
    kaut `publish_approvals` rindas nebija — attēla vārti nostrādāja pirmie un
    lapa nekad nesasniedza apstiprinājuma zaru. Skaitlis, kas nevar būt
    ne-nulle, nav saucējs (CLAUDE.md § Working Conventions).
    """
    _page(tree)  # ne approved=1 attēla, ne publish_approvals rindas
    problems = check_output.check_publish_gate([])
    assert problems, "lapai bez abiem apstiprinājumiem jābūt bloķētai"
    out = capsys.readouterr().out
    assert "1 bez publicēšanas apstiprinājuma" in out, out
    assert "1 bez approved=1 attēla" in out, out


def test_publish_gate_orphan_reports_image_gate_as_unchecked(tree, publish_setup, capsys):
    """Orfānam attēla vārti NAV izvērtējami (nav note_id) — tas ir
    „nepārbaudīts", ne „izturēja". Apstiprinājuma vārti orfānam TOMĒR ir
    izvērtējami, jo to atslēga ir lapas slugs.
    """
    _page(tree, name="2026-01-01.html")  # DB nav brief ar šo subjekta datumu
    check_output.check_publish_gate([])
    out = capsys.readouterr().out
    assert "1 attēla vārti nepārbaudīti (orfāni)" in out, out
    assert "0 bez approved=1 attēla" in out, out
    assert "1 bez publicēšanas apstiprinājuma" in out, out
...
def test_publish_gate_blocks_when_only_image_approved(tree, publish_setup, capsys):
    ...
    out = capsys.readouterr().out
    assert "1 pārskata lapas" in out
    assert "1 bez publicēšanas apstiprinājuma" in out, "saucējs obligāts"
