...
def _fake_site(redirect_html: bool = False, security_txt: bytes | None = -1, server="cloudflare",
               csp: str = CSP, drop_header: str | None = None):
...
# Mutation sample 2026-09-24: deleting either half of check 1 passed this whole
# file — nothing asserted that a missing header or an inline-script CSP fails.
# CLAUDE.md § Output Conventions: script-src must never carry 'unsafe-inline'.
def test_missing_security_header_is_a_failure():
    checks = vh.run_checks("https://x.example", SITEMAP,
                           _fake_site(drop_header="x-content-type-options"),
                           sample=5, hero_paths=["images/briefs/a.png"])
    by = {c.name: c for c in checks}
    assert not by["security_headers"].passed
    assert "x-content-type-options" in by["security_headers"].detail
    assert by["security_headers"].examined == 3


def test_unsafe_inline_script_src_is_a_failure():
    bad = "default-src 'self'; script-src 'self' 'unsafe-inline'"
    checks = vh.run_checks("https://x.example", SITEMAP, _fake_site(csp=bad),
                           sample=5, hero_paths=["images/briefs/a.png"])
    by = {c.name: c for c in checks}
    assert not by["security_headers"].passed
    assert "unsafe-inline" in by["security_headers"].detail


def test_unsafe_inline_in_style_src_only_is_allowed():
    # The gate is about scripts; an inline-style allowance must not trip it.
    ok = "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self'"
    checks = vh.run_checks("https://x.example", SITEMAP, _fake_site(csp=ok),
                           sample=5, hero_paths=["images/briefs/a.png"])
    by = {c.name: c for c in checks}
    assert by["security_headers"].passed
