...
def test_audit_finds_mismatched_subject_rows(tmp_db):
    ...
    mismatches, _scanned = find_mismatched_rows(tmp_db)
...
def test_audit_reports_scanned_subject_denominator(tmp_db):
    """Denominator counts every role='subject' junction row examined, not findings."""
    from scripts.audit_junction_roles import find_mismatched_rows
    ...
    insert_document(
        ...
        platform="twitter",
        language="lv",
        source_url="https://x.com/krusts/status/1",
        politician_links=[(45, "subject")],
    )
    insert_document(
        content="Own tweet two.",
        source_id=None,
        platform="twitter",
        language="lv",
        source_url="https://x.com/krusts/status/2",
        politician_links=[(45, "subject")],
    )

    mismatches, scanned = find_mismatched_rows(tmp_db)
    assert mismatches == []
    assert scanned == 2
