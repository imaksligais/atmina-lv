...
def test_build_prompt_includes_negative_constraints():
    ...
    vm = get_visual("airBaltic")
    p = prompt.build_prompt(vb, vm, style_key="editorial")
    # Negative constraints section must explicitly forbid people, logos, and
    # (since 2026-09-13) every national flag other than Latvia's and the EU's
    assert "people" in p.lower() or "faces" in p.lower()
    assert "no other national flag" in p.lower()
    assert "logo" in p.lower()


@pytest.mark.parametrize("constant", ["NEGATIVE_CONSTRAINTS", "TEXT_FREE_CONSTRAINTS"])
def test_constraints_allow_lv_and_eu_flags_but_no_other(constant):
    """2026-09-13: the blanket «national flags» ban contradicted the agent prompt
    (§ «Vide attēlos ir LATVISKA», 2026-08-21), which describes the Saeima hall
    WITH Latvian + EU flags; image #324 with both flags was approved that day.
    Both constraint variants must carry the same rule — #324 was a `--no-text`
    image, so TEXT_FREE_CONSTRAINTS is the one that actually fired."""
    s = getattr(prompt, constant)
    assert "national flags" not in s, "blanket flag ban must be gone"
    assert "flags of Latvia and of the European Union" in s
    low = s.lower()
    assert "no other national flag" in low
    assert "party symbol" in low
    assert "logo" in low
...
def test_text_free_constraints_forbid_authorship_marks():
    """2026-09-04: the model drew an invented cursive artist signature in a
    `--no-text` brief image (brief_images #310). atmina.lv publishes anonymously,
    so a fabricated authorship mark is exactly what may not reach the site.
    The ban must name the class — "no text" alone was not read as covering it."""
    vb = {"topic": "Aizsardzība un drošība", "headline": "X", "stat": None}
    vm = {"metaphor": "closed barrier gate", "mood": "resolute", "accent": "deep navy"}
    p = prompt.build_prompt(vb, vm, style_key="editorial", no_text=True).lower()
    for phrase in ("artist signature", "autograph", "monogram", "initials", "credit line"):
        assert phrase in p, f"authorship-mark ban missing: {phrase!r}"
