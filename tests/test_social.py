...
# 2026-09-19 — projekta paša konts (@AtminaLV) nav politiķa dokuments.
# Astoņi @AtminaLV tvīti (05-14…09-18) ienāca kā platform='twitter' caur
# izsekoto politiķu plūsmām (atbilde / citāts / pavediens, kas tago
# politiķus), teksta skenējums tiem uzlika 'mentioned' saites, un doc 110938
# (09-18 pavediena 1/6) nonāca junction-atgūšanas kandidātos — pašcitēšanas
# cilpa: projekta pārskats sāktu ražot «pozīcijas» par sevi (backlog/dati-db.md
# § 2026-09-18 (f)). Abi rakstīšanas ceļi to tagad izmet pēc URL autora.

def test_store_tweets_drops_project_account_tweets(tmp_social_db):
    """A tweet whose URL author is the project's own X account is never stored
    as a document, whatever feed surfaced it and whoever it names."""
    from src.social import _store_tweets
    tweets = [{
        "text": "Dienas pārskats — 17. jūnijs. Andris Ozoliņš un Testa Politiķis Saeimā balso par nodokļiem.",
        "source_url": "https://x.com/AtminaLV/status/2067470323700605194",
        "created_at": "2026-06-18T04:51:51+00:00",
        "lang": "lv",
    }]
    stored = _store_tweets(tweets, opponent_id=1)

    assert stored == []
    assert tmp_social_db.execute("SELECT COUNT(*) FROM documents").fetchone()[0] == 0
    assert tmp_social_db.execute("SELECT COUNT(*) FROM document_politicians").fetchone()[0] == 0


def test_store_tweets_project_handle_match_is_case_insensitive(tmp_social_db):
    """X URLs carry the handle in whatever case the author typed; the guard
    compares lowercase (extract_twitter_author_handle already lowercases)."""
    from src.social import _store_tweets
    tweets = [{
        "text": "Sestdien, 5. septembrī, airBaltic strīds nonāk koalīcijas galdā — Andris Ozoliņš iebilst.",
        "source_url": "https://x.com/atminalv/status/2096472559403290633",
        "created_at": "2026-09-06T05:36:23+00:00",
        "lang": "lv",
    }]
    _store_tweets(tweets, opponent_id=1)
    assert tmp_social_db.execute("SELECT COUNT(*) FROM documents").fetchone()[0] == 0
...
def test_link_politicians_skips_project_account_documents(tmp_social_db):
    """The later text scan (link_politicians_to_documents) is the second write
    path: an unlinked project-authored tweet in the window must stay unlinked,
    or the junction deletion of the eight historical rows is undone tomorrow."""
    from src.matcher import link_politicians_to_documents

    doc_id = db_mod.insert_document(
        content="Ceturtdien Saeima Dzelzceļa likuma grozījumus pieņem; Andris Ozoliņš balso par.",
        source_id=None,
        platform="twitter",
        source_url="https://x.com/AtminaLV/status/2100821276159881413",
    )
    assert doc_id is not None
    linked = link_politicians_to_documents(days=2)

    assert doc_id not in linked
    assert _junctions(tmp_social_db, doc_id) == set()
