...
def test_enhance_empty_dash_cell_with_align_attr():
    ...
    out, _toc = _enhance_synthesis_html(html, "s")
    # `_label_table_cells` re-serializes via bs4, which sorts attributes —
    # assert the muted td keeps BOTH attrs, not a fixed attribute order.
...
# ── td[data-label]: mobilā kartīšu etiķetes ──────────────────────────
# Backlog «Sintēžu tabulas mobilajā ir kartītes BEZ kolonnu etiķetēm»:
# ≤768 px CSS pārvērš `.post-content table` rindas kartītēs un etiķete virs
# šūnas nāk no `td[data-label]::before`. Etiķetes pieliek `_label_table_cells`
# (`src.render._common`) — šeit tās jābūt uz sintēzes markdown tabulām,
# bet NE uz `syn-*` vidžetu tabulām (tās jau ir kartītes ar savu struktūru).


def test_enhance_adds_data_label_matching_header_text():
    html = (
        "<table><thead><tr><th>Tēma</th><th>Summa</th></tr></thead>"
        "<tbody><tr><td>Aizsardzība</td><td>5% no IKP</td></tr></tbody></table>"
    )
    out, _toc = _enhance_synthesis_html(html, "s")
    assert '<td data-label="Tēma">Aizsardzība</td>' in out
    assert '<td data-label="Summa">5% no IKP</td>' in out


def test_enhance_data_label_failsafe_on_cell_count_mismatch():
    """Rindas šūnu skaits ≠ galvenes kolonnu skaits → tabula neaiztikta."""
    html = (
...
def test_enhance_widget_table_gets_no_data_labels(tmp_path, monkeypatch):
    """Vidžetu tabulas etiķetes NEVAJAG — tās jau ir kartītes ar savu struktūru."""
    widgets = _widgets_root(tmp_path, monkeypatch)
    wdir = widgets / "s"
    wdir.mkdir(parents=True)
    (wdir / "m.html").write_text(
...
def test_enhance_empty_dash_cell_muted_before_labeling():
    """`<td>—</td>` saglabā `cell-empty` UN iegūst `data-label` — muting
    skrien pirms etiķetēšanas, citādi `data-label` atribūts salauztu
    `_EMPTY_TD_RE` matchi un tukšā šūna vairs neklusinātos."""
    html = (
        "<table><thead><tr><th>Tēma</th><th>Summa</th></tr></thead>"
    ...
    out, _toc = _enhance_synthesis_html(html, "s")
    assert 'class="cell-empty"' in out
    assert 'data-label="Summa"' in out
