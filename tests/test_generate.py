"""Tests for the src.render helpers — pure utility functions."""
...
def generate_db():
    ...
    db.executescript("""
        ...
        CREATE TABLE contradictions (
            id INTEGER PRIMARY KEY,
            opponent_id INTEGER, detected_at TEXT, confirmed INTEGER
        );
        ...
        INSERT INTO documents (id, scraped_at, platform) VALUES (1, '2026-04-16', 'web');
        INSERT INTO documents (id, scraped_at, platform) VALUES (2, '2026-04-16', 'twitter');
        INSERT INTO documents (id, scraped_at, platform) VALUES (3, '2026-04-16', 'x_mention');
        INSERT INTO documents (id, scraped_at, platform) VALUES (4, '2026-04-16', 'saeima');
...
class TestBlogPostFooter:
    ...
    def test_footer_uses_next_day_ingest_for_one_day_late_brief(self, generate_db):
        """A one-day backfill must not publish a false zero-document footer."""
        db = sqlite3.connect(generate_db)
        db.execute("UPDATE documents SET scraped_at='2026-04-17'")
        db.execute("UPDATE context_notes SET created_at='2026-04-17 08:30:00'")
        db.commit()
        db.row_factory = sqlite3.Row
        ...
        footer = posts[0]["footer"]
        assert footer["doc_count"] == 3
        assert footer["web"] == 1
        assert footer["twitter"] == 1
        assert footer["mentions"] == 1
    ...
    def test_footer_prefers_brief_marker_position_count(self, generate_db):
        """Visible topic-table count wins over the narrower stated-day query."""
        db = sqlite3.connect(generate_db)
        db.execute(
            "UPDATE context_notes SET content = content || ?",
            ("\n<!-- DIENAS STATS: 3 dokumenti · 5 pozīcijas "
             "(4 politiķu + 1 auditorijas) · 0 pretrunas -->",),
        )
        db.commit()
        db.row_factory = sqlite3.Row
        from src.render.blog import _fetch_blog_posts
        ...
        assert posts[0]["footer"]["positions"] == 5
    ...
    def test_slug_derived_from_topic_not_created_at(self):
        ...
        db.executescript("""
            ...
            CREATE TABLE claims (id INTEGER PRIMARY KEY, opponent_id INTEGER, document_id INTEGER, topic TEXT, stance TEXT, source_url TEXT, stated_at TEXT, claim_type TEXT NOT NULL DEFAULT 'position');
...
class TestPretrunaDetailTemplate:
    def test_renders_with_og_meta(self):
        from jinja2 import Environment, FileSystemLoader
        from src.render._common import _autolink_bills_filter, _salience_label
        env = Environment(loader=FileSystemLoader("templates"), autoescape=True)
        env.filters["autolink_bills"] = _autolink_bills_filter
        # 2026-09-06 (f9f9c3e2) pievienoja šablonam `salience_label`, bet šis
        # tests savu env veido pats — nereģistrēts filtrs to nogāza.
...
def test_fetch_commentary_about_returns_third_party_only(tmp_path):
    ...
    from src.render.politicians import _fetch_commentary_about
...
def test_fetch_politicians_excludes_commentators(tmp_path):
    ...
    from src.db import init_db, get_db
    from src.render.politicians import _fetch_politicians
...
class TestLastActivityTweetTime:
    ...
    def test_x_post_uses_published_at_not_scraped_at(self, tmp_path):
        from src.render._common import _get_last_activity
...
class TestActivityDisplayDate:
    """Kartītes kājenes datums: 'MM-DD' kārtējā gadā, 'YYYY-MM-DD' vecākiem
    gadiem — bez gada norādes vecs ieraksts izlasās kā nesens."""

    def test_current_year_strips_year(self):
        from datetime import date
        from src.render._common import _activity_display_date

        today = date(2026, 7, 26)
        assert _activity_display_date("2026-07-25", today) == "07-25"

    def test_current_year_keeps_tweet_time(self):
        from datetime import date
        from src.render._common import _activity_display_date

        today = date(2026, 7, 26)
        assert _activity_display_date("2026-07-25 14:03", today) == "07-25 14:03"

    def test_older_year_shows_full_date_and_drops_time(self):
        from datetime import date
        from src.render._common import _activity_display_date

        today = date(2026, 7, 26)
        assert _activity_display_date("2025-11-03", today) == "2025-11-03"
        assert _activity_display_date("2025-11-03 09:41", today) == "2025-11-03"

    def test_empty(self):
        from datetime import date
        from src.render._common import _activity_display_date

        assert _activity_display_date("", date(2026, 7, 26)) == ""

    def test_last_activity_carries_display_key(self, tmp_path):
        from src.db import init_db, get_db
        from src.render._common import _get_last_activity
        ...
        db.execute(
            ...
            "VALUES (1, 'T', 's', 'https://x.lv/a', '2020-01-15 10:00:00')"
        )
        db.commit()
        la = _get_last_activity(db, 1, "Test Politiķis")
        assert la is not None
        assert la["display"] == "2020-01-15"  # vecs gads → pilns datums
        db.close()
