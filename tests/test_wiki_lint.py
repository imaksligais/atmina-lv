...
def test_lint_reports_how_many_pages_it_scanned(tmp_path):
    """A green "0 issues" must be distinguishable from "found no files"
    (CLAUDE.md § a gate that cannot fail is not evidence; 2026-08-27 the
    quality reviewer had to derive the denominator externally to justify PASS)."""
    wiki = tmp_path / "wiki"
    (wiki / "persons").mkdir(parents=True)
    (wiki / "topics").mkdir()
    (wiki / "index.md").write_text("# idx\n", encoding="utf-8")
    (wiki / "persons" / "personas.md").write_text("# p\n", encoding="utf-8")
    (wiki / "persons" / "a.md").write_text("---\nname: A\n---\n", encoding="utf-8")
    (wiki / "topics" / "temas.md").write_text("# t\n", encoding="utf-8")

    stats = lint_wiki(str(wiki))["stats"]
    assert stats["pages_scanned"] == 4
    assert stats["pages_by_dir"] == {".": 1, "persons": 2, "topics": 1}

    empty = tmp_path / "empty"
    empty.mkdir()
    assert lint_wiki(str(empty))["stats"]["pages_scanned"] == 0
