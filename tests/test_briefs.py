...
class TestAktivakieTieBreakBySalience:
    """«Aktīvākie politiķi» (top 7): among politicians tied on position count,
    the higher MAX(salience) must enter the table before alphabetical order.

    Regresija (2026-09-12/13, `@quality-reviewer` abas dienas): `cnt DESC,
    p.name ASC` deva alfabētu kā vienīgo izšķīrēju starp 1-pozīcijas
    politiķiem, tāpēc dienas galvenais runātājs (Melnis, salience 0,85) tabulā
    nebija, kamēr seši alfabētiski agrāki 1-pozīcijas politiķi bija.
    """

    def _seed_eight_solo_speakers(self, db):
        # Base fixture already holds pid 1 (2 claims) and pid 2 (1 claim) on
        # 2026-04-07. Add 8 more solo speakers, alphabetically BEFORE «Zemais»
        # only by construction: names Aa..Ah (low salience) and one «Zz Svarīgais»
        # with the day's highest salience. With 7 rows total, alphabet alone
        # would cut «Zz Svarīgais»; salience must pull it in.
        for i, letter in enumerate("abcdefgh"):
            pid = 10 + i
            db.execute(
                "INSERT INTO tracked_politicians (id, name, party, relationship_type) "
                "VALUES (?, ?, 'JV', 'tracked')", (pid, f"A{letter} Zemais")
            )
            db.execute(
                "INSERT INTO claims (opponent_id, topic, stance, source_url, stated_at, salience) "
                "VALUES (?, 'Tēma', 'Zema nozīme', ?, '2026-04-07', 0.2)",
                (pid, f"https://x.lv/low{pid}"),
            )
        db.execute(
            "INSERT INTO tracked_politicians (id, name, party, relationship_type) "
            "VALUES (30, 'Zz Svarīgais', 'JV', 'tracked')"
        )
        db.execute(
            "INSERT INTO claims (opponent_id, topic, stance, source_url, stated_at, salience) "
            "VALUES (30, 'Tēma', 'Dienas galvenais izteikums', 'https://x.lv/top', '2026-04-07', 0.95)"
        )

    def test_highest_salience_solo_speaker_enters_before_alphabet(self, briefs_db):
        db = sqlite3.connect(briefs_db)
        self._seed_eight_solo_speakers(db)
        db.commit()
        db.close()
        brief = generate_daily_brief(db_path=briefs_db, date="2026-04-07")
        table = brief.split("## Aktīvākie politiķi", 1)[1].split("\n## ", 1)[0]
        assert "Zz Svarīgais" in table, table
        # And the cut still happens at 7 rows — salience reorders, never widens.
        rows = [ln for ln in table.splitlines() if ln.startswith("| ") and "Politiķis" not in ln and "---" not in ln]
        assert len(rows) == 7, rows
        # Among 1-position rows the salient one precedes every low-salience «A? Zemais».
        assert table.index("Zz Svarīgais") < table.index("Aa Zemais")
