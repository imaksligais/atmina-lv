...
def test_saeima_vote_rows_excluded_from_sweep_and_counted():
    """Verdikts 2026-08-21: `saeima_vote` vektorus neaiztiek (jaunie bez
    vektora, vēsturiskie paliek kādi ir). Tāpēc 13. pārbaude tos NEDRĪKST
    likt stale sarakstā — citādi audits pats baro rindas, ko
    `reembed_claims.py` nedrīkst pārrakstīt (2026-09-07: 150 no 171)."""
    path = _mkdb([(1, "T", "a", "T: a"), (2, "T", "b", "T: OLD")])
    db = sqlite3.connect(path)
    db.execute("ALTER TABLE claims ADD COLUMN claim_type TEXT DEFAULT 'position'")
    db.execute("UPDATE claims SET claim_type = 'saeima_vote' WHERE id = 2")
    db.commit()
    db.close()
    try:
        rep = run_audit(path, [1, 2], embed_fn=fake_embed, control_ids=[1])
        assert rep["vote_excluded"] == 1
        assert rep["checked"] == 1 and rep["stale"] == 0
        assert rep["stale_ids"] == []
    finally:
        _unlink(path)


def test_stand_in_without_claim_type_column_still_runs():
    path = _mkdb([(1, "T", "a", "T: OLD")])
    try:
        rep = run_audit(path, [1], embed_fn=fake_embed)
        assert rep["vote_excluded"] == 0 and rep["stale"] == 1
    finally:
        _unlink(path)
