...
def test_deploy_script_accepts_legacy_delete_flags_as_noops():
    """scripts/deploy.sh must keep accepting --no-delete / --delete.

    Līdz 2026-09-15 šis tests sargāja rsync additīvo noklusējumu (DELETE_FLAG="").
    Kopš Cloudflare migrācijas deploy vienmēr ir pilns koks — dzēšanas režīma
    vairs nav, bet karogus padod dashboard (šī modeļa `--no-delete`), katrs
    runbooks un `.claude/commands/*`; tiem jāpaliek pieņemtiem kā no-op.
    """
    import pathlib
    import re
    ...
    assert re.search(r"^\s*--no-delete\|--delete\)", text, re.MULTILINE), (
        "deploy.sh case-zaram jāpieņem gan --no-delete, gan --delete"
    )
    # Nekas no karogiem nedrīkst nonākt transportā: nav DELETE_FLAG mainīgā.
    assert "DELETE_FLAG" not in text, "DELETE_FLAG vairs nav — deploy ir pilns koks"
